...
public class Notifier implements SipListener {
	...
	public void processSubscribe(RequestEvent requestEvent, ServerTransaction serverTransaction) {
		...
		try {
			...
			Address address = addressFactory.createAddress("Notifier <sip:127.0.0.1:5070>");
	...
	public void init() {
		...
		properties.setProperty("gov.nist.javax.sip.DEBUG_LOG", "notifierdebug.txt");
		properties.setProperty("gov.nist.javax.sip.SERVER_LOG", "notifierlog.txt");
		...
		try {
			...
			ListeningPoint lp1 = sipStack.createListeningPoint("127.0.0.1", 5070, "tcp");
