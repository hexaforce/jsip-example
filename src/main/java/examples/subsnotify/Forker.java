...
public class Forker implements SipListener {
	...
	static {
		try {
			...
			logger.addAppender(new FileAppender(new SimpleLayout(), "forkeroutputlog.txt"));
		} catch (Exception ex) {
			throw new RuntimeException(ex);
		}
	}

	private static final String usageString = "java " + "examples.subsnotify.Forker \n" + ">>>> is your class path set to the root?";
	...
	private void doFork(Request orig, ServerTransaction st, int port) throws Exception {
		ViaHeader myVia = headerFactory.createViaHeader("127.0.0.1", udpProvider.getListeningPoint("udp").getPort(), "udp", null);
	...
	private void doForwardStateless(Request orig, ServerTransaction st) throws ParseException, InvalidArgumentException, SipException {
		...
		ViaHeader myVia = headerFactory.createViaHeader("127.0.0.1", 5065, "udp", top.getBranch());
		orig.addFirst(myVia);
	...
	private static void initFactories() {
		...
		properties.setProperty("gov.nist.javax.sip.DEBUG_LOG", "forkerdebug.txt");
		properties.setProperty("gov.nist.javax.sip.SERVER_LOG", "forkerlog.txt");
