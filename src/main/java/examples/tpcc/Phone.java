...
/**
 * This class is a UAC template. Shootist is the guy that shoots and shootmeA is the guy that gets shot.
 *
 * @author M. Ranganathan
 */

/*
 * KMC - SHOOTMEA IS EXACTLY THE SAME TO THE CLASIC SHOOTME , ONLY LISTENING ON PORT 5070
 */
...
public class Phone implements SipListener {
	...
	public void processInvite(RequestEvent requestEvent, ServerTransaction serverTransaction) {
		...
		try {
			...
			Address address = addressFactory.createAddress("ShootmeA <sip:" + myAddress + ":" + myPort + ";lr" + ">");
	...
	private void sendInviteOK() {
		try {
			if (inviteTid.getState() != TransactionState.COMPLETED) {
				System.out.println("shootmeA: Dialog state before 200: " + inviteTid.getDialog().getState());
				inviteTid.sendResponse(okResponse);
				System.out.println(myPort + " Dialog state after 200: " + inviteTid.getDialog().getState());
			}
		} catch (SipException ex) {
			ex.printStackTrace();
	...
	public void init() {
		...
		properties.setProperty("gov.nist.javax.sip.DEBUG_LOG", "phone" + myPort + "debuglog.txt");
		properties.setProperty("gov.nist.javax.sip.SERVER_LOG", "phone" + myPort + "log.txt");
