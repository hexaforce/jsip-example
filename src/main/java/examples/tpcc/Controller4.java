...
public class Controller4 implements SipListener {
			...
			if (response.getStatusCode() == Response.OK) {
				if (cseq.getMethod().equals(Request.INVITE)) {
					if (currentState.equals("first")) {
						...
						Request ackRequest = firstDialog.createAck(cseq.getSeqNumber());
						...
						second++;
						currentState = "second";
						Request requestSecond = this.createInvite(currentState, String.valueOf(second), callId, null, peerHostPortB);
						...
						Request ackRequest = secondDialog.createAck(cseq2.getSeqNumber());// secondDialog.createRequest(Request.ACK);
						logger.info("Sending ACK second " + secondDialog);
						secondDialog.sendAck(ackRequest);// secondDialog.sendAck(ackRequest);

						Request ackRequestFirst = this.firstDialog.createAck(cseq.getSeqNumber());
	...
	public Request createInvite(String headerName, String headerValue, String callerId, String tagVal, String peerHostPort) throws ParseException, InvalidArgumentException {
		...
		FromHeader fromHeader = headerFactory.createFromHeader(fromNameAddress, Integer.valueOf((int) Math.random() * 10000).toString());
	...
	public void init() throws IOException {
		...
		logger.addAppender(new FileAppender(new SimpleLayout(), "controllerconsolelog.txt"));
		...
		try {
			...
			Request request = this.createInvite(currentState, String.valueOf(first), null, null, peerHostPortA);
