...
public class Controller implements SipListener {
	...
	protected static final String usageString = "java " + "examples.ctd.ctdControll \n" + ">>>> is your class path set to the root?";
	...
	public void processBye(Request request, ServerTransaction serverTransactionId) {
		...
		try {
			...
			Dialog secondBye = (Dialog) (serverTransactionId.getDialog().getApplicationData());
			...
			if (response.getStatusCode() == Response.OK) {
				if (cseq.getMethod().equals(Request.INVITE)) {
					if (Secuencia.equals("first")) {
						...
						second++;
						Secuencia = "second";
						Request requestSecond = this.createInvite(Secuencia, String.valueOf(second), callId, null, peerHostPortB);
						// SDP for second Invite with first response
						// ContentTypeHeader
						requestSecond.setContent(response.getContent(), (ContentTypeHeader) (response.getHeader("Content-Type")));

						inviteSecond = sipProvider.getNewClientTransaction(requestSecond);
						...
						Request ackRequest = dialogSecond.createAck(cseq.getSeqNumber());// dialogSecond.createRequest(Request.ACK);
						System.out.println("Sending ACK second");
						dialogSecond.sendAck(ackRequest);// dialogSecond.sendAck(ackRequest);

						CSeqHeader cseqFirst = (CSeqHeader) responseFirstEvent.getResponse().getHeader(CSeqHeader.NAME);
						Request ackRequestFirst = responseFirstEvent.getDialog().createAck(cseqFirst.getSeqNumber());
						ackRequestFirst.setContent(response.getContent(), (ContentTypeHeader) (response.getHeader("Content-Type")));
						...
						responseFirstEvent.getDialog().setApplicationData(dialogSecond);
						dialogSecond.setApplicationData(responseFirstEvent.getDialog());
	...
	public Request createInvite(String headerName, String headerValue, String callerId, String tagVal, String peerHostPort) throws ParseException, InvalidArgumentException {
		...
		// create >From Header
		SipURI fromAddress = addressFactory.createSipURI(fromVal, fromSipAddressDomain);
		Address fromNameAddress = addressFactory.createAddress(fromAddress);
		fromNameAddress.setDisplayName(fromDisplayName);
		FromHeader fromHeader = headerFactory.createFromHeader(fromNameAddress, "12345");

		// create To Header
		SipURI toAddress = addressFactory.createSipURI(toVal, toSipAddressDomain);
		...
		CSeqHeader cSeqHeader = headerFactory.createCSeqHeader(Long.parseLong(headerValue), Request.INVITE);
		...
		// server-sided PA
		String methods = Request.INVITE + ", " + Request.ACK + ", " + Request.OPTIONS + ", " + Request.CANCEL + ", " + Request.BYE + ", " + Request.INFO + ", " + Request.REFER + ", " + Request.MESSAGE + ", " + Request.NOTIFY + ", " + Request.SUBSCRIBE;
		AllowHeader allowHeader = headerFactory.createAllowHeader(methods);
		request.addHeader(allowHeader);

		// Add the extension header. To mantain Flow I
		Header extensionHeader = headerFactory.createHeader(headerName, headerValue);
	...
	public void init() {
		...
		properties.setProperty("gov.nist.javax.sip.DEBUG_LOG", "controllerdebug.txt");
		properties.setProperty("gov.nist.javax.sip.SERVER_LOG", "controllerlog.txt");
		...
		try {
			...
			udpListeningPoint = sipStack.createListeningPoint("127.0.0.1", 5050, "udp");
		...
		try {
			...
			Request request = this.createInvite(Secuencia, String.valueOf(first), "", null, peerHostPortA);
