...
public class Shootist extends TestCase implements SipListener {
	...
	public void processBye(SipProvider provider, Request request, ServerTransaction serverTransactionId) {
		try {
			logger.info("shootist:  got a bye .");
			/*
			 * if (serverTransactionId == null) { serverTransactionId = provider.getNewServerTransaction(request); }
			 */
			...
			if (response.getStatusCode() == Response.OK) {
				if (cseq.getMethod().equals(Request.INVITE)) {
					if (!sendDelayedCancel)
						fail("Should not see OK for the Invite");
	...
	public SipProvider createSipProvider() {
		try {
			listeningPoint = ProtocolObjects.sipStack.createListeningPoint(host, port, transport);
	...
	public void sendInvite() {
		try {
			...
			ViaHeader viaHeader = ProtocolObjects.headerFactory.createViaHeader(host, sipProvider.getListeningPoint(transport).getPort(), transport, null);
			...
			RouteHeader routeHeader = ProtocolObjects.headerFactory.createRouteHeader(ProtocolObjects.addressFactory.createAddress(sipuri));
