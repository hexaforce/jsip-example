...
public class Shootme extends TestCase implements SipListener {
	...
	static {
		try {
			logger.addAppender(new FileAppender(new SimpleLayout(), ProtocolObjects.logFileDirectory + "shootmeconsolelog.txt"));
	...
	public void processRequest(RequestEvent requestEvent) {
		...
		logger.info("\n\nRequest " + request.getMethod() + " received at shootme " + " with server transaction id " + serverTransactionId);
	...
	public void processInvite(RequestEvent requestEvent, ServerTransaction serverTransaction) {
		...
		try {
			...
			Response ringingResponse = ProtocolObjects.messageFactory.createResponse(Response.RINGING, request);
	...
	public void processCancel(RequestEvent requestEvent, ServerTransaction serverTransactionId) {
		...
		try {
			...
			if (serverTx != null && (serverTx.getState().equals(TransactionState.TRYING) || serverTx.getState().equals(TransactionState.PROCEEDING))) {
	...
	public SipProvider createProvider() {
		try {
			...
			ListeningPoint lp = ProtocolObjects.sipStack.createListeningPoint(myAddress, myPort, transport);
