...
public class LeakingApp implements SipListener {
	...
	public LeakingApp() {
		...
		try {
			...
			ALLOW = headerFactory.createAllowHeader(Request.REGISTER + ',' + Request.INVITE + ',' + Request.CANCEL + ',' + Request.ACK + ',' + Request.BYE + ',' + Request.REFER + ',' + Request.INFO);
		} catch (Exception e) {
	...
	public void processRequest(RequestEvent a_oEvent) {
		...
		System.out.println("processRequest(): CallID=" + l_sCallID + ", Request= " + l_oRequest.getMethod());
	}
	...
	public void processResponse(ResponseEvent a_oEvent) {
		...
		System.out.println("processResponse(): CallID=" + l_sCallID + ", Response= " + l_oResponse.toString());
	}
	...
	private Request createRequest() {
		try {
			...
			ViaHeader viaHeader = headerFactory.createViaHeader(LeakAudit.sipStack.getIPAddress(), listeningPoint.getPort(), listeningPoint.getTransport(), null);
			...
			Request request = messageFactory.createRequest(requestURI, Request.INVITE, callIdHeader, cseqHeader, fromHeader, toHeader, viaList, maxForwards);
