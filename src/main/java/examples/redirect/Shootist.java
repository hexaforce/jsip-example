...
public class Shootist extends TestCase implements SipListener {
		...
		try {
			if (response.getStatusCode() == Response.OK) {
				...
				if (cseq.getMethod().equals(Request.INVITE)) {
					...
					ViaHeader viaHeader = protocolObjects.headerFactory.createViaHeader("127.0.0.1", sipProvider.getListeningPoint(protocolObjects.transport).getPort(), protocolObjects.transport, null);
					// add via headers
	...
	public SipProvider createProvider() throws Exception {
		...
		this.sipProvider = protocolObjects.sipStack.createSipProvider(listeningPoint);
		assertTrue("listening point should be the same as what the provider returns for this transport", listeningPoint == sipProvider.getListeningPoint(protocolObjects.transport));
		return sipProvider;
	...
	public void sendInvite() {
		...
		try {
			...
			// create Request URI
			this.requestURI = protocolObjects.addressFactory.createSipURI(toUser, peerHostPort);
	...
	public void processTransactionTerminated(TransactionTerminatedEvent transactionTerminatedEvent) {
		logger.info("Transaction terminated event recieved for " + transactionTerminatedEvent.getClientTransaction());
