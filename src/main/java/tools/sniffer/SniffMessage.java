 ...
 * @author M. Ranganathan (ported to 1.2) Jeff Adams submitted a patch for this file.
 *
 */
...
public class SniffMessage implements ParseExceptionListener {
	...
	private void getTime(ArrayList sniffMsgList) {
		...
		while (i.hasNext()) {
			...
			if (line.startsWith("Arrival Time")) {
				...
				SimpleDateFormat formatter = new SimpleDateFormat("MMM dd',' yyyy hh:mm:ss.SSS");
	...
	private void getSipMessage(ArrayList sniffMsgList) throws ParseException {
		...
		for (int i = sipIndex + 1; i < sdpIndex; i++) {
			...
			if (line.startsWith("Request-Line") || line.startsWith("Status-Line")) {
				msgBuffer = msgBuffer + line.substring(line.indexOf(":") + 1).trim() + "\r\n";
		...
		for (int j = sdpIndex; j < sniffMsgList.size(); j++) {
			...
			if (line.indexOf("(") > 0 && line.indexOf(")") > 0) {
				msgBuffer = msgBuffer + line.charAt(line.indexOf(":") - 2) + "=" + line.substring(line.indexOf(":") + 1).trim() + "\r\n";
			}
		}
	...
	public String toXML() {
		...
		xmlMessage += "<message from=\"" + sourceIP + "\" to=\"" + destIP + "\" time=\"" + time + "\" isSender=\"" + true + "\" callId=\"" + sipMessage.getCallId().getCallId() + "\" statusMessage=\"" + statusMessage + "\" transactionId=\"" + sipMessage.getTransactionId() + "\" firstLine=\"" + firstLine + "\">\n";
		xmlMessage += "<![CDATA[";
		xmlMessage += message;
		xmlMessage += "]]>\n";
		xmlMessage += "</message>\n";
	...
	public void handleException(ParseException ex, SIPMessage sipMessage, Class headerClass, String headerText, String messageText) throws ParseException {
		...
		try {
			if (headerClass.equals(Class.forName("gov.nist.javax.sip.header.From")) || headerClass.equals(Class.forName("gov.nist.javax.sip.header.To")) || headerClass.equals(Class.forName("gov.nist.javax.sip.header.ViaList")) || headerClass.equals(Class.forName("gov.nist.javax.sip.header.CSeq")) || headerClass.equals(Class.forName("gov.nist.javax.sip.header.CallId")))
				throw ex;
