...
public class TracesCanvas extends Canvas implements MouseListener, MouseMotionListener {
	...
	public Color[] ARROWS_COLOR = { Color.blue, new Color(94, 151, 185), Color.green, new Color(55, 223, 131), Color.orange, new Color(209, 177, 69), Color.magenta, new Color(187, 91, 185), Color.cyan, new Color(199, 239, 39) };
	...
	public Dimension getPreferredSize() {
		int width = FIRST_ACTOR_GAP + LAST_ACTOR_GAP + HORIZONTAL_GAP * (actors.size() - 1);
		int height = ACTORS_STRIPE + FIRST_ARROW_GAP + LAST_ARROW_GAP + VERTICAL_GAP * (arrows.size() - 1);
	...
	public void refreshTracesCanvas(TracesSession tracesSession, String tracesOrigin) {
	...
	public void drawTop(Graphics g) {
		...
		while (e.hasMoreElements()) {
			...
			g.setFont(newFont);
			g.drawString(origin, positionX - getFontMetrics(g.getFont()).stringWidth(origin) / 2, ACTORS_STRIPE - 15);

			if (actorsImage != null) {

				g.drawImage(actorsImage, positionX - actorsImage.getWidth(this) / 2, ACTORS_STRIPE / 3, this);
			} else {
		...
		try {
			...
			for (int i = 0; i < tracesSession.size(); i++) {
				...
				if (actors.get(from) == null) {
					actors.put(from, Integer.valueOf(sizeActors * HORIZONTAL_GAP + FIRST_ACTOR_GAP));
				}
				sizeActors = actors.size();
				if (actors.get(to) == null) {
					actors.put(to, Integer.valueOf(sizeActors * HORIZONTAL_GAP + FIRST_ACTOR_GAP));
				}
	...
	public void constructArrows() {
		...
		for (int i = 0; i < tracesSession.size(); i++) {
			...
			if (positionXFrom == positionXTo) {
				// This is a loop !!
				CircleArrow circleArrow = new CircleArrow(selected, arrowName, positionXFrom, positionY - 20, positionY + 20, 40, true, info);
				...
				StraightArrow straightArrow = new StraightArrow(selected, arrowName, positionXFrom, positionXTo, positionY - 31, positionY + 31, true, info);
	...
	public void selectMessage(int x, int y) {
		...
		if (newArrow == null) {
			...
			if (messageContentTextArea != null) {
				...
				messageContentTextArea.setText(tracesMessage.getMessageString());
				if (tracesMessage.debugLine != null && !tracesMessage.debugLine.equals("")) {

					tracesViewer.messageContentButton.setText("SIP message: (debug log line: " + tracesMessage.debugLine + " )");
				}
			}
	...
	public void setBackground(Graphics g) {
		...
		if (backgroundImage != null && backgroundImage.getWidth(this) != -1 && backgroundImage.getHeight(this) != -1) {
			...
			for (int i = 0; i < nbImagesX; i++)
				for (int j = 0; j < nbImagesY; j++)
					g.drawImage(backgroundImage, i * widthImage, j * heightImage + 95, this);
		} else {
	...
	/*
	 * // Without this method, the canvas is clippping!!!!!! public void update(Graphics g){ try{ // Update is called only by selectMessage: //System.out.println("update method called");
	 * 
	 * setBackground(g); drawTop(g); drawArrows(g);
	 * 
	 * 
	...
	public void paint(Graphics g) {
		try {
			 ...
			 * 
			 * // The width and height of the canvas int w = getSize().width; int h = getSize().height; // Create an ellipse and use it as the clipping path Ellipse2D e = new Ellipse2D.Float(w/4.0f,h/4.0f, w/2.0f,h/2.0f); g2.setClip(e);
			 * 
			 * // Fill the canvas. Only the area within the clip is rendered g2.setColor(Color.cyan); g2.fillRect(0,0,w,h);
			 * 
			 * // Change the clipping path, setting it to the intersection of // the current clip and a new rectangle. Rectangle r = new Rectangle(w/4+10,h/4+10,w/2-20,h/2-20); g2.clip(r);
			 * 
			 * // Fill the canvas. Only the area within the new clip // is rendered g2.setColor(Color.magenta); g2.fillRect(0,0,w,h);
			 */
		} catch (Exception exception) {
		}
	}
