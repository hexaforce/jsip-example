...
public class ListenerTracesViewer {
	...
	public void debugActionPerformed(ActionEvent evt) {
		...
		if (debug.beforeDebug != null && debug.beforeDebug != null && !debug.beforeDebug.trim().equals("") && !debug.afterDebug.trim().equals("")) {
			DebugWindow debugWindow = new DebugWindow(debug.beforeDebug, debug.afterDebug, debug.debugLine);
	...
	public void animationActionPerformed(ActionEvent evt) {
		if (tracesViewer.tracesCanvas.arrows.size() == 0) {
			new AlertFrame("Please hit Refresh, first!", JOptionPane.ERROR_MESSAGE);
	...
	/*
	 * public void refreshActionPerformed(ActionEvent evt){ if (ANIMATION_STARTED) new AlertFrame("You must stop the animation before refreshing the traces!",JOptionPane.ERROR_MESSAGE); else { TracesSessions tracesSessions=tracesViewerLauncher.refreshTracesSessions(); tracesViewerLauncher.tracesSessionsList.setTracesSessions(tracesSessions); tracesViewerLauncher.tracesSessionsList.updateTracesCanvas(); tracesViewerLauncher.initComboBox(); if (tracesSessionsDisplayer.isVisible()) tracesSessionsDisplayer.show(tracesSessions); }
	 * 
	 * }
	 */
	...
	public void refreshActionPerformed(MouseEvent evt) {
		if (ANIMATION_STARTED)
			...
			TracesSessions tracesSessions = tracesViewer.refreshTracesSessions();
