...
public abstract class GenericObject implements Serializable, Cloneable {
	 ...
	 *
	 * @param mergeObject is the replacement object. The override obect must be of the same class as this object. Set any fields that you do not want to override as null in the mergeOject object.
	 */
	 ...
	 * 
	 * @param other the match pattern to test against. The match object has to be of the same type (class). Primitive types and non-sip fields that are non null are matched for equality. Null in any field matches anything. Some book-keeping fields are ignored when making the comparison.
	 */
			...
			try {
				...
				if (fieldType.isPrimitive()) {
					...
					if (hisObj != null && myObj == null)
						...
						return false;
					else if (GenericObjectList.isMySubclass(myObj.getClass()) && !((GenericObjectList) myObj).match(hisObj))
						return false;
