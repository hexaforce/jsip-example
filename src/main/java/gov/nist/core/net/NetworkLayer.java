...
public interface NetworkLayer {
	...
	/**
	 * Creates an SSL server with the specified port, listen backlog, and local IP address to bind to. Added by Daniel J. Martinez Manzano <dani@dif.um.es>
	 *
	 * @param port
	 * @param backlog
	 * @param bindAddress
	 ...
	 * @throws IOException if binding or connecting the socket fail for a reason (exception relayed from the correspoonding Socket methods)
	 */
	public Socket createSocket(InetAddress address, int port, InetAddress myAddress, int myPort) throws IOException;

	/**
	 * Creates a stream SSL socket and connects it to the specified port number at the specified IP address. Added by Daniel J. Martinez Manzano <dani@dif.um.es>
	 *
	 * @param address
	 * @param port
	 * @return the socket
