...
//    }
	...
	public Token match(int tok) throws ParseException {
		...
		if (tok > START && tok < END) {
			...
			if (next == ch) {
				/*
				 * this.currentMatch = new Token(); this.currentMatch.tokenValue = String.valueOf(ch); this.currentMatch.tokenType = tok;
				 */
				consume(1);
			} else
	 ...
	 *
	 * @see RFC3261 section 25.1: token = 1*(alphanum / "-" / "." / "!" / "%" / "*" / "_" / "+" / "`" / "'" / "~" )
	 * 
	 ...
	 * 
	 * @return the substring containing the quoted string excluding the closing quote.
	 */
	 ...
	 * 
	 * @return the substring containing the comment excluding the closing brace.
	 */
	 ...
	 * 
	 * @return a substring corresponding to a number (i.e. sequence of digits).
	 */
	...
	public String getRest() {
		if (ptr > bufferLen)
			return null;
		else if (ptr == bufferLen)
			return "";
		else
			return String.valueOf(buffer, ptr, bufferLen - ptr);
	}
