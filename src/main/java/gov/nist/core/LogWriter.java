		...
		if (logLevel != null) {
			if (logLevel.equals("LOG4J")) {
				...
				try {
					...
					/*
					 * If user specifies a logging file as part of the startup properties then we try to create the appender.
					 */
					if (this.needsLogging && this.logFileName != null) {

						boolean overwrite = Boolean.valueOf(configurationProperties.getProperty("gov.nist.javax.sip.DEBUG_LOG_OVERWRITE"));

						FileAppender fa = null;
						try {
							fa = new FileAppender(new SimpleLayout(), this.logFileName, !overwrite);
