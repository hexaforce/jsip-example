...
/**
 * Thread Auditor class: - Provides a mechanism for applications to check the health of internal threads - The mechanism is fairly simple: - Threads register with the auditor at startup and "ping" the auditor every so often. - The application queries the auditor about the health of the system periodically. The auditor reports if the threads are healthy or if any of them failed to ping and are considered dead or stuck. - The main implication for the monitored threads is that they can no longer block waiting for an event forever. Any wait() must be implemented with a timeout so that the thread can periodically ping the auditor.
 *
...
public class ThreadAuditor {
	...
	public class ThreadHandle {
		...
		public String toString() {
			StringBuilder toString = new StringBuilder().append("Thread Name: ").append(thread.getName()).append(", Alive: ").append(thread.isAlive());
	...
	public String auditThreads() {
		...
		while (it.hasNext()) {
			...
			if (!threadHandle.isThreadActive()) {
				...
				/*
				 * Stack traces are not available with JDK 1.4. Feel free to uncomment this block to get a better report if you're using JDK 1.5.
				 */
