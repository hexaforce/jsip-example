...
public class HostNameParser extends ParserCore {
	...
	public Host host() throws ParseException {
		...
		try {
			...
			// IPv6 address (i.e. missing square brackets)
				...
				hostname = new StringBuilder("[").append(lexer.getBuffer().substring(startPtr, lexer.getPtr())).append("]").toString();
			}
	 ...
	 *
	 * @param uriHeader the string (supposedly the value of a URI header) that we have received for parsing.
	 *
	 * @return true if the host part of <tt>uriHeader</tt> could be an IPv6 address (i.e. contains at least two colons) and false otherwise.
	 */
