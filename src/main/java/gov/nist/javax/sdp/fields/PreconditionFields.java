...
public class PreconditionFields implements Serializable {
	...
	public void setPreconditionCurr(String precondCurrValue) throws SdpException {
		if (precondCurrValue == null)
			...
			try {
				/*
				 * split the "curr" attribute value into words attributes[0] = "qos" attributes[1] = status-type (e2e/local/remote) attributes[2] = direction-tag (none/send/recv/sendrecv)
				 *
				 * split() - regular expression: \s -> a whitespace character [ \t\n\x0B\f\r] \b -> a word boundary
				 */
				String[] attributes = precondCurrValue.split(" ");

				// which is this length?! of the string or the []?
				/*
				 * if (attributes.length < 3) { throw new SdpException ("The Precondition \"curr\" attribute value mal-formed (<3words)"); }
				 */
	...
	public void setPreconditionDes(String precondDesValue) throws SdpException {
		if (precondDesValue == null)
			...
			/*
			 * split the "des" attribute value into words attributes[0] = "qos" attributes[1] = strength-tag (unknown/failure/none/optional/mandatory) attributes[2] = status-type (e2e/local/remote) attributes[3] = direction-tag (none/send/recv/sendrecv)
			 *
			 * split() - regular expression: \s -> a whitespace character [ \t\n\x0B\f\r] \b -> a word boundary
			 */
			try {
				...
				// which is this length?! of the string or the []?
				/*
				 * if (attributes.length < 4) { throw new SdpException ("The Precondition \"des\" attribute value mal-formed (<4words)"); }
				 */
	...
	public void setPreconditionConfirmStatus(String precondConfValue) throws SdpException {
		if (precondConfValue == null || precondConfValue.length() == 0)
			...
			/*
			 * split the "conf" attribute value into words attributes[0] = "qos" attributes[1] = status-type (e2e/local/remote) attributes[2] = direction-tag (none/send/recv/sendrecv)
			 */
			try {
				String[] attributes = precondConfValue.split(" ");

				setPreconditionConfirmStatus(attributes[1], // status-type
						attributes[2] // direction-tag
				);
			} catch (ArrayIndexOutOfBoundsException ex) {
	 ...
	 *
	 * @param status - (local, remote, e2e)
	 * @return a vector with the attribute field that match status-type (with only one element or null if none exists)
	 * @throws SdpParseException
	 */
	...
	public static final String[] STRENGTH = { "unknown", "failure", "none", "optional", "mandatory" };
	/* direction-tag */
