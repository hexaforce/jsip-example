	...
	public String debugDump() {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			try {
				...
				} else if (Class.forName(CORE_PACKAGE + ".GenericObject").isAssignableFrom(fieldType)) {
					if (f.get(this) != null) {
				...
				} else if (Class.forName(CORE_PACKAGE + ".GenericObjectList").isAssignableFrom(fieldType)) {
					if (f.get(this) != null) {
