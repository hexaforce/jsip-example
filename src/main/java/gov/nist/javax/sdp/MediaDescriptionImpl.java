...
public class MediaDescriptionImpl implements javax.sdp.MediaDescription {
	...
	public String encode() {
		...
		if (bandwidthFields != null) {
			for (int i = 0; i < bandwidthFields.size(); i++) {
				...
				retval.append(((SDPField) bandwidthFields.elementAt(i)).encode());

				/*
				 * original code BandwidthField bandwidthField = (BandwidthField) bandwidthFields.elementAt(i); retval.append(bandwidthField.encode());
				 */
				// end //
			}
			if (preconditionFields != null) {
				...
				for (int i = 0; i < precondSize; i++) {
					retval.append(((SDPField) preconditionFields.getPreconditions().elementAt(i)).encode());
				}
			}
		}
		...
		if (attributeFields != null) {
			for (int i = 0; i < attributeFields.size(); i++)
				retval.append(((SDPField) attributeFields.elementAt(i)).encode());
		}
	...
	public int getBandwidth(String name) throws SdpParseException {
		...
		if (bandwidthFields == null)
			...
			for (int i = 0; i < bandwidthFields.size(); i++) {
				BandwidthField bandwidthField = (BandwidthField) bandwidthFields.elementAt(i);
	 ...
	 *
	 * @param payloadNames  a Vector of String - each one the name of a dynamic payload to be added (usually an integer larger than SdpConstants.AVP_DYNAMIC_MIN).
	 * @param payloadValues a Vector of String - each contains the value describing the correlated dynamic payloads to be added
	 * @throws SdpException if either vector is null or empty. if the vector sizes are unequal.
	 */
