...
public interface DialogExt extends Dialog {
	...
	/**
	 * If set to true it will release all references that it no longer needs. This will include the reference to the Request, Transactions, Any unused timers etc. This will significantly reduce memory consumption under high load
	 * 
	...
	/**
	 * True if this dialog has been created following the forking of a previous dialog. This could happen by example if Alice would Invite a forking proxy whom would 180Ring Bob but finally 200Ok on Carol. The original Dialog known by Alice after Bob's ringing would be overriden by a new dialog bound to Carol. The original dialog can be retrieved using {@link #getOriginalDialog()}
	 */
