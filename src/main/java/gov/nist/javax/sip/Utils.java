...
public class Utils implements UtilsExt {
	 ...
	 * 
	 * @param str the String to convert. Using english as default as header strings are in English. Resolves a bug in parsing and uppercase when default locale is set to Turkish.
	 ...
	 *
	 *         synchronized: needed for access to 'rand', else risk to generate same tag twice
	 */
	 ...
	 *
	 * @return a cryptographically random gloablly unique string that can be used as a branch identifier.
	 */
	public String generateBranchId() {
		...
		synchronized (digester) {
			...
			return SIPConstants.BRANCH_MAGIC_COOKIE + "-" + this.signature + "-" + Utils.toHexString(bid);
		}
	}
	...
	public boolean responseBelongsToUs(SIPResponse response) {
		...
		return branch != null && branch.startsWith(SIPConstants.BRANCH_MAGIC_COOKIE + "-" + this.signature);
	}
