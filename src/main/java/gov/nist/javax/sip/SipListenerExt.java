 ...
 * <ul>
 * <li>{@link DialogTimeoutEvent}- these are timeout notifications emitted as events by the SipProvider. Timeout events represent timers expiring in the underlying SipProvider dialog state machine. These timeout's events notify the application that a dialog has timed out.</li>
 * </ul>
 * 
...
public interface SipListenerExt extends SipListener {
	 ...
	 * 
	 * Applications implementing this method should take care of sending the BYE or terminating the dialog to avoid any dialog leaks.
	 * 
	...
	/**
	 * Processes a keepalive Timeout of an underlying TCP based {@link MessageChannel} handled by this SipListener. This Event notifies the application that a keepalive double crlf was not received on the channel after the expîred amount of time defined by stack property gov.nist.javax.sip.RELIABLE_CONNECTION_KEEP_ALIVE_TIMEOUT. The KeepAliveTimeoutEvent encapsulates the local and peer hosts, ports and transport.
	 * 
