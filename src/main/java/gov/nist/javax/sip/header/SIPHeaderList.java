...
public abstract class SIPHeaderList<HDR extends SIPHeader> extends SIPHeader implements java.util.List<HDR>, Header {
	 ...
	 * @param topFlag flag which indicates which end to concatenate the lists.
	 ...
	 *
	 * @return String encoded string representation of this list of headers. (Contains string append of each encoded header).
	 */
	 ...
	 *
	 * @return LinkedList containing encoded strings in this header list. an empty list is returned if this header list contains no sip headers.
	 */
	 ...
	 *
	 * @param other the other object to compare with. true is returned iff the classes match and list of headers herein is equal to the list of headers in the target (order of the headers is not important).
	 */
	...
	public boolean match(SIPHeaderList<?> template) {
		...
		if (this.hlist == that.hlist)
			...
			for (Iterator<SIPHeader> it = that.hlist.iterator(); it.hasNext();) {
				...
				boolean found = false;
				for (Iterator<HDR> it1 = this.hlist.iterator(); it1.hasNext() && !found;) {
