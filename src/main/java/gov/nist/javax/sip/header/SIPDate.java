...
public class SIPDate implements Cloneable, Serializable {
	...
	public StringBuilder encode(StringBuilder encoding) {
		...
		return encoding.append(year).append(Separators.SP).append(hourString).append(Separators.COLON).append(minuteString).append(Separators.COLON).append(secondString).append(Separators.SP).append(GMT);
	}
...
/*
 * $Log: not supported by cvs2svn $ Revision 1.9 2009/10/18 13:46:33 deruelle_jean FindBugs Fixes (Category Performance Warnings)
 *
 * Issue number: Obtained from: Submitted by: Jean Deruelle Reviewed by:
 *
 * Revision 1.8 2009/07/17 18:57:37 emcho Converts indentation tabs to spaces so that we have a uniform indentation policy in the whole project.
 *
 * Revision 1.7 2006/07/13 09:01:16 mranga Issue number: Obtained from: Submitted by: jeroen van bemmel Reviewed by: mranga Moved some changes from jain-sip-1.2 to java.net
 *
 ...
 *
 *
 * Revision 1.5 2005/04/16 20:35:10 dmuresan SIPDate made cloneable.
 *
 * Revision 1.4 2004/07/28 14:41:53 mranga Submitted by: mranga
 *
 * fixed equality check for SIPDate.
 *
 * Revision 1.3 2004/04/05 21:46:08 mranga Submitted by: Bruno Konik Reviewed by: mranga
 *
