 ...
 * @version 1.2 $Revision: 1.9 $ $Date: 2010-05-06 14:07:55 $
 *
 *
 *
...
public class Warning extends SIPHeader implements WarningHeader {
	...
	public StringBuilder encodeBody(StringBuilder buffer) {
		return text != null ? buffer.append(Integer.toString(code)).append(SP).append(agent).append(SP).append(DOUBLE_QUOTE).append(text).append(DOUBLE_QUOTE) : buffer.append(Integer.toString(code)).append(SP).append(agent);
	}
...
/*
 * $Log: not supported by cvs2svn $ Revision 1.8 2009/10/18 13:46:33 deruelle_jean FindBugs Fixes (Category Performance Warnings)
 *
 * Issue number: Obtained from: Submitted by: Jean Deruelle Reviewed by:
 *
 * Revision 1.7 2009/07/17 18:57:41 emcho Converts indentation tabs to spaces so that we have a uniform indentation policy in the whole project.
 *
 * Revision 1.6 2006/07/13 09:01:44 mranga Issue number: Obtained from: Submitted by: jeroen van bemmel Reviewed by: mranga Moved some changes from jain-sip-1.2 to java.net
 *
 ...
 *
 *
 * Revision 1.4 2004/04/22 22:51:16 mranga Submitted by: Thomas Froment Reviewed by: mranga
 *
 * Fixed corner cases.
 *
