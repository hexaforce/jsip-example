...
public class MinExpires extends SIPHeader implements MinExpiresHeader {
	...
	/**
	 * Gets the expires value of the ExpiresHeader as type int. This expires value is relative time. Values larger than Integer.MAX_VALUE will be returned as Integer.MAX_VALUE.
	 * 
	 * @return the expires value of the ExpiresHeader.
	 */
	...
	/**
	 * Sets the relative expires value of the ExpiresHeader. The expires value MUST be greater or equal than zero and MUST be less than 2**32.
	 *
	...
	/**
	 * Sets the relative expires value of the ExpiresHeader. The expires value MUST be greater or equal than zero and MUST be less than 2**31.
	 *
	 * @param expires - the new expires value of this ExpiresHeader
	 *
	 * @throws InvalidArgumentException if supplied value is less than zero.
...
/*
 * $Log: not supported by cvs2svn $ Revision 1.6 2009/10/18 13:46:35 deruelle_jean FindBugs Fixes (Category Performance Warnings)
 *
 * Issue number: Obtained from: Submitted by: Jean Deruelle Reviewed by:
 *
 * Revision 1.5 2009/07/17 18:57:32 emcho Converts indentation tabs to spaces so that we have a uniform indentation policy in the whole project.
 *
 * Revision 1.4 2006/07/13 09:01:20 mranga Issue number: Obtained from: Submitted by: jeroen van bemmel Reviewed by: mranga Moved some changes from jain-sip-1.2 to java.net
 *
