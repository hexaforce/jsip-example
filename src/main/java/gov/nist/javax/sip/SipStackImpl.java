 ...
 * <b> Use of this property is still supported but deprecated. Please use gov.nist.javax.sip.STACK_LOGGER and gov.nist.javax.sip.SERVER_LOGGER for integration with logging frameworks and for custom formatting of log records. </b> Log valid incoming messages here. If this is left null AND the TRACE_LEVEL is above INFO (or TRACE) then the messages are printed to stdout. Otherwise messages are logged in a format that can later be viewed using the trace viewer application which is located in the tools/tracesviewer directory. <font color=red> Mail this to us with bug reports. </font></li>
 * 
 * <li><b>gov.nist.javax.sip.DEBUG_LOG = fileName </b> <b> Use of this property is still supported but deprecated. Please use gov.nist.javax.sip.STACK_LOGGER and gov.nist.javax.sip.SERVER_LOGGER for integration with logging frameworks and for custom formatting of log records. </b> <br/>
 * Where the debug log goes. <font color=red> Mail this to us with bug reports. </font></li>
 * 
 * <li><b>gov.nist.javax.sip.LOG_MESSAGE_CONTENT = true|false </b><br/>
 * Set true if you want to capture content into the log. Default is false. A bad idea to log content if you are using SIP to push a lot of bytes through TCP.</li>
 * 
 * <li><b>gov.nist.javax.sip.LOG_STACK_TRACE_ON_MESSAGE_SEND = true|false </b><br/>
 * Set true if you want to to log a stack trace at INFO level for each message send. This is really handy for debugging.</li>
 * 
 * <li><b>gov.nist.javax.sip.STACK_LOGGER = full path name to the class implementing gov.nist.core.StackLogger interface</b><br/>
 * If this property is defined the sip stack will try to instantiate it through a no arg constructor. This allows to use different logging implementations than the ones provided by default to log what happens within the stack while processing SIP messages. If this property is not defined, the default sip stack LogWriter will be used for logging</li>
 * 
 * <li><b>gov.nist.javax.sip.SERVER_LOGGER = full path name to the class implementing gov.nist.core.ServerLogger interface</b><br/>
 * If this property is defined the sip stack will try to instantiate it through a no arg constructor. This allows to use different logging implementations than the ones provided by default to log sent/received messages by the sip stack. If this property is not defined, the default sip stack ServerLog will be used for logging</li>
 * 
 * <li><b>gov.nist.javax.sip.AUTOMATIC_DIALOG_ERROR_HANDLING = [true|false] </b> <br/>
 ...
 * <ul>
 * <li>
 * <li>If this is not specified, <b> and the listener is re-entrant</b>, each event delivered to the listener is run in the context of a new thread.</li>
 * <li>If this is specified and the listener is re-entrant, then the stack will run the listener using a thread from the thread pool. This allows you to manage the level of concurrency to a fixed maximum. Threads are pre-allocated when the stack is instantiated.</li>
 * <li>If this is specified and the listener is not re-entrant, then the stack will use the thread pool thread from this pool to parse and manage the state machine but will run the listener in its own thread.</li>
 * </ul>
 * 
 * <li><b>gov.nist.javax.sip.REENTRANT_LISTENER = true|false </b> <br/>
 * Default is false. Set to true if the listener is re-entrant. If the listener is re-entrant then the stack manages a thread pool and synchronously calls the listener from the same thread which read the message. Multiple transactions may concurrently receive messages and this will result in multiple threads being active in the listener at the same time. The listener has to be written with this in mind. <b> If you want good performance on a multithreaded machine write your listener to be re-entrant and set this property to be true </b></li>
 * 
 ...
 * Maximum size of server transaction table. The low water mark is 80% of the high water mark. Requests are selectively dropped in the lowater mark to highwater mark range. Requests are unconditionally accepted if the table is smaller than the low water mark. The default highwater mark is 5000</li>
 * 
 * <li><b>gov.nist.javax.sip.MAX_CLIENT_TRANSACTIONS = integer </b> <br/>
 * Max number of active client transactions before the caller blocks and waits for the number to drop below a threshold. Default is unlimited, i.e. the caller never blocks and waits for a client transaction to become available (i.e. it does its own resource management in the application).</li>
 * 
 * <li><b>gov.nist.javax.sip.PASS_INVITE_NON_2XX_ACK_TO_LISTENER = true|false </b> <br/>
 * If true then the listener will see the ACK for non-2xx responses for server transactions. This is not standard behavior per RFC 3261 (INVITE server transaction state machine) but this is a useful flag for testing. The TCK uses this flag for example.</li>
 * 
 * <li><b>gov.nist.javax.sip.MAX_LISTENER_RESPONSE_TIME = Integer </b> <br/>
 * Max time (seconds) to wait on the transaction lock used to serialize message delivery. Default time is "infinity" - i.e. if the listener never responds, the stack will hang on to a reference for the transaction and result in a unusable thread stuck waiting for the lock to be released. A good value for this property is the lifespan of the transaction or the expected blocking delay in the listener.
 * 
 ...
 * The fully qualified class path for an implementation of the AddressResolver interface. The AddressResolver allows you to support lookup schemes for addresses that are not directly resolvable to IP adresses using getHostByName. Specifying your own address resolver allows you to customize address lookup. The default address resolver is a pass-through address resolver (i.e. just returns the input string without doing a resolution). See gov.nist.javax.sip.DefaultAddressResolver.</li>
 * 
 * <li><b>gov.nist.javax.sip.AUTO_GENERATE_TIMESTAMP= [true| false] </b><br/>
 * (default is false) Automatically generate a getTimeOfDay timestamp for a retransmitted request if the original request contained a timestamp. This is useful for profiling.</li>
 * 
 * <li><b>gov.nist.javax.sip.THREAD_AUDIT_INTERVAL_IN_MILLISECS = long </b> <br/>
 * Defines how often the application intends to audit the SIP Stack about the health of its internal threads (the property specifies the time in miliseconds between successive audits). The audit allows the application to detect catastrophic failures like an internal thread terminating because of an exception or getting stuck in a deadlock condition. Events like these will make the stack inoperable and therefore require immediate action from the application layer (e.g., alarms, traps, reboot, failover, etc.) Thread audits are disabled by default. If this property is not specified, audits will remain disabled. An example of how to use this property is in src/examples/threadaudit.</li>
 * 
 * <li><b>gov.nist.javax.sip.NIO_MAX_SOCKET_IDLE_TIME = long </b> <br/>
 * Defines the number of milliseconds a NIO TCP socket will be kept alive after the last IO operation on that socket. This allows to clean up after high initial load of new calls that hang up or stay idle. Note that disconnecting the socket does't end the SIP call. A new socket will be established when needed for any existing calls by the SIP RFC spec.</li>
 * 
 * <li><b>gov.nist.javax.sip.stack.USE_DIRECT_BUFFERS = [true|false]</b> <br/>
 * Default is <it>true</it> If set to <it>false</it>, the NIO stack won't use direct buffers. As Direct buffers reside outside of the heap memory, they can lead to unforeseen out of memory exceptions as seen in http://java.net/jira/browse/JSIP-430. This flag allows to use non direct buffers for better memory monitoring and management.</li>
 * 
 ...
 * 
 * <li><b>gov.nist.javax.sip.CONGESTION_CONTROL_TIMEOUT = int </b> How much time messages are allowed to wait in queue before being dropped due to stack being too slow to respond. Default value is 8000 ms. The value is in milliseconds</li>
 * 
 * <li><b>gov.nist.javax.sip.TCP_POST_PARSING_THREAD_POOL_SIZE = integer </b> Use 0 or do not set this option to disable it.
 * 
 * When using TCP your phones/clients usually connect independently creating their own TCP sockets. Sometimes however SIP devices are allowed to tunnel multiple calls over a single socket. This can also be simulated with SIPP by running "sipp -t t1".
 * 
 * In the stack each TCP socket has it's own thread. When all calls are using the same socket they all use a single thread, which leads to severe performance penalty, especially on multi-core machines.
 * 
 * This option instructs the SIP stack to use a thread pool and split the CPU load between many threads. The number of the threads is specified in this parameter.
 * 
 * The processing is split immediately after the parsing of the message. It cannot be split before the parsing because in TCP the SIP message size is in the Content-Length header of the message and the access to the TCP network stream has to be synchronized. Additionally in TCP the message size can be larger. This causes most of the parsing for all calls to occur in a single thread, which may have impact on the performance in trivial applications using a single socket for all calls. In most applications it doesn't have performance impact.
 * 
 * If the phones/clients use separate TCP sockets for each call this option doesn't have much impact, except the slightly increased memory footprint caused by the thread pool. It is recommended to disable this option in this case by setting it 0 or not setting it at all. You can simulate multi-socket mode with "sipp -t t0".
 * 
 * With this option also we avoid closing the TCP socket when something fails, because we must keep processing other messages for other calls.
 * 
 * Note: This option relies on accurate Content-Length headers in the SIP messages. It cannot recover once a malformed message is processed, because the stream iterator will not be aligned any more. Eventually the connection will be closed.</li>
 * 
 * <li><b>gov.nist.javax.sip.DELIVER_UNSOLICITED_NOTIFY = [true|false] </b> <br/>
 * Default is <it>false</it>. This flag is added to allow Sip Listeners to receive all NOTIFY requests including those that are not part of a valid dialog.</li>
 * 
 * <li><b>gov.nist.javax.sip.REJECT_STRAY_RESPONSES = [true|false] </b> Default is <it>false</it> A flag that checks responses to test whether the response corresponds to a via header that was previously generated by us. Note that setting this flag implies that the stack will take control over setting the VIA header for Sip Requests sent through the stack. The stack will attach a suffix to the VIA header branch and check any response arriving at the stack to see if that response suffix is present. If it is not present, then the stack will silently drop the response.</li>
 * 
 * <li><b>gov.nist.javax.sip.MAX_FORK_TIME_SECONDS = integer </b> Maximum time for which the original transaction for which a forked response is received is tracked. This property is only relevant to Dialog Stateful applications ( User Agents or B2BUA). When a forked response is received in this time interval from when the original INVITE client transaction was sent, the stack will place the original INVITE client transction in the ResponseEventExt and deliver that to the application. The event handler can get the original transaction from this event.</li>
 * 
 * <li><b>gov.nist.javax.sip.EARLY_DIALOG_TIMEOUT_SECONDS=integer </b> Maximum time for which a dialog can remain in early state. This is defaulted to 3 minutes ( 180 seconds).</li>
 * 
 * <li><b>gov.nist.javax.sip.THREAD_PRIORITY=integer </b> Control the priority of the threads started by the stack.</li>
 * 
 * <li><b>gov.nist.javax.sip.MESSAGE_PARSER_FACTORY = name of the class implementing gov.nist.javax.sip.parser.MessageParserFactory</b> This factory allows pluggable implementations of the MessageParser that will take care of parsing the incoming messages. By example one could plug a lazy parser through this factory.</li>
 * 
 * <li><b>gov.nist.javax.sip.MESSAGE_PROCESSOR_FACTORY = name of the class implementing gov.nist.javax.sip.parser.MessageProcessorFactory</b> This factory allows pluggable implementations of the MessageProcessor that will take care of incoming messages. By example one could plug a NIO Processor through this factory.</li>
 * 
 * <li><b>gov.nist.javax.sip.TIMER_CLASS_NAME = name of the class implementing gov.nist.javax.sip.stack.timers.SipTimer</b> interface This allows pluggable implementations of the Timer that will take care of scheduling the various SIP Timers. By example one could plug a regular timer, a scheduled thread pool executor.</li>
 * 
 * <li><b>gov.nist.javax.sip.DELIVER_RETRANSMITTED_ACK_TO_LISTENER=boolean</b> A testing property that allows application to see the ACK for retransmitted 200 OK requests. <b>Note that this is for test purposes only</b></li>
 * 
 * *
 * <li><b>gov.nist.javax.sip.AGGRESSIVE_CLEANUP=boolean</b> A property that will cleanup Dialog, and Transaction structures agrressively to improve memroy usage and performance (up to 50% gain). However one needs to be careful in its code on how and when it accesses transaction and dialog data since it cleans up aggressively when transactions changes state to COMPLETED or TERMINATED and for Dialog once the ACK is received/sent</li>
 * 
 * <li><b>gov.nist.javax.sip.MIN_KEEPALIVE_TIME_SECONDS = integer</b> Minimum time between keep alive pings (CRLF CRLF) from clients. If pings arrive with less than this frequency they will be replied with CRLF CRLF if greater they will be rejected. The default is -1 (i.e. do not respond to CRLF CRLF).</li>
 * 
 * <li><b>gov.nist.javax.sip.DIALOG_TIMEOUT_FACTOR= integer</b> Default to 64. The number of ticks before a dialog that does not receive an ACK receives a Timeout notification. Note that this is only relevant if the registered SipListener is of type SipListenerExt</li>
 * 
 * <li><b>gov.nist.javax.sip.SIP_MESSAGE_VALVE= String</b> Default to null. The class name of your custom valve component. An instance of this class will be created and the SIPMessageValve.processRequest/Response() methods will be called for every message before any long-lived SIP Stack resources are allocated (no transactions, no dialogs). From within the processRequest callback implementation you can drop messages, send a response statelessly or otherwise transform/pre-process the message before it reaches the next steps of the pipeline. Similarly from processResponse() you can manipulate a response or drop it silently, but dropping responses is not recommended, because the transaction already exists when the request for the response was sent.</li>
 * 
 * <li><b>gov.nist.javax.sip.SIP_EVENT_INTERCEPTOR</b> Default to null. The class name of your custom interceptor object. An instance of this object will be created at initialization of the stack. You must implement the interface gov.nist.javax.sip.stack.SIPEventInterceptor and handle the lifecycle callbacks. This interface is the solution for https://jain-sip.dev.java.net/issues/show_bug.cgi?id=337 . It allows to wrap the JSIP pipeline and execute custom analysis logic as SIP messages advance through the pipeline checkpoints. One example implementation of this interceptor is <b>gov.nist.javax.sip.stack.CallAnalysisInterceptor</b>, which will periodically check for requests stuck in the JAIN SIP threads and if some request is taking too long it will log the stack traces for all threads. The logging can occur only on certain events, so it will not overwhelm the CPU. The overall performance penalty by using this class in production under load is only 2% peak on average laptop machine. There is minimal locking inside. One known limitation of this feature is that you must use gov.nist.javax.sip.REENTRANT_LISTENER=true to ensure that the request will be processed in the original thread completely for UDP.</li>
 * 
 * <li><b>gov.nist.javax.sip.TLS_CLIENT_PROTOCOLS = String </b> Comma-separated list of protocols to use when creating outgoing TLS connections. The default is "TLSv1.2, TLSv1.1, TLSv1". It's advisable not to use SSL protocols because of http://googleonlinesecurity.blogspot.fr/2014/10/this-poodle-bites-exploiting-ssl-30.html</li>
 * 
 * <li><b>gov.nist.javax.sip.TLS_SECURITY_POLICY = String </b> The fully qualified path name of a TLS Security Policy implementation that is consulted for certificate verification of outbund TLS connections.</li>
 * 
 * <li><b>gov.nist.javax.sip.TLS_CLIENT_AUTH_TYPE = String </b> Valid values are Default (backward compatible with previous versions) , Enabled, Want, Disabled or DisabledAll. Set to Enabled if you want the SSL stack to require a valid certificate chain from the client before accepting a connection. Set to Want if you want the SSL stack to request a client Certificate, but not fail if one isn't presented. A Disabled value will not require a certificate chain for the Server Connection. A DisabledAll will not require a certificate chain for both Server and Client Connections.</li>
 *
 * <li><b>gov.nist.javax.sip.RELIABLE_CONNECTION_KEEP_ALIVE_TIMEOUT</b> Value in seconds which is used as default keepalive timeout (See also http://tools.ietf.org/html/rfc5626#section-4.4.1). Defaults to "infinity" seconds (i.e. timeout event not delivered).</li>
 *
 * <li><b>gov.nist.javax.sip.TCP_NODELAY = [true|false]</b> Whether or not to disable Nagle's algorithm for TCP sockets. Defaults to {@code false}.</li>
 * 
 * <li><b>gov.nist.javax.sip.SSL_HANDSHAKE_TIMEOUT</b> Value in seconds which is used as default timeout for performing the SSL Handshake This prevents bad clients of connecting without sending any data to block the server</li>
 * 
 * <li><b>gov.nist.javax.sip.SSL_RENEGOTIATION_ENABLED = [true|false]</b> Default value is <b>true</b>. Allow or disallow SSL renegotiation to resolve potential DoS problem - <a href="http://web.nvd.nist.gov/view/vuln/detail?vulnId=CVE-2011-1473">reference</a> and <a href="http://www.ietf.org/mail-archive/web/tls/current/msg07553.html">another reference</a>. The safe option is to disable it.</li>
 *
 *
 ...
 * </code> <br>
 * If not trustStorePassword is provided then the keyStorePassword will be used for both. The trust store can be changed, to a separate file with the following setting: <br>
 * <code>
 * properties.setProperty("javax.net.ssl.trustStore", "&lt;trustStoreFileName location&gt;");
 * </code> <br>
 * If the trust store property is provided the password on the trust store must be the same as the key store. <br>
 * <br>
 * </li>
 * </ul>
 * <b> Note that the stack supports the extensions that are defined in SipStackExt. These will be supported in the next release of JAIN-SIP. You should only use the extensions that are defined in this class. </b>
 *
 *
...
public class SipStackImpl extends SIPTransactionStack implements javax.sip.SipStack, SipStackExt {
	...
	// compat
	private String[] cipherSuites = { "TLS_RSA_WITH_AES_128_CBC_SHA", // AES difficult to get with
																		// c++/Windows
	...
	public SipStackImpl(Properties configurationProperties) throws PeerUnavailableException {
		...
		super.setReliableConnectionKeepAliveTimeout(1000 * Integer.parseInt(configurationProperties.getProperty("gov.nist.javax.sip.RELIABLE_CONNECTION_KEEP_ALIVE_TIMEOUT", "-1")));
		...
		super.setSslHandshakeTimeout(Long.parseLong(configurationProperties.getProperty("gov.nist.javax.sip.SSL_HANDSHAKE_TIMEOUT", "-1")));

		super.setThreadPriority(Integer.parseInt(configurationProperties.getProperty("gov.nist.javax.sip.THREAD_PRIORITY", "" + Thread.MAX_PRIORITY)));
		...
		ByteBufferFactory.getInstance().setUseDirect(Boolean.valueOf(configurationProperties.getProperty("gov.nist.javax.sip.stack.USE_DIRECT_BUFFERS", Boolean.TRUE.toString())));
		...
		if (extensionMethods != null) {
			...
			while (st.hasMoreTokens()) {
				...
				if (em.equalsIgnoreCase(Request.BYE) || em.equalsIgnoreCase(Request.INVITE) || em.equalsIgnoreCase(Request.SUBSCRIBE) || em.equalsIgnoreCase(Request.NOTIFY) || em.equalsIgnoreCase(Request.ACK) || em.equalsIgnoreCase(Request.OPTIONS))
		...
		super.isServerLoopDetectionEnabled = configurationProperties.getProperty("gov.nist.javax.sip.SERVER_LOOP_DETECTION", "on").equalsIgnoreCase("on");
		...
		if (configurationProperties.getProperty("gov.nist.javax.sip.MAX_TX_LIFETIME_INVITE") != null) {
			super.maxTxLifetimeInvite = Integer.parseInt(configurationProperties.getProperty("gov.nist.javax.sip.MAX_TX_LIFETIME_INVITE"));
			if (super.getMaxTxLifetimeInvite() <= 0)
				throw new PeerUnavailableException("Bad configuration parameter gov.nist.javax.sip.MAX_TX_LIFETIME_INVITE : should be positive");
		} else {
			super.maxTxLifetimeInvite = -1;
		}

		// http://java.net/jira/browse/JSIP-420
		if (configurationProperties.getProperty("gov.nist.javax.sip.MAX_TX_LIFETIME_NON_INVITE") != null) {
			super.maxTxLifetimeNonInvite = Integer.parseInt(configurationProperties.getProperty("gov.nist.javax.sip.MAX_TX_LIFETIME_NON_INVITE"));
			if (super.getMaxTxLifetimeNonInvite() <= 0)
				throw new PeerUnavailableException("Bad configuration parameter gov.nist.javax.sip.MAX_TX_LIFETIME_NON_INVITE : should be positive");
		} else {
			super.maxTxLifetimeNonInvite = -1;
		}

		this.setDeliverTerminatedEventForAck(configurationProperties.getProperty("gov.nist.javax.sip.DELIVER_TERMINATED_EVENT_FOR_ACK", "false").equalsIgnoreCase("true"));

		super.setDeliverUnsolicitedNotify(Boolean.parseBoolean(configurationProperties.getProperty("gov.nist.javax.sip.DELIVER_UNSOLICITED_NOTIFY", "false")));
		...
		if (threadPoolSize != null) {
			try {
				...
				if (logger.isLoggingEnabled())
					this.logger.logError("thread pool size - bad value " + ex.getMessage());
			}
		}

		int congetstionControlTimeout = Integer.parseInt(configurationProperties.getProperty("gov.nist.javax.sip.CONGESTION_CONTROL_TIMEOUT", "8000"));
		super.stackCongenstionControlTimeout = congetstionControlTimeout;

		String tcpTreadPoolSize = configurationProperties.getProperty("gov.nist.javax.sip.TCP_POST_PARSING_THREAD_POOL_SIZE");
		if (tcpTreadPoolSize != null) {
			try {
				...
				if (logger.isLoggingEnabled())
					this.logger.logError("TCP post-parse thread pool size - bad value " + tcpTreadPoolSize + " : " + ex.getMessage());
			}
		}

		String isTcpNoDelayEnabled = configurationProperties.getProperty("gov.nist.javax.sip.TCP_NODELAY");
		...
		if (serverTransactionTableSize != null) {
			try {
				this.serverTransactionTableHighwaterMark = Integer.valueOf(serverTransactionTableSize).intValue();
				this.serverTransactionTableLowaterMark = this.serverTransactionTableHighwaterMark * 80 / 100;
				// Lowater is 80% of highwater
			} catch (NumberFormatException ex) {
				if (logger.isLoggingEnabled())
					this.logger.logError("transaction table size - bad value " + ex.getMessage());
			}
		} else {
		...
		if (clientTransactionTableSize != null) {
			try {
				this.clientTransactionTableHiwaterMark = Integer.valueOf(clientTransactionTableSize).intValue();
		...
		/*
		 * gets the SecurityManagerProvider implementation, if any. Note that this is a NIST only feature.
		 */
		...
		if (configurationProperties.containsKey(SECURITY_MANAGER_PROVIDER_KEY)) {
			String path = configurationProperties.getProperty(SECURITY_MANAGER_PROVIDER_KEY);
			try {
				Class<?> clazz = Class.forName(path);
				Constructor<?> c = clazz.getConstructor(new Class[0]);
		...
		if (stunAddr != null)
			this.logger.logWarning("Ignoring obsolete property " + "gov.nist.javax.sip.STUN_SERVER");
		...
		super.earlyDialogTimeout = Integer.parseInt(configurationProperties.getProperty("gov.nist.javax.sip.EARLY_DIALOG_TIMEOUT_SECONDS", "180"));
		...
		super.deliverRetransmittedAckToListener = Boolean.parseBoolean(configurationProperties.getProperty("gov.nist.javax.sip.DELIVER_RETRANSMITTED_ACK_TO_LISTENER", "false"));
		...
		try {
			...
			logger.logError("Bad configuration value for gov.nist.javax.sip.MESSAGE_PARSER_FACTORY", e);
		}
		...
		try {
			...
			logger.logError("Bad configuration value for gov.nist.javax.sip.MESSAGE_PROCESSOR_FACTORY", e);
		}
		...
		try {
			...
			logger.logError("Bad configuration value for gov.nist.javax.sip.NIO_MAX_SOCKET_IDLE_TIME=" + maxIdleTimeString, e);
		}
		...
		super.aggressiveCleanup = Boolean.parseBoolean(configurationProperties.getProperty("gov.nist.javax.sip.AGGRESSIVE_CLEANUP", Boolean.FALSE.toString()));
		...
		if (valveClassName != null && !valveClassName.equals("")) {
			try {
				...
				logger.logError("Bad configuration value for gov.nist.javax.sip.SIP_MESSAGE_VALVE", e);
			}
		}
		...
		if (interceptorClassName != null && !interceptorClassName.equals("")) {
			try {
				...
				logger.logError("Bad configuration value for gov.nist.javax.sip.SIP_EVENT_INTERCEPTOR", e);
			}
		}

		boolean sslRenegotiationEnabled = Boolean.parseBoolean(configurationProperties.getProperty("gov.nist.javax.sip.SSL_RENEGOTIATION_ENABLED", "true"));
	...
	public SipProvider createSipProvider(ListeningPoint listeningPoint) throws ObjectInUseException {
		...
		if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG))
			this.logger.logDebug("createSipProvider: " + listeningPoint);
	...
	public void stop() {
		...
		this.listeningPoints = new Hashtable<String, ListeningPointImpl>();
		/*
		 * Check for presence of an event scanner ( may happen if stack is stopped before listener is attached ).
		 */
	...
	/**
	 * Set the list of cipher suites supported by the stack. A stack can have only one set of suites. These are not validated against the supported cipher suites of the java runtime, so specifying a cipher here does not guarantee that it will work.<br>
	...
	/**
	 * Set the list of protocols supported by the stack for outgoing TLS connections. A stack can have only one set of protocols. These are not validated against the supported protocols of the java runtime, so specifying a protocol here does not guarantee that it will work.<br>
	 ...
	 * 
	 * @param String [] The new set of protocols to use for outgoing TLS connections.
	 * @return
	 * 
	 */
