...
public class SIPRequest extends SIPMessage implements javax.sip.message.Request, RequestExt {
	...
	/*
	 * A table that maps a name string to its cannonical constant. This is used to speed up parsing of messages .equals reduces to == if we use the constant value.
	 * 
	...
	public void checkHeaders() throws ParseException {
		...
		if (SIPTransactionStack.isDialogCreated(method)) {
			...
			/*
			 * Vladimir Ralev : Removing this check to handle proxy with JSIP. For proxy the contact is not relevent, so the UAC behavior RFC 8.1.1.8 doesn't apply, we remove this check so double-record-routing proxies will work .... CSeq: 1 INVITE From: <sip:proxy-tls@sip-servlets.com>;tag=5326009 To: <sip:receiver@sip-servlets.com> Via: SIP/2.0/TLS 127.0.0.1:5071;branch=z9hG4bKd72cf250-04fc-49a8-bde1-2b9728ab0dc1_8a474751_1317304087784710000 Via: SIP/2.0/TCP 127.0.0.1:5080;branch=z9hG4bK-343139-25f9b919076ced8b4c20506d35036178;rport=60554 Max-Forwards: 69 REM: RRRREM Contact: <sip:proxy-tls@127.0.0.1:5080> Record-Route: <sip:127.0.0.1:5071;transport=tls;appname=8a474751;proxy=true;app_id=d72cf250-04fc-49a8-bde1-2b9728ab0dc1;lr> Record-Route: <sip:127.0.0.1:5070;transport=tcp;appname=8a474751;proxy=true;app_id=d72cf250-04fc-49a8-bde1-2b9728ab0dc1;lr> Content-Length: 0
			 * 
			 * Caused by: java.text.ParseException: Scheme for contact should be sips:sip:proxy-tls@127.0.0.1:5080 at gov.nist.javax.sip.message.SIPRequest.checkHeaders(SIPRequest.java:321) at gov.nist.javax.sip.SipProviderImpl.getNewClientTransaction(SipProviderImpl.java:302) ... 23 more
			 * 
			 * if (requestLine.getUri() instanceof SipUri) { String scheme = ((SipUri) requestLine.getUri()).getScheme(); if ("sips".equalsIgnoreCase(scheme)) { RecordRouteHeader rrh = (RecordRouteHeader) getHeader(RecordRouteHeader.NAME);
			 * 
			 * if(rrh == null) { // If RRH is not null, then this is proxy so RFC 8.1.1.8 doesn't apply, contact doesnt matter when there is RRH SipUri sipUri = (SipUri) this.getContactHeader().getAddress().getURI(); if (!sipUri.getScheme().equals("sips")) { throw new ParseException("Scheme for contact should be sips:" + sipUri, 0); } } } }
			 */
		}
		 ...
		 */
		/*
		 * emmartins: dupe logic, see above if (this.getContactHeader() == null && SIPTransactionStack.isDialogCreated(method)) { throw new ParseException("Contact Header is Mandatory for a SIP INVITE", 0); }
		 */

		if (requestLine != null && method != null && getCSeq().getMethod() != null && method.compareTo(getCSeq().getMethod()) != 0) {
	 ...
	 * 
	 * @return the method from the request line if the method exits and null if the request line or the method does not exist.
	 */
	 ...
	 * 
	 * @return A SIPResponse with the status and reason supplied, and a copy of all the original headers from this request.
	 */
	 ...
	 * 
	 * @return A SIPResponse with the status and reason supplied, and a copy of all the original headers from this request except the ones that are not supposed to be part of the response .
	 */
...
//        newRequest.setRequestLine((RequestLine) this.requestLine.clone());
	...
	public String getMergeId() {
		...
		/*
		 * NOTE : The RFC does NOT specify you need to include a Request URI This is added here for the case of Back to Back User Agents.
		 */
		String requestUri = this.getRequestURI().toString();

		if (fromTag != null) {
			return new StringBuilder().append(requestUri).append(":").append(fromTag).append(":").append(cseq).append(":").append(callId).toString();
		} else
			return null;
