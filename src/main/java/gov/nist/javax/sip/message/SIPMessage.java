 ...
 */
/**
 * This is the main SIP Message structure. <b> This is an implementation class. WARNING do not directly use the methods of this class in your application. Use the methods of the interfaces implemented by this class. </b>
 *
 * @see StringMsgParser
...
public abstract class SIPMessage extends MessageObject implements javax.sip.message.Message, MessageExt {
	...
	/**
	 * The local address that we will send the message from or that we received it on.
	 */
	private InetAddress localAddress;

	/**
	 * The local port that we will send the message from or that we received it on.
	 */
	 ...
	 *
	 * @return a linked list with each element of the list containing a string encoded header in canonical form.
	 */
...
//        StringBuilder encoding = new StringBuilder();
	 ...
	 *
	 * @return The Canonical String representation of the message (including the canonical string representation of the SDP payload if it exists).
	 */
	 ...
	 *
	 * @return The Canonical byte array representation of the message (including the canonical byte array representation of the SDP payload if it exists all in one contiguous byte array).
	 */
	 ...
	 *
	 * @return Formatted string representation of the object. Note that this is NOT the same as encode(). This is used mainly for debugging purposes.
	 */
	 ...
	 * @throws SIPDuplicateHeaderException If replaceFlag is false and only a singleton header is allowed (fpr example CSeq).
	 */
	 ...
	 * @exception SIPDuplicateHeaderException if the header is of a type that cannot tolerate duplicates and one of this type already exists (e.g. CSeq header).
	 * @throws IndexOutOfBoundsException If the index specified is greater than the number of headers that are in this message.
	 */
	...
	public void attachHeader(SIPHeader header, boolean replaceFlag, boolean top) throws SIPDuplicateHeaderException {
		...
		if (!headerTable.containsKey(headerNameLowerCase)) {
			...
			if (h instanceof SIPHeaderList) {
				SIPHeaderList<?> hdrlist = (SIPHeaderList<?>) headerTable.get(headerNameLowerCase);
				if (hdrlist != null)
	 ...
	 *
	 * @return A string containing the concatenation of various portions of the From,To,Via and RequestURI portions of this message as specified in RFC 2543: All responses to a request contain the same values in the Call-ID, CSeq, To, and From fields (with the possible addition of a tag in the To field (section 10.43)). This allows responses to be matched with requests. Incorporates a bug fix for a bug sent in by Gordon Ledgard of IPera for generating transactionIDs when no port is present in the via header. Incorporates a bug fix for a bug report sent in by Chris Mills of Nortel Networks (converts to lower case when returning the transaction identifier).
	 *
	 * @return a string that can be used as a transaction identifier for this message. This can be used for matching responses and requests (i.e. an outgoing request and its matching response have the same computed transaction identifier).
	 */
...
//        }
	 ...
	 * @throws UnsupportedEncodingException if the platform does not support the charset specified in the content type header.
	 *
	 */
	...
	public void setMessageContent(byte[] content, boolean strict, boolean computeContentLength, int givenLength) throws ParseException {
		...
		if ((!computeContentLength)) {
			if ((!strict && this.contentLengthHeader.getContentLength() != givenLength) || this.contentLengthHeader.getContentLength() < givenLength) {
	...
	public ListIterator<SIPHeader> getHeaders(String headerName) {
		...
		SIPHeader sipHeader = (SIPHeader) headerTable.get(SIPHeaderNamesCache.toLowerCase(headerName));
		// empty iterator
		if (sipHeader == null)
	 ...
	 *
	 * @return a list iterator to a list of header names. These are ordered in the same order as are present in the message.
	 */
	 ...
	 *
	 * @param applicationData -- application data pointer to set. null clears the application data pointer.
	 */
