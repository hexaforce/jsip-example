...
public class SipProviderImpl implements javax.sip.SipProvider, gov.nist.javax.sip.SipProviderExt, SIPTransactionEventListener, SIPDialogEventListener {
	...
	/*
	 * A set of listening points associated with the provider At most one LP per transport
	 */
	...
	protected SIPClientTransaction createClientTransaction(Request request, Hop hop) throws TransactionUnavailableException {
		...
		/*
		 * User decided to give us his own via header branch. Lets see if it results in a clash. If so reject the request.
		 */
		if (sipRequest.getTopmostVia().getBranch() != null && sipRequest.getTopmostVia().getBranch().startsWith(SIPConstants.BRANCH_MAGIC_COOKIE) && sipStack.findTransaction((SIPRequest) sipRequest, false) != null) {
		...
		if (sipRequest.getMethod().equalsIgnoreCase(Request.CANCEL)) {
			SIPClientTransaction ct = (SIPClientTransaction) sipStack.findCancelTransaction((SIPRequest) sipRequest, false);
			if (ct != null) {
		...
		if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG))
			logger.logDebug("could not find existing transaction for " + sipRequest.getFirstLine() + " creating a new one ");
		...
		try {
			...
			MessageChannel messageChannel = sipStack.createMessageChannel(sipRequest, listeningPoint.getMessageProcessor(), hop);
	...
	public ServerTransaction getNewServerTransaction(Request request) throws TransactionAlreadyExistsException, TransactionUnavailableException {
		...
		try {
			if (SIPTransactionStack.isDialogCreated(sipRequest.getMethod())) {
				...
				if (isAutomaticDialogSupportEnabled()) {
					/*
					 * Under automatic dialog support, dialog is tied into a transaction. You cannot create a server tx except for dialog creating transactions. After that, all subsequent transactions are created for you by the stack.
					 */
	...
	public void sendRequest(Request request) throws SipException {
		...
		try {
			/*
			 * JvB: Via branch should already be OK, dont touch it here? Some apps forward statelessly, and then it's not set. So set only when not set already, dont overwrite CANCEL branch here..
			 */
	...
	public void transactionErrorEvent(SIPTransactionErrorEvent transactionErrorEvent) {
		...
		if (transactionErrorEvent.getErrorID() == SIPTransactionErrorEvent.TRANSPORT_ERROR) {
			...
			if (tx.getDialog() != null)
				InternalErrorHandler.handleException("Unexpected event !", this.logger);
	...
	public synchronized void removeListeningPoints() {
		for (Iterator it = this.listeningPoints.values().iterator(); it.hasNext();) {
