		...
		if (sipEvent instanceof RequestEvent) {
			try {
				...
				if (tx != null && !tx.passToListener()) {
					...
					if (sipRequest.getMethod().equals(Request.ACK) && tx.isInviteTransaction() && (tx.getLastResponseStatusCode() / 100 == 2 || sipStack.isNon2XXAckPassedToListener())) {
				...
				if (eventWrapper.transaction.getMethod().equals(Request.ACK)) {
					...
					// termination
					eventWrapper.transaction.setState(TransactionState._TERMINATED);
				}
			}
			...
			try {
				...
				try {
					...
					/*
					 * The Client tx disappears after the first 2xx response However, additional 2xx responses may arrive later for example in the following scenario:
					 *
					 ...
					 *
					 * If the Listener does not ACK the 200 then we assume he does not care about the dialog and gc the dialog after some time. However, this is really an application bug. This garbage collects unacknowledged dialogs.
					 *
					 */
				...
				if (ct != null && TransactionState._COMPLETED == ct.getInternalState()
		...
		try {
			...
			while (true) {
				...
				while (iterator.hasNext()) {
					...
					if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
						logger.logDebug("Processing " + eventWrapper + "nevents " + eventsToDeliver.size());
					}
					try {
						deliverEvent(eventWrapper);
					} catch (Exception e) {
						if (logger.isLoggingEnabled()) {
							logger.logError("Unexpected exception caught while delivering event -- carrying on bravely", e);
						}
					}
				}
			} // end While
