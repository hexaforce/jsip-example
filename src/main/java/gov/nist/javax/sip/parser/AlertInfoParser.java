			...
			while ((lexer.lookAhead(0) != '\n') && (lineCount < 20)) {
				do {
					...
					if (this.lexer.lookAhead(0) == '<') {
						...
						/*
						 * This is non standard for Polycom support. I know it is bad grammar but please do not remove. mranga
						 */
...
/*
 * $Log: not supported by cvs2svn $ Revision 1.9 2009/10/22 10:27:36 jbemmel Fix for issue #230, restructured the code such that parsing for any address appearing without '<' '>' stops at ';', then parameters are assigned to the header as expected
 *
 * Revision 1.8 2009/07/17 18:57:57 emcho Converts indentation tabs to spaces so that we have a uniform indentation policy in the whole project.
 *
 * Revision 1.7 2007/10/18 17:48:09 mranga Issue number: Obtained from: Submitted by: mardy Reviewed by: mranga Alert info patch to accept non standard alert info headers.
 *
 * Revision 1.6 2006/07/13 09:02:19 mranga Issue number: Obtained from: Submitted by: jeroen van bemmel Reviewed by: mranga Moved some changes from jain-sip-1.2 to java.net
 *
