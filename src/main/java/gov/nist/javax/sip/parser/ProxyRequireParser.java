...
public class ProxyRequireParser extends HeaderParser {
	...
	/**
	 * Test program public static void main(String args[]) throws ParseException { String r[] = { "Proxy-Require: foo \n", "Proxy-Require: foo1, foo2 , 389\n" };
	 * 
	 * for (int i = 0; i < r.length; i++ ) { ProxyRequireParser parser = new ProxyRequireParser(r[i]); ProxyRequireList rl= (ProxyRequireList) parser.parse(); System.out.println("encoded = " + rl.encode()); } }
	 */
}
/*
 * $Log: not supported by cvs2svn $ Revision 1.6 2006/07/13 09:02:15 mranga Issue number: Obtained from: Submitted by: jeroen van bemmel Reviewed by: mranga Moved some changes from jain-sip-1.2 to java.net
 *
