...
public class ServerParser extends HeaderParser {
	...
	/*
	 * public static void main(String args[]) throws ParseException { String server[] = { "Server: Softphone/Beta1.5 \n", "Server: HomeServer v2\n", "Server: Nist/Beta1 (beta version) \n", "Server: Nist proxy (beta version)\n", "Server: Nist1.0/Beta2 UbiServer/vers.1.0 (new stuff) (Cool) \n", "Server: Sip EXpress router (0.8.11 (sparc64/solaris))\n" };
	 * 
	 * for (int i = 0; i < server.length; i++ ) { ServerParser parser = new ServerParser(server[i]); Server s= (Server) parser.parse(); System.out.println("encoded = " + s.encode()); }
	 * 
	 * }
	 */
...
/*
 * $Log: not supported by cvs2svn $ Revision 1.8 2006/07/13 09:02:16 mranga Issue number: Obtained from: Submitted by: jeroen van bemmel Reviewed by: mranga Moved some changes from jain-sip-1.2 to java.net
 *
 ...
 *
 *
 * Revision 1.6 2004/01/30 17:10:47 mranga Reviewed by: mranga Server and user agent parser leave an extra Linefeed at the end of token.
 *
 * Revision 1.5 2004/01/27 13:52:11 mranga Reviewed by: mranga Fixed server/user-agent parser. suppress sending ack to TU when retransFilter is enabled and ack is retransmitted.
 *
