...
public final class PipelinedMsgParser implements Runnable {
	 ...
	 *
	 * @param sipMessageListener Message listener which has methods that get called back from the parser when a parse is complete
	 ...
	 *
	 * @param mhandler a SIPMessageListener implementation that provides the message handlers to handle correctly and incorrectly parsed messages.
				...
				while (true) {
					try {
						...
						if (maxMessageSize > 0 && bytesRead > (maxMessageSize / 2))
							throw new IOException("Pre-content-length headers size exceeded. The size of the message of the headers prior to Content-Length is too large. This must be an invalid message. Limit is MAX_MESSAGE_SIZE/2=" + maxMessageSize / 2);
...
//                smp.setReadBody(false);
				...
				if (maxMessageSize > 0 && contentLength > maxMessageSize)
					throw new RuntimeException("Max content size Exceeded! :" + contentLength + " allowed max size is " + maxMessageSize);
				...
				if (sipMessageListener != null) {
					try {
						if (PostParseExecutorServices.getPostParseExecutor() == null) {
							...
							/**
							 * If gov.nist.javax.sip.TCP_POST_PARSING_THREAD_POOL_SIZE is disabled we continue with the old logic here.
							 */
							if (sipStack.sipEventInterceptor != null) {
							...
							/**
							 * gov.nist.javax.sip.TCP_POST_PARSING_THREAD_POOL_SIZE is enabled so we use the threadpool to execute the task.
							 */
...
/*
 * $Log: not supported by cvs2svn $ Revision 1.38 2010/12/02 11:44:16 vralev Issue number: 346 Obtained from: vralev
 *
 * Congestion control. Patch + Tests for HEAD
 *
 * Revision 1.37 2010/10/26 23:49:12 vralev Issue number: 337 Obtained from: vralev
 *
 * Revision 1.36 2010/10/13 15:26:59 deruelle_jean Fix for TCP calls under load freeze JAIN SIP with TCP_POST_PARSING_THREAD_POOL_SIZE > 0
 *
 * Issue number: Obtained from: Submitted by: Jean Deruelle Reviewed by:
 *
 * Revision 1.35 2010/10/12 18:47:16 deruelle_jean Fix for restarting the stack when the gov.nist.javax.sip.TCP_POST_PARSING_THREAD_POOL_SIZE option is used
 *
 * Issue number: Obtained from: Submitted by: Jean Deruelle Reviewed by:
 *
 * Revision 1.34 2010/10/07 15:40:25 deruelle_jean Adding some cleaning up to POST_PARSING_THREAD_POOL option
 *
 * Issue number: Obtained from: Submitted by: Jean Deruelle Reviewed by:
 *
 * Revision 1.33 2010/10/07 15:03:49 deruelle_jean Fixing a deadlock on one post_parser_thread_pool option when there is only 1 thread and message ordering on multiple threads + adding non regression test case
 *
 * Issue number: Obtained from: Submitted by: Jean Deruelle Reviewed by:
 *
 * Revision 1.32 2010/08/19 19:18:01 deruelle_jean Fixing Message Order, there could be race conditions on TCP with multiple threads the order should be maintained
 *
 * Issue number: 301 Obtained from: Submitted by: Jean Deruelle Reviewed by:
 *
 * Revision 1.31 2010/07/01 18:53:25 vralev Issue number: 301 Obtained from: vralev Submitted by: vralev Reviewed by: ranga
 *
 ...
 *
 * Revision 1.30 2010/07/01 18:22:56 vralev Issue number: 301 Obtained from: vralev Submitted by: vralev Reviewed by: ranga
 *
 * Revision 1.29 2010/05/06 14:07:45 deruelle_jean Big update to improve performance by 50% in some cases, TCK + testsuite (cc-buildloop) green, Mobicents Sip Servlets TCK + testsuite green as well
 *
 * Issue number: Obtained from: Submitted by: Jean Deruelle Reviewed by:
 *
 * Revision 1.28 2010/03/19 17:29:46 deruelle_jean Adding getters and setters for the new factories
 *
 * Issue number: Obtained from: Submitted by: Jean Deruelle Reviewed by:
 *
 * Revision 1.27 2010/03/15 17:08:57 deruelle_jean Adding javadoc
 *
 * Issue number: 251 Obtained from: Submitted by: Jean Deruelle Reviewed by: Ranga
 *
 * Revision 1.26 2010/03/15 17:01:21 deruelle_jean Applying patch allowing pluggable message parser implementation
 *
 * Issue number: 251 Obtained from: Submitted by: Jean Deruelle Reviewed by: Ranga
 *
 * Revision 1.25 2010/02/27 17:34:04 mranga Issue number: 269 Fix PipelinedMessageParser.java to use UTF-8 encoding when reading stream.
 *
 * Revision 1.24 2010/02/27 06:09:00 mranga Patch from Frederic
 *
 * Revision 1.23 2009/08/16 17:28:28 mranga Issue number: 208 Obtained from: Submitted by: Reviewed by:
 *
 * Add authentication mechanism that uses H(username:domain:password)
 *
 * Revision 1.22 2009/07/17 18:58:02 emcho Converts indentation tabs to spaces so that we have a uniform indentation policy in the whole project.
 *
 * Revision 1.21 2008/05/24 04:10:01 mranga
 *
 * Issue number: 158 Obtained from: Submitted by: Reviewed by: mranga
 *
 * Deliver tx timeout for Canceled INVITE. Fix pipeline thread exit.
 *
 * Revision 1.20 2008/05/22 19:38:07 jbemmel Fix for issue 149: the logic wasn't always closing the internal socket pipe, causing the pipe reader thread to block indefinitely
 *
 * Repeatedly starting/stopping the stack then gives hanging threads Revision 1.19 2007/01/28 13:06:21 mranga Issue number: 99 Obtained from: Submitted by: Reviewed by: mranga
 *
 ...
 *
 * Revision 1.18 2006/07/13 09:02:10 mranga Issue number: Obtained from: Submitted by: jeroen van bemmel Reviewed by: mranga Moved some changes from jain-sip-1.2 to java.net
 *
 * CVS: ---------------------------------------------------------------------- CVS: Issue number: CVS: If this change addresses one or more issues, CVS: then enter the issue number(s) here. CVS: Obtained from: CVS: If this change has been taken from another system, CVS: then name the system in this line, otherwise delete it. CVS: Submitted by: CVS: If this code has been contributed to the project by someone else; i.e., CVS: they sent us a patch or a set of diffs, then include their name/email CVS: address here. If this is your work then delete this line. CVS: Reviewed by: CVS: If we are doing pre-commit code reviews and someone else has CVS: reviewed your changes, include their name(s) here. CVS: If you have not had it reviewed then delete this line.
 *
 * Revision 1.4 2006/06/19 06:47:27 mranga javadoc fixups
 *
 * Revision 1.3 2006/06/17 10:18:14 mranga Added some synchronization to the sequence number checking. Small javadoc fixups
 *
 ...
 *
 *
 * Revision 1.16 2004/11/30 23:28:14 mranga Issue number: 44 Submitted by: Rob Daugherty Reviewed by: M. Ranganathan
 *
 * TCP Pipelining truncates content when other end of pipe is closed.
 *
 * Revision 1.15 2004/05/30 18:55:56 mranga Reviewed by: mranga Move to timers and eliminate the Transaction scanner Thread to improve scalability and reduce cpu usage.
 *
 * Revision 1.14 2004/05/16 14:13:22 mranga Reviewed by: mranga Fixed the use-count issue reported by Peter Parnes. Added property to prevent against content-length dos attacks.
 *
 * Revision 1.13 2004/03/19 04:22:22 mranga Reviewed by: mranga Added IO Pacing for long writes - split write into chunks and flush after each chunk to avoid socket back pressure.
 *
 * Revision 1.12 2004/03/18 22:01:19 mranga Reviewed by: mranga Get rid of the PipedInputStream from pipelined parser to avoid a copy.
 *
 * Revision 1.11 2004/03/07 22:25:23 mranga Reviewed by: mranga Added a new configuration parameter that instructs the stack to drop a server connection after server transaction termination set gov.nist.javax.sip.CACHE_SERVER_CONNECTIONS=false for this Default behavior is true.
 *
 * Revision 1.10 2004/02/29 15:32:58 mranga Reviewed by: mranga bug fixes on limiting the max message size.
 *
 * Revision 1.9 2004/02/29 00:46:34 mranga Reviewed by: mranga Added new configuration property to limit max message size for TCP transport. The property is gov.nist.javax.sip.MAX_MESSAGE_SIZE
 *
 * Revision 1.8 2004/02/25 21:43:03 mranga Reviewed by: mranga Added a couple of todo's and removed some debug printlns that could slow code down by a bit.
 *
 * Revision 1.7 2004/02/25 20:52:46 mranga Reviewed by: mranga Fix TCP transport so messages in excess of 8192 bytes are accepted.
 *
 * Revision 1.6 2004/01/22 18:39:41 mranga Reviewed by: M. Ranganathan Moved the ifdef SIMULATION and associated tags to the first column so Prep preprocessor can deal with them.
 *
 * Revision 1.5 2004/01/22 14:23:45 mranga Reviewed by: mranga Fixed some minor formatting issues.
 *
