...
public class StringMsgParser implements MessageParser {
	 ...
	 *
	 * @param msgBuffer a byte buffer containing the messages to be parsed. This can consist of multiple SIP Messages concatenated together.
	 * @return a SIPMessage[] structure (request or response) containing the parsed SIP message.
	 * @exception ParseException is thrown when an illegal message has been encountered (and the rest of the buffer is discarded).
		...
		if (!firstLine.startsWith(SIPConstants.SIP_VERSION_STRING)) {
			...
			try {
				...
				if (parseExceptionListener != null)
					try {
						parseExceptionListener.handleException(ex, message, RequestLine.class, firstLine, new String(msgBuffer, "UTF-8"));
					} catch (UnsupportedEncodingException e) {
						e.printStackTrace();
					}
				else
			...
			try {
				...
				if (parseExceptionListener != null) {
					try {
						parseExceptionListener.handleException(ex, message, StatusLine.class, firstLine, new String(msgBuffer, "UTF-8"));
					} catch (UnsupportedEncodingException e) {
						e.printStackTrace();
					}
				} else
	 ...
	 *
	 *                   public HostPort parseHostPort(String hostport) throws ParseException { Lexer lexer = new Lexer("charLexer", hostport); return new HostNameParser(lexer).hostPort();
	 * 
	 *                   }
	 */
	 ...
	 * @return a parsed address structure.
	 * @since v1.0
	 * @exception ParseException a ParseException when the hostname is badly formatted.
	 */
	 ...
	 * @return a parsed address structure.
	 * @since v1.0
	 * @exception ParseException a ParseException when the address is badly formatted.
	 */
		...
		class ParserThread implements Runnable {
			...
			public void run() {
				for (int i = 0; i < messages.length; i++) {
					...
					try {
						SIPMessage sipMessage = smp.parseSIPMessage(messages[i].getBytes(), true, false, null);
