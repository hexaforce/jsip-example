...
/**
 * This is a FSM that can parse a single stream of messages with they bodies and then pass the sip message to the listeners. It accumulates bytes until end of message is detected or some DoS trigger terminates it due to excessive amount of bytes per message or line.
 * 
...
public class NioPipelineParser {
	...
	/*
	 * This is where we receive the bytes from the stream and we analyze the through message structure. For TCP the key things to identify are message lines for the headers, parse the Content-Length header and then read the message body (aka message content). For TCP the Content-Length must be 100% accurate.
	 */
