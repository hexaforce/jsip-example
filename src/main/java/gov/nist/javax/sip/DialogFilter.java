...
class DialogFilter implements ServerRequestInterface, ServerResponseInterface {
	...
	private void sendTryingResponse(SIPRequest sipRequest, SIPServerTransaction transaction) {
		SIPResponse sipResponse = sipRequest.createResponse(Response.TRYING);
		if (transaction.getState() != TransactionState.TERMINATED) {
	...
	public void processRequest(SIPRequest sipRequest, MessageChannel incomingMessageChannel) {
		...
		if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG) && listeningPoint != null)
			logger.logDebug("PROCESSING INCOMING REQUEST " + sipRequest + " transactionChannel = " + transactionChannel + " listening point = " + listeningPoint.getIPAddress() + ":" + listeningPoint.getPort());
		...
		/*
		 * Check if we got this request on the contact address of the dialog If not the dialog does not belong to this request. We check this condition if a contact address has been assigned to the dialog. Forgive the sins of B2BUA's that like to record route ACK's
		 */
		if (dialog != null && sipProvider != dialog.getSipProvider()) {
			...
			if (contact != null) {
				...
				if (ipAddress != null && (!ipAddress.equals(listeningPoint.getIPAddress()) || contactPort != listeningPoint.getPort())) {
					if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
						logger.logDebug("nulling dialog -- listening point mismatch!  " + contactPort + "  lp port = " + listeningPoint.getPort());
		...
		if (sipRequestMethod.equals(Request.REFER) && dialog != null && sipProvider.isDialogErrorsAutomaticallyHandled()) {
			...
			if (lastTransaction != null && sipProvider.isDialogErrorsAutomaticallyHandled()) {
				...
				if (lastTransaction instanceof SIPServerTransaction) {
					...
					if ((lastTransaction.getInternalState() == TransactionState._PROCEEDING || lastTransaction.getInternalState() == TransactionState._TRYING) && lastTransactionMethod.equals(Request.INVITE)) {
						this.sendRequestPendingResponse(sipRequest, transaction);
						return;
					}
				} else if (lastTransaction instanceof SIPClientTransaction) {
					if (lastTransactionMethod.equals(Request.INVITE) && lastTransaction.getInternalState() != TransactionState._TERMINATED && lastTransaction.getInternalState() != TransactionState._COMPLETED) {
						this.sendRequestPendingResponse(sipRequest, transaction);
						return;
					}
				}
			...
			/*
			 * Got an UPDATE method and the user dialog does not exist and the user wants to be a User agent.
			 */
			...
			if (transaction != null && transaction.isInviteTransaction()) {
				...
				if (dialog == null) {
					...
					/*
					 * JvB: must never drop ACKs that dont match a transaction! One cannot be sure if it isn't an ACK for a 2xx response
					 */
					...
					if (!dialog.handleAck(transaction)) {
						if (!dialog.isSequenceNumberValidation()) {
							...
							/*
							 * For test only we support a flag that will deliver retransmitted ACK for 200 OK responses to the listener.
							 */
							if ((!sipStack.isDeliverRetransmittedAckToListener()) || (ackTransaction != null && !sipStack.isNon2XXAckPassedToListener())) {
								return;
							}
						}
					} else {
						...
						/*
						 * Note that ACK is a pseudo transaction. It is never added to the stack and you do not get transaction terminated events on ACK.
						 */
			...
			/*
			 * Saw an interleaved invite before ACK was sent. RFC 3261 Chapter 14. A UAS that receives an INVITE on a dialog while an INVITE it had sent on that dialog is in progress MUST return a 491 (Request Pending) response to the received INVITE.
			 */
			...
			if (dialog != null && lastTransaction != null && sipProvider.isDialogErrorsAutomaticallyHandled() && lastTransaction.isInviteTransaction() && lastTransaction instanceof ServerTransaction && sipRequest.getCSeq().getSeqNumber() > lastTransaction.getCSeq()
			// Handle Pseudo State Trying on Server Transaction
					&& (lastTransaction.getInternalState() == TransactionState._PROCEEDING || lastTransaction.getInternalState() == TransactionState._TRYING)) {
		...
		if (dialog != null && transaction != null && !sipRequestMethod.equals(Request.BYE) && !sipRequestMethod.equals(Request.CANCEL) && !sipRequestMethod.equals(Request.ACK) && !sipRequestMethod.equals(Request.PRACK)) {

			if (!dialog.isRequestConsumable(sipRequest)) {

				/*
				 * RFC 3261: "UAS Behavior" section (12.2.2): If the remote sequence number was not empty, but the sequence number of the request is lower than the remote sequence number, the request is out of order and MUST be rejected with a 500 (Server Internal Error) response.
				 */
				...
				/*
				 * && (transaction.getState() == TransactionState.TRYING || transaction .getState() == TransactionState.PROCEEDING)
				 */
		...
		if (dialog == null && sipRequestMethod.equals(Request.NOTIFY)) {
			...
			/*
			 * RFC 3265: Upon receiving a NOTIFY request, the subscriber should check that it matches at least one of its outstanding subscriptions; if not, it MUST return a "481 Subscription does not exist" response unless another 400- or -class response is more appropriate.
			 */
			if (sipProvider.isAutomaticDialogSupportEnabled() && pendingSubscribeClientTx == null && !sipStack.isDeliverUnsolicitedNotify()) {
				/*
				 * This is the case of the UAC receiving a Stray NOTIFY for which it has not previously sent out a SUBSCRIBE and for which it does not have an established dialog.
				 */
				try {
					if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
			...
			if (pendingSubscribeClientTx != null) {
				...
				if (transaction != null && ((SIPServerTransaction) transaction).isTransactionMapped()) {
					...
					// knows
					// about it.
					sipEvent = new RequestEventExt((SipProvider) sipProvider, (ServerTransaction) transaction, subscriptionDialog, (Request) sipRequest);

				} else {
					/*
					 * Shadow transaction has been created but the stack does not know about it.
					 */
					sipEvent = new RequestEventExt((SipProvider) sipProvider, null, subscriptionDialog, (Request) sipRequest);
				...
				sipEvent = new RequestEventExt(sipProvider, null, null, (Request) sipRequest);
			}
			...
			if (transaction != null && (((SIPServerTransaction) transaction).isTransactionMapped())) {
				sipEvent = new RequestEventExt(sipProvider, (ServerTransaction) transaction, dialog, (Request) sipRequest);
			} else {
				sipEvent = new RequestEventExt(sipProvider, null, dialog, (Request) sipRequest);
	...
	public void processResponse(SIPResponse response, MessageChannel incomingMessageChannel, SIPDialog dialog) {
		...
		if (transaction == null) {
			...
			if (sipStack.getMaxForkTime() != 0 && SIPTransactionStack.isDialogCreated(response.getCSeqHeader().getMethod())) {
				if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
		...
		ResponseEventExt responseEvent = new ResponseEventExt(sipProvider, (ClientTransactionExt) transaction, dialog, (Response) response);
		if (sipStack.getMaxForkTime() != 0 && SIPTransactionStack.isDialogCreated(response.getCSeqHeader().getMethod())) {
			SIPClientTransaction forked = this.sipStack.getForkedTransaction(response.getForkId());
			if (dialog != null && forked != null) {
				dialog.checkRetransmissionForForking(response);
	 ...
	 * 
	 * @return auxiliary information that we may have generated during the message processing which is retrieved by the message logger.
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see gov.nist.javax.sip.stack.ServerResponseInterface#processResponse(gov. nist.javax.sip.message.SIPResponse, gov.nist.javax.sip.stack.MessageChannel)
	 */
	public void processResponse(SIPResponse sipResponse, MessageChannel incomingChannel) {
		...
		if (SIPTransactionStack.isDialogCreated(method) && sipResponse.getStatusCode() != 100 && sipResponse.getFrom().getTag() != null && sipResponse.getTo().getTag() != null && sipDialog == null) {
			...
			if (sipProvider.isAutomaticDialogSupportEnabled()) {
				...
				ClientTransactionExt originalTx = this.sipStack.getForkedTransaction(sipResponse.getForkId());
		...
		if (sipStack.getMaxForkTime() != 0 && SIPTransactionStack.isDialogCreated(sipResponse.getCSeqHeader().getMethod())) {
			ClientTransactionExt originalTx = this.sipStack.getForkedTransaction(sipResponse.getForkId());
