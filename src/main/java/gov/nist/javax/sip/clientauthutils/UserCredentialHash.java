...
/**
 * Interface for those clients that only supply hash(user:domain:password). This is more secure than simply supplying password because the password cannot be extracted. Implementations tend to prefer to store information in user accounts using such a hash rather than plain text passwords because it offers better security.
 * 
 */
