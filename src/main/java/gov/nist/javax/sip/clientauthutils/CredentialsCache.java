...
/**
 * A cache of authorization headers to be used for subsequent processing when we set up calls. We cache credentials on a per proxy domain per user basis.
 *
 */
...
class CredentialsCache {
	...
	/**
	 * The key for this map is the proxy domain name. A given proxy authorizes a user for a number of domains. The Hashtable value of the mapping is a mapping of user names to AuthorizationHeader list for that proxy domain.
	 */
	 ...
	 *
	 * @param callid        the id of the call that the <tt>authorization</tt> header belongs to.
	 ...
	 *
	 * @param callid the call id that we'd like to retrive a cached authorization header for.
	 *
