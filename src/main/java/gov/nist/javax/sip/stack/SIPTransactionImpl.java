...
public abstract class SIPTransactionImpl implements SIPTransaction {
	...
	/**
	 * The linger timer is used to remove the transaction from the transaction table after it goes into terminated state. This allows connection caching and also takes care of race conditins.
	 *
	 *
	 */
	...
	protected SIPTransactionImpl(SIPTransactionStack newParentStack, MessageChannel newEncapsulatedChannel) {
		...
		if (this.isReliable()) {
			...
			if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
				logger.logDebug("use count for encapsulated channel" + this + " " + encapsulatedChannel.useCount);
		}
...
//                originalRequestBytes = null;
	...
	public void setState(int newState) {
		...
		if (currentState == TransactionState._COMPLETED) {
			if (newState != TransactionState._TERMINATED && newState != TransactionState._CONFIRMED)
	 ...
	 *
	 * @param tickCount Number of ticks before the next retransmission timer event occurs.
	 */
	...
	protected void enableTimeoutTimer(int tickCount) {
		if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
			logger.logDebug("enableTimeoutTimer " + this + " tickCount " + tickCount + " currentTickCount = " + timeoutTimerTicksLeft);
	...
	public void sendMessage(final SIPMessage messageToSend) throws IOException {
		...
		try {
			...
			for (MessageProcessor messageProcessor : sipStack.getMessageProcessors()) {
				...
				if (addrmatch && messageProcessor.getPort() == this.getPeerPort() && messageProcessor.getTransport().equalsIgnoreCase(this.getPeerProtocol())) {
					if (channel instanceof TCPMessageChannel) {
						try {
							...
							Runnable processMessageTask = new Runnable() {
								...
								public void run() {
									try {
										((TCPMessageChannel) channel).processMessage((SIPMessage) messageToSend.clone(), getPeerInetAddress());
									} catch (Exception ex) {
					...
					if (channel instanceof TLSMessageChannel) {
						try {
							...
							Runnable processMessageTask = new Runnable() {
								...
								public void run() {
									try {
										((TLSMessageChannel) channel).processMessage((SIPMessage) messageToSend.clone(), getPeerInetAddress());
									} catch (Exception ex) {
										if (logger.isLoggingEnabled(ServerLogger.TRACE_ERROR)) {
...
//            topViaHeader = (Via) viaHeaders.getFirst();
			...
			if (messageBranch != null && this.getBranch() != null) {
				...
				// this message,
				if (getBranch().equalsIgnoreCase(messageBranch) && topViaHeader.getSentBy().equals(origRequest.getTopmostVia().getSentBy())) {
...
//      TIMER_D = 32000 / BASE_TIMER_INTERVAL;
	...
	public void scheduleMaxTxLifeTimeTimer() {
		if (maxTxLifeTimeListener == null && this.getMethod().equalsIgnoreCase(Request.INVITE) && sipStack.getMaxTxLifetimeInvite() > 0) {
			...
			sipStack.getTimer().schedule(maxTxLifeTimeListener, sipStack.getMaxTxLifetimeInvite() * 1000);
		}
