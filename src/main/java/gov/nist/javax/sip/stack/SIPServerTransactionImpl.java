...
//                topViaHeader = (Via) viaHeaders.getFirst();
				...
				if (messageBranch != null && this.getBranch() != null) {
					if (method.equals(Request.CANCEL)) {
						...
						transactionMatches = this.getMethod().equals(Request.CANCEL) && getBranch().equalsIgnoreCase(messageBranch) && topViaHeader.getSentBy().equals(origRequest.getTopmostVia().getSentBy());

					} else {
						// Matching server side transaction with only the
						// branch parameter.
						if (origRequest != null) {
							transactionMatches = getBranch().equalsIgnoreCase(messageBranch) && topViaHeader.getSentBy().equals(origRequest.getTopmostVia().getSentBy());
						} else {
							transactionMatches = getBranch().equalsIgnoreCase(messageBranch) && topViaHeader.getSentBy().equals(originalRequestSentBy);
						}
					...
					} else if ((isResponse || origRequest.getRequestURI().equals(((SIPRequest) messageToTest).getRequestURI())) && (skipFrom || originalFromTag != null && originalFromTag.equalsIgnoreCase(thisFromTag)) && (skipTo || originalToTag != null && originalToTag.equalsIgnoreCase(thisToTag)) && origRequest.getCallId().getCallId().equalsIgnoreCase(messageToTest.getCallId().getCallId()) && origRequest.getCSeq().getSeqNumber() == messageToTest.getCSeq().getSeqNumber() && ((!messageToTest.getCSeq().getMethod().equals(Request.CANCEL)) || getMethod().equals(messageToTest.getCSeq().getMethod())) && topViaHeader.equals(origRequest.getTopmostVia())) {
	...
	public void processRequest(SIPRequest transactionRequest, MessageChannel sourceChannel) {
		...
		try {
			...
			if (getRealState() < 0) {
				...
				// the completed state,
			} else if (isInviteTransaction() && TransactionState._COMPLETED == getRealState() && transactionRequest.getMethod().equals(Request.ACK)) {
				...
				if (TransactionState._PROCEEDING == getRealState() || TransactionState._COMPLETED == getRealState()) {
					this.semRelease();
			...
			if (TransactionState._COMPLETED != getRealState() && TransactionState._TERMINATED != getRealState() && requestOf != null) {
				...
				if (SIPTransactionStack.isDialogCreated(getMethod()) && getRealState() == TransactionState._TERMINATED && transactionRequest.getMethod().equals(Request.ACK) && requestOf != null) {
...
//                }
			...
			if (isReliable()) {
				if (logger.isLoggingEnabled(ServerLogger.TRACE_MESSAGES)) {
					...
					try {
						...
						lastReparsedResponse.setLocalPort(getMessageChannel().getPort());
						lastReparsedResponse.setLocalAddress(getMessageChannel().getMessageProcessor().getIpAddress());
				...
				Hop hop = sipStack.addressResolver.resolveAddress(new HopImpl(lastResponseHost, lastResponsePort, lastResponseTransport));
	...
	public void fireTimeoutTimer() {
		...
		if (SIPTransactionStack.isDialogCreated(getMethod()) && (TransactionState._CALLING == this.getRealState() || TransactionState._TRYING == this.getRealState())) {
		...
		if (TransactionState._COMPLETED == this.getRealState() && isInviteTransaction()) {
			...
			this.setState(TransactionState._TERMINATED);
			sipStack.removeTransaction(this);
		} else if (!isInviteTransaction() && (TransactionState._COMPLETED == this.getRealState() || TransactionState._CONFIRMED == this.getRealState())) {
			this.setState(TransactionState._TERMINATED);
	...
	public void sendResponse(Response response) throws SipException {
		...
		if (!responseMethod.equals(this.getMethod())) {
			throw new IllegalTransactionStateException("CSeq method does not match Request method of request that created the tx.", Reason.UnmatchingCSeq);
		}

		/*
		 * 200-class responses to SUBSCRIBE requests also MUST contain an "Expires" header. The period of time in the response MAY be shorter but MUST NOT be longer than specified in the request.
		 */
		...
		if (this.getMethod().equals(Request.SUBSCRIBE) && statusCode / 100 == 2) {
			...
			if (response.getHeader(ExpiresHeader.NAME) == null) {
				 ...
				 */
				if (requestExpires != null && responseExpires.getExpiresLong() > requestExpires.getExpiresLong()) {
		...
		if (statusCode == 200 && responseMethod.equals(Request.INVITE) && sipResponse.getHeader(ContactHeader.NAME) == null)
		...
		try {
			...
			if (this.pendingReliableResponseAsBytes != null && this.getDialog() != null && this.getInternalState() != TransactionState._TERMINATED && statusCode / 100 == 2 && contentTypeHeader != null && contentTypeHeader.getContentType().equalsIgnoreCase(CONTENT_TYPE_APPLICATION) && contentTypeHeader.getContentSubType().equalsIgnoreCase(CONTENT_SUBTYPE_SDP)) {
			...
			if (dialog != null && statusCode != 100) {
			...
			} else if (dialog == null && isInviteTransaction() && this.retransmissionAlertEnabled && this.retransmissionAlertTimerTask == null && response.getStatusCode() / 100 == 2) {
				...
				sipStack.getTimer().scheduleWithFixedDelay(this.retransmissionAlertTimerTask, 0, SIPTransactionStack.BASE_TIMER_INTERVAL);
	...
	public void setState(int newState) {
		...
		if (newState == TransactionState._TERMINATED && this.isReliable() && (!getSIPStack().cacheServerConnections)) {
	...
	public void setDialog(SIPDialog sipDialog, String dialogId) {
		...
		if (this.retransmissionAlertEnabled && this.retransmissionAlertTimerTask != null) {
			...
			if (this.retransmissionAlertTimerTask.dialogId != null) {
				sipStack.retransmissionAlertTransactions.remove(this.retransmissionAlertTimerTask.dialogId);
			}
	...
	public void sendReliableProvisionalResponse(Response relResponse) throws SipException {
		...
		/*
		 * After the first reliable provisional response for a request has been acknowledged, the UAS MAY send additional reliable provisional responses. The UAS MUST NOT send a second reliable provisional response until the first is acknowledged.
		 */
		...
		try {
			...
			this.sipStack.getTimer().scheduleWithFixedDelay(provisionalResponseTask, 0, SIPTransactionStack.BASE_TIMER_INTERVAL);
	...
	public void releaseSem() {
		if (this.pendingSubscribeTransaction != null) {
			/*
			 * When a notify is being processed we take a lock on the subscribe to avoid racing with the OK of the subscribe.
			 */
			...
			/*
			 * When a CANCEL is being processed we take a nested lock on the associated INVITE server tx.
			 */
...
//            transactionTimerStarted = null;
		...
		if ((!sipStack.cacheServerConnections) && isReliable() && --getMessageChannel().useCount <= 0) {
			// Close the encapsulated socket if stack is configured
			close();
		} else {
			if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG) && (!sipStack.cacheServerConnections) && isReliable()) {
