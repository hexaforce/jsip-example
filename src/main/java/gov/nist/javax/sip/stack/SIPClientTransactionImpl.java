...
public class SIPClientTransactionImpl extends SIPTransactionImpl implements SIPClientTransaction {
	...
	public boolean isMessagePartOfTransaction(SIPMessage messageToTest) {
		...
		if (TransactionState._COMPLETED == this.getInternalState()) {
			if (rfc3261Compliant) {
				transactionMatches = getBranch().equalsIgnoreCase(topMostViaHeader.getBranch()) && getMethod().equals(messageToTest.getCSeq().getMethod());
			} else {
			...
			if (rfc3261Compliant) {
				...
				if (getBranch() != null) {
					...
					transactionMatches = ((SIPRequest) getRequest()).getTransactionId().equalsIgnoreCase(messageToTest.getTransactionId());
				}
	...
	public void sendMessage(SIPMessage messageToSend) throws IOException {
		...
		try {
			...
			if (TransactionState._PROCEEDING == getInternalState() || TransactionState._CALLING == getInternalState()) {
	...
	/**
	 * @see gov.nist.javax.sip.stack.SIPClientTransaction#processResponse(gov.nist.javax.sip.message.SIPResponse, gov.nist.javax.sip.stack.MessageChannel, gov.nist.javax.sip.stack.SIPDialog)
	 */
	@Override
	public synchronized void processResponse(SIPResponse transactionResponse, MessageChannel sourceChannel, SIPDialog dialog) {
		...
		// Ignore 1xx
		if ((TransactionState._COMPLETED == this.getInternalState() || TransactionState._TERMINATED == this.getInternalState()) && transactionResponse.getStatusCode() / 100 == 1) {
			return;
		}
		 ...
		 * 
		 * if (dialog != null && transactionResponse.getStatusCode() != 100 && (transactionResponse.getTo().getTag() != null || sipStack .isRfc2543Supported())) { // add the route before you process the response. dialog.setLastResponse(this, transactionResponse); this.setDialog(dialog, transactionResponse.getDialogId(false)); }
		 */
	...
	private void inviteClientTransaction(SIPResponse transactionResponse, MessageChannel sourceChannel, SIPDialog dialog) throws IOException {
		...
		if (TransactionState._TERMINATED == this.getInternalState()) {
			...
			if (statusCode / 100 == 2) {
				...
				/*
				 * When in either the "Calling" or "Proceeding" states, reception of response with status code from 300-699 MUST cause the client transaction to transition to "Completed". The client transaction MUST pass the received response up to the TU, and the client transaction MUST generate an ACK request.
				 */
	...
	public void sendRequest() throws SipException {
		...
		try {
			/*
			 * This check is removed because it causes problems for load balancers ( See issue 136) reported by Raghav Ramesh ( BT )
			 */
			if (this.getMethod().equals(Request.CANCEL) && sipStack.isCancelClientTransactionChecked()) {
				...
				if (ct == null) {
					/*
					 * If the original request has generated a final response, the CANCEL SHOULD NOT be sent, as it is an effective no-op, since CANCEL has no effect on requests that have already generated a final response.
					 */
			...
			if (this.getDefaultDialog() != null && isInviteTransaction() && expiresTime != -1 && expiresTimerTask == null) {
	...
	public void fireRetransmissionTimer() {
		...
		try {
			...
			if ((inv && TransactionState._CALLING == s) || (!inv && (TransactionState._TRYING == s || TransactionState._PROCEEDING == s))) {
				...
				if (lastRequest != null) {
					...
					if (this.timeoutIfStillInCallingState && this.getInternalState() == TransactionState._CALLING) {
	...
	public void fireTimeoutTimer() {
		...
		if (TransactionState._CALLING == this.getInternalState() || TransactionState._TRYING == this.getInternalState() || TransactionState._PROCEEDING == this.getInternalState()) {
		...
		if (TransactionState._COMPLETED != this.getInternalState() && TransactionState._TERMINATED != this.getInternalState()) {
			...
			if (this.getMethod().equalsIgnoreCase(Request.CANCEL)) {
				...
				if (inviteTx != null && ((inviteTx.getInternalState() == TransactionState._CALLING || inviteTx.getInternalState() == TransactionState._PROCEEDING)) && inviteTx.getDialog() != null) {
					/*
					 * A proxy server should have started TIMER C and take care of the Termination using transaction.terminate() by itself (i.e. this is not the job of the stack at this point but we do it to be nice.
					 */
	...
	public void setState(int newState) {
		...
		if (newState == TransactionState._TERMINATED && this.isReliable() && (!getSIPStack().cacheClientConnections)) {
			// Set a time after which the connection
			// is closed.
			this.collectionTime = TIMER_J;

		}
		if (super.getInternalState() != TransactionState._COMPLETED && (newState == TransactionState._COMPLETED || newState == TransactionState._TERMINATED)) {
	 ...
	 */
	/**
	 * @see gov.nist.javax.sip.stack.SIPClientTransaction#processResponse(gov.nist.javax.sip.message.SIPResponse, gov.nist.javax.sip.stack.MessageChannel)
	 */
	@Override
	public void processResponse(SIPResponse sipResponse, MessageChannel incomingChannel) {
		...
		if (dialog == null) {
			if ((code > 100 && code < 300)
					...
					&& (sipResponse.getToTag() != null || sipStack.isRfc2543Supported()) && SIPTransactionStack.isDialogCreated(method)) {

				/*
				 * Dialog cannot be found for the response. This must be a forked response. no dialog assigned to this response but a default dialog has been assigned. Note that if automatic dialog support is configured then a default dialog is always created.
				 */

				synchronized (this) {
					/*
					 * We need synchronization here because two responses may compete for the default dialog simultaneously
					 */
					if (defaultDialog != null) {
						if (sipResponse.getFromTag() != null) {
							String defaultDialogId = defaultDialog.getDialogId();
							if (defaultDialog.getLastResponseMethod() == null || (method.equals(Request.SUBSCRIBE) && defaultDialog.getLastResponseMethod().equals(Request.NOTIFY) && defaultDialogId.equals(dialogId))) {
								// The default dialog has not been claimed yet.
								defaultDialog.setLastResponse(this, sipResponse);
								dialog = defaultDialog;
							} else {
								/*
								 * check if we have created one previously (happens in the case of REINVITE processing. JvB: should not happen, this.defaultDialog should then get set in Dialog#sendRequest line 1662
								 */
								...
								if (dialog == null) {
									if (defaultDialog.isAssigned()) {
										/*
										 * Nop we dont have one. so go ahead and allocate a new one.
										 */
	...
	public Dialog getDialog() {
		...
		if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
			logger.logDebug(" sipDialogs =  " + sipDialogs + " default dialog " + this.getDefaultDialog() + " retval " + retval);
		}
		return retval;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see gov.nist.javax.sip.stack.SIPTransaction#setDialog(gov.nist.javax.sip.stack.SIPDialog, gov.nist.javax.sip.message.SIPMessage)
	 */
	/**
	 ...
	 */
	/**
	 * @see gov.nist.javax.sip.stack.SIPClientTransaction#setDialog(gov.nist.javax.sip.stack.SIPDialog, java.lang.String)
	 */
	@Override
