...
public class IOHandler {
	...
	public Socket sendBytes(InetAddress senderAddress, InetAddress receiverAddress, int contactPort, String transport, byte[] bytes, boolean isClient, MessageChannel messageChannel) throws IOException {
		...
		if (logger.isLoggingEnabled(LogLevels.TRACE_INFO) && sipStack.isLogStackTraceOnMessageSend()) {
	...
	/*
	 * private void enterIOCriticalSection(String key) throws IOException { try { if ( ! this.ioSemaphore.tryAcquire(10,TimeUnit.SECONDS) ) { throw new IOException("Could not acquire semaphore"); } } catch (InterruptedException e) { throw new IOException("exception in acquiring sem"); } }
	 *
	 *
	 * private void leaveIOCriticalSection(String key) { this.ioSemaphore.release(); }
	 */
	...
	public void closeAll() {
		if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
			logger.logDebug("Closing " + socketTable.size() + " sockets from IOHandler");
