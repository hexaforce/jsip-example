 ...
 * <ul>
 * <li>If the request contains one or more Route headers, use the URI of the topmost Route header as next hop, possibly modifying the request in the process if the topmost Route header contains no lr parameter(*)
 ...
 *
 *
 * @version 1.2 $Revision: 1.20 $ $Date: 2010-12-02 22:04:12 $
 *
...
public class DefaultRouter implements Router {
	...
	/**
	 * Return addresses for default proxy to forward the request to. The list is organized in the following priority. If the requestURI refers directly to a host, the host and port information are extracted from it and made the next hop on the list. If the default route has been specified, then it is used to construct the next element of the list. <code>
	...
	public Hop getNextHop(Request request) throws SipException {
		...
		/*
		 * In case the topmost Route header contains no 'lr' parameter (which means the next hop is a strict router), the implementation will perform 'Route Information Postprocessing' as described in RFC3261 section 16.6 step 6 (also known as "Route header popping"). That is, the following modifications will be made to the request:
		 *
		...
		if (routes != null) {
			...
			if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
				logger.logDebug("Using outbound proxy to route the request = " + defaultRoute.toString());
			return defaultRoute;
	 ...
	 *
	 * pre: top route header in request has no 'lr' parameter in URI post: request-URI added as last route header, new req-URI = top-route-URI
	 */
	 ...
	 * Get the default hop.
	 *
	 * @return defaultRoute is the default route. public java.util.Iterator getDefaultRoute(Request request) { return this.getNextHops((SIPRequest)request); }
	 */
