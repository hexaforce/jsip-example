...
//	}
	...
	class ProcessorTask implements Runnable {
		...
		public void connect(SelectionKey selectionKey) throws IOException {
			 ...
			 * SocketChannel socketChannel = (SocketChannel) selectionKey.channel();
			 * 
			 * try { socketChannel.finishConnect(); } catch (IOException e) { selectionKey.cancel(); logger.logError("Cant connect", e); return; } synchronized (socketChannel) { logger.logDebug("Notifying to wake up the blocking connect"); socketChannel.notify(); }
			 * 
			 * // Register an interest in writing on this channel selectionKey.interestOps(SelectionKey.OP_WRITE);
			 */
		}
	...
	public MessageChannel createMessageChannel(HostPort targetHostPort) throws IOException {
		...
		try {
			...
			if (messageChannels.get(key) != null) {
				...
				NioTcpMessageChannel retval = new NioTcpMessageChannel(targetHostPort.getInetAddress(), targetHostPort.getPort(), sipStack, this);
