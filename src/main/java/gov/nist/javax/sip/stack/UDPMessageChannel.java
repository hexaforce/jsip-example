...
public class UDPMessageChannel extends MessageChannel implements ParseExceptionListener, Runnable, RawMessageChannel {
	...
	/*
	 * A table that keeps track of when the last pingback was sent to a given remote IP address and port. This is for NAT compensation. This stays in the table for 1 seconds and prevents infinite loop. If a second pingback happens in that period of time, it will be dropped.
	 */
	...
	protected UDPMessageChannel(InetAddress targetAddr, int port, SIPTransactionStack sipStack, UDPMessageProcessor messageProcessor) {
		...
		if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
			this.logger.logDebug("Creating message channel " + targetAddr.getHostAddress() + "/" + port);
		}
	}
	...
	public void run() {
		...
		while (true) {
			...
			if (sipStack.threadPoolSize != -1) {
				...
				try {
					...
					if (sipStack.getThreadAuditor().isEnabled()) {
						work = udpMessageProcessor.messageQueue.poll(threadHandle.getPingIntervalInMillisecs(), TimeUnit.MILLISECONDS);
					} else {
			...
			try {
				...
				logger.logError("Error while processing incoming UDP packet" + Arrays.toString(packet.getData()), e);
			}
	...
	private void processIncomingDataPacket(DatagramPacket packet) throws Exception {
		...
		// Do debug logging.
		if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
			this.logger.logDebug("UDPMessageChannel: processIncomingDataPacket : peerAddress = " + peerAddress.getHostAddress() + "/" + packet.getPort() + " Length = " + packetLength);
		...
		try {
			...
			if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
				this.logger.logDebug("Rejecting message !  " + new String(msgBytes));
				this.logger.logDebug("error message " + ex.getMessage());
		...
		if (sipMessage == null) {
			if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
				this.logger.logDebug("Rejecting message !  + Null message parsed.");
			}
		...
		if (sipMessage.getFrom() == null || sipMessage.getTo() == null || sipMessage.getCallId() == null || sipMessage.getCSeq() == null || topMostVia == null) {
			...
			if (logger.isLoggingEnabled()) {
				this.logger.logError("bad message " + badmsg);
				this.logger.logError(">>> Dropped Bad Msg " + "From = " + sipMessage.getFrom() + "To = " + sipMessage.getTo() + "CallId = " + sipMessage.getCallId() + "CSeq = " + sipMessage.getCSeq() + "Via = " + sipMessage.getViaHeaders());
			}
			return;
		}
	...
	public void processMessage(SIPMessage sipMessage) {
		...
		if (sipMessage instanceof SIPRequest) {
			...
			final ServerRequestInterface sipServerRequest = sipStack.newSIPServerRequest(sipRequest, this);
			...
			if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
				this.logger.logDebug("About to process " + sipRequest.getFirstLine() + "/" + sipServerRequest);
			try {
			...
			if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
				this.logger.logDebug("Done processing " + sipRequest.getFirstLine() + "/" + sipServerRequest);
			...
			if (sipServerResponse != null) {
				...
				if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
					this.logger.logDebug("null sipServerResponse!");
				}
			}
	...
	/**
	 * JvB: added method to check for known buggy clients (Windows Messenger) to fix the port to which responses are sent
	 *
	 * checks for User-Agent: RTC/1.3.5470 (Messenger 5.1.0701)
	 *
	 * JvB 22/7/2006 better to take this out for the moment, it is only a problem in rare cases (unregister)
	 *
	 * private final boolean isBuggyClient( SIPRequest r ) { UserAgent uah = (UserAgent) r.getHeader( UserAgent.NAME ); if (uah!=null) { java.util.ListIterator i = uah.getProduct(); if (i.hasNext()) { String p = (String) uah.getProduct().next(); return p.startsWith( "RTC" ); } } return false; }
	 */
	...
	public void sendMessage(final SIPMessage sipMessage) throws IOException {
		if (logger.isLoggingEnabled(LogLevels.TRACE_INFO) && this.sipStack.isLogStackTraceOnMessageSend()) {
			if (sipMessage instanceof SIPRequest && ((SIPRequest) sipMessage).getRequestLine() != null) {
				/*
				 * We dont want to log empty trace messages.
				 */
				this.logger.logStackTrace(StackLogger.TRACE_INFO);
			} else {
				this.logger.logStackTrace(StackLogger.TRACE_INFO);
			}
		}
		...
		try {
			...
			if (logger.isLoggingEnabled(ServerLogger.TRACE_MESSAGES) && !sipMessage.isNullRequest())
				logMessage(sipMessage, peerAddress, peerPort, time);
			else if (logger.isLoggingEnabled(ServerLogger.TRACE_DEBUG))
	...
	protected void sendMessage(byte[] msg, InetAddress peerAddress, int peerPort, boolean reConnect) throws IOException {
		...
		if (peerPort == -1) {
			if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
				this.logger.logDebug(getClass().getName() + ":sendMessage: Dropping reply!");
			}
			throw new IOException("Receiver port not set ");
		} else {
			if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
				this.logger.logDebug("sendMessage " + peerAddress.getHostAddress() + "/" + peerPort + "\n" + "messageSize =  " + msg.length + " message = " + new String(msg));
