...
public abstract class MessageChannel {
	...
	public static String getKey(HostPort hostPort, String transport) {
		...
		return (transport + ":" + ipAddress.replaceAll("[\\[\\]]", "") + ":" + hostPort.getPort()).toLowerCase();
	}
	...
	public void logMessage(SIPMessage sipMessage, InetAddress address, int port, long time) {
		...
		if (port == -1)
			port = 5060;
		getSIPStack().serverLogger.logMessage(sipMessage, this.getHost() + ":" + this.getPort(), address.getHostAddress().toString() + ":" + port, true, time);
	}
