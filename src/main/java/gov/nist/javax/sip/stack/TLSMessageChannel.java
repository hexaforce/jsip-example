...
public class TLSMessageChannel extends ConnectionOrientedMessageChannel {
	 ...
	 *
	 * @param sock         Socket from which to read and write messages. The socket is already connected (was created as a result of an accept).
	 *
	...
	protected synchronized void sendMessage(byte[] msg, boolean retry) throws IOException {
		...
		if (sock == null) { // http://java.net/jira/browse/JSIP-362 If we couldn't connect to the host, try the advertised host:port as failsafe
			if (peerAddressAdvertisedInHeaders != null && peerPortAdvertisedInHeaders > 0) {
				...
				sock = this.sipStack.ioHandler.sendBytes(this.messageProcessor.getIpAddress(), address, this.peerPortAdvertisedInHeaders, this.peerProtocol, msg, retry, this);
		...
		if (sock != mySock && sock != null) {
			...
			if (problem == null) {
				if (mySockLocal != null) {
					if (logger.isLoggingEnabled(LogWriter.TRACE_WARN)) {
						logger.logWarning("There was no exception for the retry mechanism so creating a new thread based on the new socket for incoming " + key);
					}
				}
				...
				if (logger.isLoggingEnabled(LogWriter.TRACE_WARN)) {
					logger.logWarning("There was an exception for the retry mechanism so not creating a new thread based on the new socket for incoming " + key);
				}
				mySock = sock;
			}
		}
	...
	public synchronized void sendMessage(byte message[], InetAddress receiverAddress, int receiverPort, boolean retry) throws IOException {
		...
		if (sock == null) { // http://java.net/jira/browse/JSIP-362 If we couldn't connect to the host, try the advertised host:port as failsafe
			if (peerAddressAdvertisedInHeaders != null && peerPortAdvertisedInHeaders > 0) {
				...
				InetAddress address = InetAddress.getByName(peerAddressAdvertisedInHeaders);
				sock = this.sipStack.ioHandler.sendBytes(this.messageProcessor.getIpAddress(), address, this.peerPortAdvertisedInHeaders, "TLS", message, retry, this);
				this.peerPort = this.peerPortAdvertisedInHeaders;
				this.peerAddress = address;
		...
		if (mySock != null && mySock instanceof SSLSocket && !handshakeCompleted) {
			...
			try {
				...
				if (!getSIPStack().isSslRenegotiationEnabled()) {
					/*
					 * disable TLS re-negotiations to avoid DOS attack http://web.nvd.nist.gov/view/vuln/detail?vulnId=CVE-2011-1473 http://www.ietf.org/mail-archive/web/tls/current/msg07553.html http://192.9.162.55/javase/javaseforbusiness/docs/TLSReadme.html
					 */
