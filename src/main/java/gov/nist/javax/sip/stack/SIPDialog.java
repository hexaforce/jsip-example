...
public class SIPDialog implements javax.sip.Dialog, DialogExt {
	...
	public class ReInviteSender implements Runnable, Serializable {
		...
		public void run() {
			try {
				...
				/*
				 * If we had to wait for ACK then wait for the ACK to actually get to the other side. Wait for any ACK retransmissions to finish. Then send out the request. This is a hack in support of some UA that want re-INVITEs to be spaced out in time ( else they return a 400 error code ).
				 */
				try {
					if (timeToWait != 0) {
	...
	class DialogTimerTask extends SIPStackTimerTask implements Serializable {
		...
		public void runTask() {
			...
			/*
			 * Issue 106. Section 13.3.1.4 RFC 3261 The 2xx response is passed to the transport with an interval that starts at T1 seconds and doubles for each retransmission until it reaches T2 seconds If the server retransmits the 2xx response for 64T1 seconds without receiving an ACK, the dialog is confirmed, but the session SHOULD be terminated.
			 */
	...
	class DialogDeleteIfNoAckSentTask extends SIPStackTimerTask implements Serializable {
		...
		public void runTask() {
			if (SIPDialog.this.highestSequenceNumberAcknowledged < seqno) {
				/*
				 * Did not send ACK so we need to delete the dialog. B2BUA NOTE: we may want to send BYE to the Dialog at this point. Do we want to make this behavior tailorable?
				 */
				dialogDeleteIfNoAckSentTask = null;
				if (!SIPDialog.this.isBackToBackUserAgent) {
					if (logger.isLoggingEnabled())
						logger.logError("ACK Was not sent. killing dialog " + dialogId);
					...
					if (logger.isLoggingEnabled())
						logger.logError("ACK Was not sent. Sending BYE " + dialogId);
					if (((SipProviderImpl) sipProvider).getSipListener() instanceof SipListenerExt) {
						raiseErrorEvent(SIPDialogErrorEvent.DIALOG_ACK_NOT_SENT_TIMEOUT);
					} else {
	...
	/**
	 * Creates a new dialog based on a received NOTIFY. The dialog state is initialized appropriately. The NOTIFY differs in the From tag
	 * 
	 * Made this a separate method to clearly distinguish what's happening here - this is a non-trivial case
	 * 
	...
	private void raiseErrorEvent(int dialogTimeoutError, SIPClientTransaction clientTransaction) {
		...
		// dialog
		if (dialogTimeoutError != SIPDialogErrorEvent.DIALOG_ACK_NOT_SENT_TIMEOUT && dialogTimeoutError != SIPDialogErrorEvent.DIALOG_ACK_NOT_RECEIVED_TIMEOUT && dialogTimeoutError != SIPDialogErrorEvent.EARLY_STATE_TIMEOUT && dialogTimeoutError != SIPDialogErrorEvent.DIALOG_REINVITE_TIMEOUT) {
			delete();
		}
	...
	private synchronized RouteList getRouteList() {
		...
		if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
			...
			if (routeList != null)
				logger.logDebug("myRouteList = " + routeList.encode());
			logger.logDebug("----- ");
		}
		return retval;
	}
	 ...
	 * @param throwIOExceptionAsSipException - throws SipException if IOEx encountered. Otherwise, no exception is propagated.
	...
	void ackReceived(long cseqNumber) {
		...
		if (tr != null) {
			if (tr.getCSeq() == cseqNumber) {
				...
				if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
					logger.logDebug("ackReceived for " + ((SIPTransaction) tr).getMethod());
	...
	public void printDebugInfo() {
		if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
			...
			logger.logDebug("ackLine:" + this.getRemoteTag() + " " + ackLine);
		}
	}
	...
	public boolean isAckSeen() {
		...
		if (lastAckReceivedCSeqNumber == null && lastResponseStatusCode == Response.OK) {
			...
			return false;
		} else if (lastAckReceivedCSeqNumber == null && lastResponseStatusCode / 100 > 2) {
			if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
				logger.logDebug("SIPDialog::isAckSeen:" + this + "lastResponse statusCode " + lastResponseStatusCode);
			}
			return true;
		} else {
			if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
	 ...
	 * 
	 * @return an Iterator containing a list of route headers to be used for forwarding. Empty iterator is returned if route has not been established.
	 */
	...
	public boolean addTransaction(SIPTransaction transaction) {
		...
		if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
			logger.logDebug("Transaction Added " + this + myTag + "/" + hisTag);
	 ...
	 * 
	 * @param mytag is the tag to use in From headers client transactions that belong to this dialog and for generating To tags for Server transaction requests that belong to this dialog.
	 */
	 ...
	 * 
	 * @return <code>true</code> if this dialogue was established using a sips URI over TLS, and <code>false</code> otherwise.
	 */
	...
	private SIPRequest createRequest(String method, String topMostViaTransport) throws SipException {
		/*
		 * Check if the dialog is in the right state (RFC 3261 section 15). The caller's UA MAY send a BYE for either CONFIRMED or EARLY dialogs, and the callee's UA MAY send a BYE on CONFIRMED dialogs, but MUST NOT send a BYE on EARLY dialogs.
		 * 
		...
		/*
		 * Add a via header for the outbound request based on the transport of the message processor.
		 */
		...
		/*
		 * The default contact header is obtained from the provider. The application can override this.
		 * 
		...
		try {
			/*
			 * Guess of local sequence number - this is being re-set when the request is actually dispatched
			 */
	...
	public void sendRequest(ClientTransaction clientTransaction, boolean allowInterleaving) throws TransactionDoesNotExistException, SipException {
		...
		this.proxyAuthorizationHeader = (ProxyAuthorizationHeader) dialogRequest.getHeader(ProxyAuthorizationHeader.NAME);
		...
		if (this.getRemoteTag() != null && to.getTag() != null && !to.getTag().equals(this.getRemoteTag())) {
			if (logger.isLoggingEnabled())
				this.logger.logWarning("SIPDialog::sendRequest:To header tag mismatch expecting " + this.getRemoteTag());
		}
		/*
		...
		try {
			...
			if (messageChannel == null) {
				/*
				 * At this point the procedures of 8.1.2 and 12.2.1.1 of RFC3261 have been tried but the resulting next hop cannot be resolved (recall that the exception thrown is caught and ignored in SIPStack.createMessageChannel() so we end up here with a null messageChannel instead of the exception handler below). All else failing, try the outbound proxy in accordance with 8.1.2, in particular: This ensures that outbound proxies that do not add Record-Route header field values will drop out of the path of subsequent requests. It allows endpoints that cannot resolve the first Route URI to delegate that task to an outbound proxy.
				 * 
				 * if one considers the 'first Route URI' of a request constructed according to 12.2.1.1 to be the request URI when the route set is empty.
				 */
				if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
		...
		try {
			...
			/*
			 * Note that if the BYE is rejected then the Dialog should bo back to the ESTABLISHED state so we only set state after successful send.
			 */
	...
	protected void startTimer(SIPServerTransaction transaction) {
		...
		try {
			if (this.timerTask != null) {
				...
				if (sipStack.getTimer() != null && sipStack.getTimer().isStarted()) {
					sipStack.getTimer().scheduleWithFixedDelay(timerTask, transaction.getBaseTimerInterval(), transaction.getBaseTimerInterval());
				}
			}
		} finally {
			releaseTimerTaskSem();
	 ...
	 * 
	 * Note that for the case of forked requests, you can create multiple outgoing invites each with a different cseq and hence you need to supply the invite.
	 * 
	...
	public Request createAck(long cseqno) throws InvalidArgumentException, SipException {
		...
		if (lastInviteOkReceived < cseqno) {
			...
			throw new SipException("Dialog not yet established -- no OK response! lastInviteOkReceived=" + lastInviteOkReceived + " cseqno=" + cseqno);
		}
			...
			if (transaction == null || transaction instanceof ClientTransaction) {
				if (SIPTransactionStack.isDialogCreated(cseqMethod)) {
					...
					if (getState() == null && (statusCode / 100 == 1)) {
						...
						/*
						 * This case occurs for forked dialog responses. The To tag can change as a result of the forking. The remote target can also change as a result of the forking.
						 */
						...
						if (cseqMethod.equals(getMethod()) && (sipResponse.getToTag() != null || sipStack.rfc2543Supported) && (this.getState() != DialogState.CONFIRMED || (this.getState() == DialogState.CONFIRMED && cseqMethod.equals(Request.SUBSCRIBE) && this.pendingRouteUpdateOn202Response && sipResponse.getStatusCode() == Response.ACCEPTED))) {
							...
							/*
							 * Note: Subscribe NOTIFY processing. The route set is computed only after we get the 202 response but the NOTIFY may come in before we get the 202 response. So we need to update the route set after we see the 202 despite the fact that the dialog is in the CONFIRMED state. We do this only on the dialog forming SUBSCRIBE an not a resubscribe.
							 */

							if (cseqMethod.equals(Request.SUBSCRIBE) && sipResponse.getStatusCode() == Response.ACCEPTED && this.pendingRouteUpdateOn202Response) {
								setRemoteTag(sipResponse.getToTag());
								this.addRoute(sipResponse);
				...
				if (cseqMethod.equals(Request.BYE) && statusCode / 100 == 2 && this.isTerminatedOnBye()) {
					/*
					 * Only transition to terminated state when 200 OK is returned for the BYE. Other status codes just result in leaving the state in COMPLETED state.
					 */
					this.setState(SIPDialog.TERMINATED_STATE);
				} else {
					boolean doPutDialog = false;

					if (getLocalTag() == null && sipResponse.getTo().getTag() != null && SIPTransactionStack.isDialogCreated(cseqMethod) && cseqMethod.equals(getMethod())) {
					...
					if (statusCode / 100 != 2) {
						if (statusCode / 100 == 1) {
							...
							/*
							 * RFC 3265 chapter 3.1.4.1 "Non-200 class final responses indicate that no subscription or dialog has been created, and no subsequent NOTIFY message will be sent. All non-200 class" + responses (with the exception of "489", described herein) have the same meanings and handling as described in SIP"
							 */
						...
						/*
						 * JvB: RFC4235 says that when sending 2xx on UAS side, state should move to CONFIRMED
						 */
	...
	private void doTargetRefresh(SIPMessage sipMessage) {
		...
		/*
		 * INVITE is the target refresh for INVITE dialogs. SUBSCRIBE is the target refresh for subscribe dialogs from the client side. This modifies the remote target URI potentially
		 */
		if (contactList != null) {
	...
	public Response createReliableProvisionalResponse(int statusCode) throws InvalidArgumentException, SipException {
		...
		/*
		 * The provisional response to be sent reliably is constructed by the UAS core according to the procedures of Section 8.2.6 of RFC 3261. In addition, it MUST contain a Require header field containing the option tag 100rel, and MUST include an RSeq header field. The value of the header field for the first reliable provisional response in a transaction MUST be between 1 and 231 - 1. It is RECOMMENDED that it be chosen uniformly in this range. The RSeq numbering space is within a single transaction. This means that provisional responses for different requests MAY use the same values for the RSeq number.
		 */
		...
		/*
		 * Copy the record route headers from the request to the response ( Issue 160 ). Note that other 1xx headers do not get their Record Route headers copied over but reliable provisional responses do. See RFC 3262 Table 2.
		 */
	 ...
	 * 
	 * @param prackRequest
	 * @return true if this is the first time the tx has seen the prack ( and hence needs to be passed up to the TU)
	 */
	...
	public void sendReliableProvisionalResponse(Response relResponse) throws SipException {
		...
		/*
		 * put into the dialog table before sending the response so as to avoid race condition with PRACK
		 */
	 ...
	 * @return -- true if the ACK was successfully consumed by the Dialog and resulted in the dialog state being changed.
	 */
	public boolean handleAck(SIPServerTransaction ackTransaction) {
		...
		if (isAckSeen() && getRemoteSeqNumber() == ackTransaction.getCSeq()) {
			...
			if (lastResponseStatusCode != null && this.lastInviteResponseCode / 100 == 2 && lastInviteResponseCSeqNumber == ackTransaction.getCSeq()) {
				...
				/*
				 * This happens when the ACK is re-transmitted and arrives too late to be processed.
				 */
