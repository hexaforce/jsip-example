...
public class Shootist implements SipListener {
	...
	protected static final String usageString = "java " + "examples.multi.Shootist \n" + ">>>> is your class path set to the root?";
	...
	public void processResponse(ResponseEvent responseReceivedEvent) {
		...
		try {
			if (response.getStatusCode() == Response.OK && ((CSeqHeader) response.getHeader(CSeqHeader.NAME)).getMethod().equals(Request.INVITE)) {
				...
				System.out.println("Dialog Confirmed: Count = " + ++this.confirmationCount + " dialogID = " + dialog.getDialogId() + " dialogState = " + dialog.getState());
				if (tid == null)
	...
	public void init() {
		...
		try {
			...
			/* udp ListeningPoint */
			udpListeningPoint = sipStack.createListeningPoint(localHost, 5060, "udp");
			udpProvider = sipStack.createSipProvider(udpListeningPoint);
			listener = this;
			udpProvider.addSipListener(listener);

			/* tcp ListeningPoint */
			tcpListeningPoint = sipStack.createListeningPoint(localHost, 5060, "tcp");
			tcpProvider = sipStack.createSipProvider(tcpListeningPoint);
			tcpProvider.addSipListener(listener);

			sipProvider = transport.equalsIgnoreCase("udp") ? udpProvider : tcpProvider;
	...
	void sendInviteRequest(int cmdSeq) {
		...
		try {
			...
			ViaHeader viaHeader = headerFactory.createViaHeader(localHost, sipProvider.getListeningPoint(transport).getPort(), transport, null);
	...
	public static void main(String args[]) throws Exception {
		...
		System.out.println("\n***localHost=<" + localHost + ">, peerHostPort=<" + peerHostPort + ">.");
	...
	public void processDialogTerminated(DialogTerminatedEvent dialogTerminatedEvent) {
		...
		System.out.println("DialogTerminatedEvent notification " + this.terminatedCount + " dialog ID = " + dialogTerminatedEvent.getDialog().getDialogId());
	}
}
