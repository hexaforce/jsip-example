...
public class Shootist extends TestCase implements SipListener {
	...
	class TestTimer extends TimerTask {
		...
		public void run() {
			assertTrue("Missed BYE " + shootist.byeCount, shootist.byeCount >= NDIALOGS);
	...
	public void processBye(Request request, ServerTransaction serverTransactionId) {
		try {
			...
			if (ndialogs > MAXCONCURRENTINVITE)
				System.out.println("Concurrent invites = " + ndialogs);
			synchronized (this) {
				if (ndialogs < MAXCONCURRENTINVITE / 2)
					this.notify();
			}
	...
	public void sendInvite() {
		try {
			...
			ViaHeader viaHeader = headerFactory.createViaHeader(sipProvider.getListeningPoint(transport).getIPAddress(), port, transport, null);
