...
/**
 * This class is a Subscriber template. The subscriber handles upstream forking by a proxy server. The stack is set up to expect forking of this event type. The test checks that exactly two distinct dialogs are created.
 *
...
public class Subscriber implements SipListener {
	...
	static {
		try {
			...
			logger.addAppender(new FileAppender(new SimpleLayout(), "logs/subscriberoutputlog.txt"));
		} catch (Exception ex) {
		...
		try {
			...
			if (dialog != subscriberDialog) {
				if (forkedDialog == null) {
					...
					AbstractSubsnotifyTestCase.assertTrue("Dialog should be either the subscriber dialog ", forkedDialog == dialog);
				}
			}
			...
			if (state.equalsIgnoreCase(SubscriptionStateHeader.TERMINATED)) {
				...
				// usually
				AbstractSubsnotifyTestCase.assertTrue("State should be pending was " + state, state.equalsIgnoreCase(SubscriptionStateHeader.PENDING));
	...
	public SipProvider createProvider(int newPort) {
		try {
			...
			listeningPoint = sipStack.createListeningPoint("127.0.0.1", this.port, transport);
