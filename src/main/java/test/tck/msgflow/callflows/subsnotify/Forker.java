...
/**
 * This implements a simple forking proxy to test proper handling of multiple NOTIFYs. An initial SUBSCRIBE request (i.e. without to-tag) is forked two times to the same destination. Each response should have a different to-tag; this proxy only passes the first 2xx response through, and discards the second
 *
 * NOTIFYs should go directly to the Contact announced in the SUBSCRIBE, hence this proxy won't see them
 *
 *
...
public class Forker implements SipListener {
	...
	static {
		try {
			logger.addAppender(new FileAppender(new SimpleLayout(), "logs/forkeroutputlog.txt"));
		} catch (Exception ex) {
	...
	private void recordRoute(Message m, String uniqueId) throws ParseException, SipException {
		Address me = addressFactory.createAddress("<sip:127.0.0.1:" + port + ";id=" + uniqueId + '>');
		if (!nonRFC3261Proxy)
	...
	public void processSubscribe(RequestEvent re, ServerTransaction st) {
		...
		try {
			...
			if (to.getTag() == null) {
				...
				logger.info("forker: got a mid-dialog Subscribe, forwarding statelessly...");
	...
	private void doFork(Request orig, ServerTransaction st, int port) throws Exception {
		ViaHeader myVia = headerFactory.createViaHeader("127.0.0.1", sipProvider.getListeningPoint("udp").getPort(), "udp", null);
	...
	private void doForwardStateless(Request orig, ServerTransaction st) throws ParseException, InvalidArgumentException, SipException {
		...
		ViaHeader myVia = headerFactory.createViaHeader("127.0.0.1", port, "udp", top.getBranch());
		orig.addFirst(myVia);
	...
	public void processResponse(ResponseEvent responseReceivedEvent) {
		...
		logger.info("Response received with client transaction id " + ct + ": " + response.getStatusCode());
	...
	public SipProvider createProvider(int newPort) {
		...
		try {
			...
			ListeningPoint lp = sipStack.createListeningPoint("127.0.0.1", port, transport);
