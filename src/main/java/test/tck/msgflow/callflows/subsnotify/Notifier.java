...
public class Notifier implements SipListener {
	...
	static {
		try {
			...
			logger.addAppender(new FileAppender(new SimpleLayout(), "logs/notifieroutputlog.txt"));
		} catch (Exception ex) {
	...
	public void processSubscribe(RequestEvent requestEvent, ServerTransaction serverTransaction) {
		...
		try {
			...
			AbstractSubsnotifyTestCase.assertTrue("Dialog state after pending NOTIFY ", dialog.getState() == DialogState.CONFIRMED);
	...
	public void processResponse(ResponseEvent responseReceivedEvent) {
		...
		logger.info("Response received with client transaction id " + tid + " CSeq = " + response.getHeader(CSeqHeader.NAME) + " status code = " + response.getStatusCode());
	...
	public SipProvider createProvider(int newPort) {
		...
		try {
			...
			ListeningPoint lp = sipStack.createListeningPoint("127.0.0.1", this.port, transport);
