...
public class Referee implements SipListener {
	...
	static {
		try {
			...
			logger.addAppender(new FileAppender(new SimpleLayout(), "logs/refereeoutputlog.txt"));
		} catch (Exception ex) {
	...
	private void sendNotify(int code, String reason) throws SipException, ParseException {
		...
		logger.info("NOTIFY Branch ID " + ((ViaHeader) notifyRequest.getHeader(ViaHeader.NAME)).getParameter("branch"));
		logger.info("Dialog " + dialog);
	...
	public void processResponse(ResponseEvent responseReceivedEvent) {
		...
		if (tid != null) {
			logger.info("Response received with client transaction id " + tid + ":\n" + response.getStatusCode() + " cseq = " + response.getHeader(CSeqHeader.NAME) + " dialog " + tid.getDialog());
		} else {
			logger.info("Response received with client transaction id " + tid + ":\n" + response.getStatusCode() + " cseq = " + response.getHeader(CSeqHeader.NAME) + " dialog " + responseReceivedEvent.getDialog());
		}
	...
	public void sendInvite(ReferToHeader to) {
		...
		try {
			...
			ViaHeader viaHeader = headerFactory.createViaHeader("127.0.0.1", lp.getPort(), transport, null);
