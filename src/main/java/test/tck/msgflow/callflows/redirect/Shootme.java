...
public class Shootme extends TestHarness implements SipListener {
	...
	public void processResponse(ResponseEvent responseEvent) {
		if (((CSeqHeader) responseEvent.getResponse().getHeader(CSeqHeader.NAME)).getMethod().equals(Request.BYE))
			this.byeOkRecieved++;
		else {
	...
	public void processAck(RequestEvent requestEvent, ServerTransaction serverTransaction) {
		try {
			...
			logger.debug("Dialog State = " + dialog.getState() + " sending BYE ");
	...
	public void processInvite(RequestEvent requestEvent, ServerTransaction serverTransaction) {
		...
		try {
			...
			/**
			 * We distinguish here after the display header in the Request URI to manage the different tags
			 */
			Address address = protocolObjects.addressFactory.createAddress("Shootme <sip:" + myAddress + ":" + myPort + ";transport=" + protocolObjects.transport + ">");
			...
			if (((SipURI) (request.getRequestURI())).getParameter("redirection") == null) {
				Response moved = protocolObjects.messageFactory.createResponse(Response.MOVED_TEMPORARILY, request);
				...
				Response ringing = protocolObjects.messageFactory.createResponse(Response.RINGING, request);
				...
				assertEquals("server tx state should be proceeding", st.getState(), TransactionState.PROCEEDING);

				this.okResponse = protocolObjects.messageFactory.createResponse(Response.OK, request);
	...
	private void sendInviteOK() {
		try {
			...
			logger.info("shootme: Dialog state after response: " + okResponse.getStatusCode() + " " + inviteTid.getDialog().getState());
	...
	public SipProvider createProvider() throws Exception {
		ListeningPoint lp = protocolObjects.sipStack.createListeningPoint("127.0.0.1", myPort, protocolObjects.transport);
