...
public class Shootme implements SipListener {
	...
	public void processInvite(RequestEvent requestEvent, ServerTransaction st) {
		...
		try {
			...
			if (st == null) {
				...
				TlsTest.assertSame("Dialog mismatch ", st.getDialog(), this.dialog);
				TlsTest.assertTrue("Secure dialog", this.dialog.isSecure());
			}
	...
	public void processResponse(ResponseEvent responseReceivedEvent) {
		...
		try {
			if (response.getStatusCode() == Response.OK && ((CSeqHeader) response.getHeader(CSeqHeader.NAME)).getMethod().equals(Request.INVITE)) {
				...
				TlsTest.assertNotNull("INVITE 200 response should match a transaction", tid);
	...
	public void checkState() {
		...
		TlsTest.assertTrue(data.ackCount == 1);
		TlsTest.assertTrue(okRecieved);
	}
