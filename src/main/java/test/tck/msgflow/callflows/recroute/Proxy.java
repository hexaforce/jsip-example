...
public class Proxy extends TestHarness implements SipListener {
	...
	public void processRequest(RequestEvent requestEvent) {
		try {
			...
			if (request.getMethod().equals(Request.INVITE)) {
				...
				//
				ViaHeader viaHeader = protocolObjects.headerFactory.createViaHeader(host, port, protocolObjects.transport, null);
				...
				if (requestEvent.getServerTransaction() == null) {
					...
					logger.debug("Saw a retransmission? State = " + requestEvent.getServerTransaction().getState());
				}
			}
	...
	public void processResponse(ResponseEvent responseEvent) {
		try {
			...
			logger.debug("ClientTxID = " + responseEvent.getClientTransaction() + " client tx id " + ((ViaHeader) response.getHeader(ViaHeader.NAME)).getBranch() + " CSeq header = " + response.getHeader(CSeqHeader.NAME) + " status code = " + response.getStatusCode());
	...
	public SipProvider createSipProvider() {
		try {
			ListeningPoint listeningPoint = protocolObjects.sipStack.createListeningPoint(host, port, protocolObjects.transport);
