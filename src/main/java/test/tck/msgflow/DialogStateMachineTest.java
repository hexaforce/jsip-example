...
public class DialogStateMachineTest extends MessageFlowHarness {
	...
	public void testClientDialogStates() {
		try {
			...
			assertNull("A dialog passed into the " + dialog.getState() + " state before receiving any response!", dialog.getState());
			...
			assertEquals("The Dialog did not pass into the early state upon reception of a RINGING response", DialogState.EARLY, dialog.getState());

			// JvB: @todo Test that UPDATE (or PRACK) requests during early dialog
			// dont make dialog CONFIRMED
			/*
			 * Request update = null; try { eventCollector.collectRequestEvent(riSipProvider); update = dialog.createRequest( "UPDATE" ); update.setHeader(createTiContact()); dialog.sendRequest( tiSipProvider.getNewClientTransaction(update) ); } catch (Exception e) { throw new TckInternalError( "Failed to create and send an UPDATE request", e); } waitForMessage(); RequestEvent updateEvt = eventCollector.extractCollectedRequestEvent(); if (updateEvt == null || updateEvt.getRequest() == null) throw new TiUnexpectedError("The TI did not send the UPDATE request"); //The dialog should still be in its EARLY state. assertEquals( "The Dialog did not stay in the EARLY state upon reception of an UPDATE request", DialogState.EARLY, dialog.getState());
			 * 
			 * // finish the UPDATE riSipProvider.sendResponse( riMessageFactory.createResponse(200, updateEvt.getRequest()) );
			 */
			...
			Response ok = null;
			try {
				ok = riMessageFactory.createResponse(Response.OK, inviteReqEvt.getRequest());
			...
			assertEquals("The Dialog did not pass into the CONFIRMED state upon reception of an OK response", DialogState.CONFIRMED, dialog.getState());
			// Say bye and go COMPLETED
			Request bye = null;
			try {
				bye = dialog.createRequest(Request.BYE);
				ClientTransaction byeTran = tiSipProvider.getNewClientTransaction(bye);
			...
			waitForMessage();
			assertTrue("The dialog did not pass into a final ( TERMINATED) state after getting OK for a BYE.", DialogState.TERMINATED.equals(dialog.getState()));
		} catch (Throwable exc) {
			exc.printStackTrace();
	...
	public void testServerDialogStates() {
		try {
			...
			try {
				tran = tiSipProvider.getNewServerTransaction(inviteReqEvt.getRequest());
			} catch (Exception ex) {
			...
			// We should have a null state here
			assertNull("A dialog passed into the " + dialog.getState() + " state before sending any response!", dialog.getState());
			...
			Response ringing = null;
			try {
				ringing = tiMessageFactory.createResponse(Response.RINGING, inviteReqEvt.getRequest());
				// !discuss with Ranga
				((ToHeader) ringing.getHeader(ToHeader.NAME)).setTag(Integer.toString(hashCode()));
			...
			// The dialog should now be in its early state.
			assertEquals("The Dialog did not pass into the early state after sending a RINGING response", DialogState.EARLY, dialog.getState());
			...
			Response ok = null;
			try {
				ok = tiMessageFactory.createResponse(Response.OK, inviteReqEvt.getRequest());
			...
			try {
				ok = tiMessageFactory.createResponse(Response.OK, byeEvt.getRequest());
			...
			waitForMessage();
			ResponseEvent byeOkRespEvt = eventCollector.extractCollectedResponseEvent();
			...
			assertTrue("The dialog did not pass into a final (COMPLETED or TERMINATED) state after recieving a BYE.", DialogState.COMPLETED.equals(dialog.getState()) || DialogState.TERMINATED.equals(dialog.getState()));
		} catch (Throwable exc) {
			exc.printStackTrace();
			fail(exc.getClass().getName() + ": " + exc.getMessage());
		}
