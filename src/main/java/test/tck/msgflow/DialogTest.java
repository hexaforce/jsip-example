...
public class DialogTest extends MessageFlowHarness {
	...
	public void setUp() throws Exception {
		try {
			...
			waitForMessage();
			RequestEvent inviteReqEvt = eventCollector.extractCollectedRequestEvent();
			...
			try {
				ringing = riMessageFactory.createResponse(Response.RINGING, inviteReqEvt.getRequest());
				((ToHeader) ringing.getHeader(ToHeader.NAME)).setTag(riToTag = Integer.toString(hashCode()));
			...
			waitForMessage();
			ResponseEvent ringingRespEvt = eventCollector.extractCollectedResponseEvent();
	...
	public void testDialogProperties() {
		try {
			// CallId
			assertEquals("The Dialog did not have the right Call ID.", ((CallIdHeader) riInvite.getHeader(CallIdHeader.NAME)).getCallId(), dialog.getCallId().getCallId());
			// Tran
			/*
			 * Deprecated assertSame( "The Dialog.getTransaction did not return the right transaction.", cliTran, dialog.getFirstTransaction());
			 */
			// LocalParty
			assertEquals("Dialog.getLocalParty() returned a bad address.", ((FromHeader) tiInvite.getHeader(FromHeader.NAME)).getAddress(), dialog.getLocalParty());
			// SeqNum
			assertTrue("Dialog.getLocalSequenceNumber() returned a bad value.", 1 == dialog.getLocalSeqNumber());
			// LocalTag
			assertEquals("Dialog.getLocalTag() returned a bad tag", ((FromHeader) riInvite.getHeader(FromHeader.NAME)).getTag(), dialog.getLocalTag());
			// RemoteParty
			assertEquals("Dialog.getRemoteParty() returned a bad address.", ((ToHeader) tiInvite.getHeader(ToHeader.NAME)).getAddress(), dialog.getRemoteParty());
			// RemoteTag
			assertEquals("Dialog.getRemoteTag() returned a bad tag", ((ToHeader) ringing.getHeader(ToHeader.NAME)).getTag(), dialog.getRemoteTag());
			// is server
			assertFalse("Dialog.isServer returned true for a client side dialog", dialog.isServer());
		} catch (Throwable exc) {
			exc.printStackTrace();
			fail(exc.getClass().getName() + ": " + exc.getMessage());
		}
	...
	public void testCreateRequest() {
		try {
			...
			// check method
			assertEquals("Dialog.createRequest() returned a request with a bad method.", Request.BYE, bye.getMethod());
			// check CSeq number
			assertEquals("Dialog.createRequest() returned a request with a bad sequence number.", dialog.getLocalSeqNumber() + 1, ((CSeqHeader) bye.getHeader(CSeqHeader.NAME)).getSeqNumber());
			// Check From
			FromHeader byeFrom = (FromHeader) bye.getHeader(FromHeader.NAME);
			assertEquals("Dialog.createRequest() returned a request with a bad From header.", dialog.getLocalParty(), byeFrom.getAddress());
			// Check From tags
			assertEquals("Dialog.createRequest() returned a request with a bad From tag.", dialog.getLocalTag(), byeFrom.getTag());
			// Check To
			ToHeader byeTo = (ToHeader) bye.getHeader(ToHeader.NAME);
			assertEquals("Dialog.createRequest() returned a request with a bad To header.", dialog.getRemoteParty(), byeTo.getAddress());
			// Check To tags
			assertEquals("Dialog.createRequest() returned a request with a bad To tag.", dialog.getRemoteTag(), byeTo.getTag());
	 ...
	 * 
	 * public void testSendAckWithSameBranch() { this.doTestSendAck(true); }
	 */
	...
	private void doTestSendAck(boolean sameBranch) {
		...
		try {
			...
			try {
				...
				ok.addHeader(createRiInviteRequest(null, null, null).getHeader(ContactHeader.NAME));
			...
			waitForMessage();
			ResponseEvent okRespEvt = eventCollector.extractCollectedResponseEvent();
			if (okRespEvt == null || okRespEvt.getResponse() == null)
				throw new TiUnexpectedError("The TI did not dispatch an OK response.");

			String okBranch = ((ViaHeader) okRespEvt.getResponse().getHeader(ViaHeader.NAME)).getBranch();
			...
			assertNotNull("No requestEvent sent by Dialog.sendAck() was received by the RI", ackEvt);
			assertNotNull("The request sent by Dialog.sendAck() was not received by the RI", ackEvt.getRequest());
			if (!sameBranch) {
				String ackBranchId = ((ViaHeader) ackEvt.getRequest().getHeader(ViaHeader.NAME)).getBranch();
				super.assertNotSame("ACK branch ID must differ from INVITE OK branch ID", ackBranchId, okBranch);
			}
		} catch (Throwable exc) {
			exc.printStackTrace();
			fail(exc.getClass().getName() + ": " + exc.getMessage());
	...
	public void testSendRequest() {
		try {
			...
			waitForMessage();
			// Did they get it?
			RequestEvent cancelEvt = eventCollector.extractCollectedRequestEvent();
			assertNotNull("The RI did not receive the sent request", cancelEvt);
			assertNotNull("The RI did not receive the sent request", cancelEvt.getRequest());
		} catch (Throwable exc) {
			exc.printStackTrace();
			fail(exc.getClass().getName() + ": " + exc.getMessage());
		}
