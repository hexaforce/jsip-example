...
public class ServerTransactionTest extends MessageFlowHarness {
	...
	public void testCancel() {
		try {
			...
			ServerTransaction st = tiSipProvider.getNewServerTransaction(inviteReceivedEvent.getRequest());
			...
			try {
				Response resp = tiMessageFactory.createResponse(Response.TRYING, inviteReceivedEvent.getRequest());
			...
			assertEquals("Must see 100 trying response", responseEvent.getResponse().getStatusCode(), 100);
			...
			assertNotNull("Cancel dialog must be non null", cancelReceivedEvent.getDialog() != null);
			assertSame("Cancel dialog must match Invite", cancelReceivedEvent.getDialog(), dialog);
			try {
				eventCollector.collectResponseEvent(riSipProvider);
			} catch (TooManyListenersException ex) {
				throw new TiUnexpectedError("Failed to register a SipListener with TI", ex);
			...
			try {
				tiCancelOk = tiMessageFactory.createResponse(Response.OK, cancelReceivedEvent.getRequest());
			...
			assertTrue("Cancel tx must not be the same as the invite Tx", cancelTx != st);

			assertTrue("Method must be cancel ", cancelTx.getRequest().getMethod().equals(Request.CANCEL));

			assertTrue("Branch must match invite branch", ((ViaHeader) cancelTx.getRequest().getHeader(ViaHeader.NAME)).getBranch().equals(((ViaHeader) inviteRequest.getHeader(ViaHeader.NAME)).getBranch()));
			...
			assertEquals("Must see a 200", eventCollector.extractCollectedResponseEvent().getResponse().getStatusCode(), 200);
			...
			try {
				tiInviteTerminated = tiMessageFactory.createResponse(Response.REQUEST_TERMINATED, inviteReceivedEvent.getRequest());
			...
			waitForMessage();
			assertEquals("Must see a 487", eventCollector.extractCollectedResponseEvent().getResponse().getStatusCode(), 487);
			...
			assertNull("The TI MUST NOT see the ACK", ackReceivedEvent);
