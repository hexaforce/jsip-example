 ...
 * </p>
 *
 * @author Emil Ivov Network Research Team, Louis Pasteur University, Strasbourg, France
...
public class MessageFlowHarness extends TestHarness {
	...
	protected void addStatus(HeaderFactory headerFactory, Request request) {
		try {
			Header extension = headerFactory.createHeader(EXTENSION_HDR, Integer.valueOf(counter++).toString());
	 ...
	 *
	 * @throws java.lang.Exception All Let all exceptions that come from the underlying stack to pass through and surface at JUnit Level.
	 */
	public void setUp() throws java.lang.Exception {

		riListeningPoint = riSipStack.createListeningPoint(LOCAL_ADDRESS, RI_PORT, "udp");
		riSipProvider = riSipStack.createSipProvider(riListeningPoint);

		tiListeningPoint = tiSipStack.createListeningPoint(LOCAL_ADDRESS, TI_PORT, "udp");
	 ...
	 * @param srcProvider    the provider that will eventually be used to send the request
	 * @param dstProvider    the provider that will eventually dispatch the request to a SipListener
	 * @param contentType    if the content parameter is not null then this is its content type.
	 * @param contentSubType if the content parameter is not null then this is its sub content type.
	 * @param content        the content of the request. if null this parameter is ignored
	 * @return a request generated by the specified factories and destined to go from srcProvider to dstProvider
	 * @throws Exception if anything should go wrong. further exception handling is left to calling methods (or JUnit).
	 */
	protected Request createRequest(String method, AddressFactory addressFactory, HeaderFactory headerFactory, MessageFactory messageFactory, SipProvider srcProvider, SipProvider dstProvider, String contentType, String contentSubType, Object content) throws Exception {
		...
		ViaHeader viaHeader = headerFactory.createViaHeader(srclp.getIPAddress(), srclp.getPort(), srclp.getTransport(),
		...
		Request request = messageFactory.createRequest(dstSipURI, method, callId, cSeq, from, to, via, maxForwards);
	 ...
	 *
	 * @param contentType    if the content parameter is not null then this is its content type.
	 * @param contentSubType if the content parameter is not null then this is its content sub type.
	 * @param content        if the request is to have any content then this parameter is used to set it. Th content parameter is to be left to null if the request won't have any content.
	...
	protected Request createTiInviteRequest(String contentType, String contentSubType, Object content) throws TiUnexpectedError {
		try {
			return createRequest(Request.INVITE, tiAddressFactory, tiHeaderFactory, tiMessageFactory, tiSipProvider, riSipProvider, contentType, contentSubType, content);
		} catch (Throwable exc) {
	...
	protected Request createRiRegisterRequest() throws TckInternalError {
		try {
			return createRequest(Request.REGISTER, riAddressFactory, riHeaderFactory, riMessageFactory, riSipProvider, tiSipProvider, null, null, null);
		} catch (Throwable exc) {
	...
	protected Request createTiRegisterRequest() throws TiUnexpectedError {
		try {
			return createRequest(Request.REGISTER, tiAddressFactory, tiHeaderFactory, tiMessageFactory, tiSipProvider, riSipProvider, null, null, null);
	...
	public ContactHeader createTiContact() throws Exception {
		try {
			...
			if (ip == null) {
				ListeningPoint lp = (ListeningPoint) tiSipProvider.getSipStack().getListeningPoints().next();
				ip = lp.getIPAddress();
			}
