...
class SipEventCollector {
	 ...
	 * @throws TooManyListenersException in case the specified provider throws that exception. It is the responsibility of the calling class to encapsulate it with the corresponding error (e.g. TckInternalError, AssertFailure or TiUnexpectedError) and throw it in its turn to upper layers.
	 */
	 ...
	 * @throws TckInternalError in case the method is called without first calling the collectRequestEvent
	 */
	 ...
	 * @throws TckInternalError in case the method is called without first calling the collectResponseEvent
	 */
	 ...
	 * @throws TckInternalError in case the method is called without first calling the collectTimeoutEvent
	 */
	...
	private void resetCollectors() {
		 ...
		 * sipProvider = null;
		 *
		 * requestCollector = null; responseCollector = null; timeoutCollector = null; dialogTerminationCollector = null; transactionTerminationCollector = null;
		 */
	...
	private void assertInitialState(Class eventClass) {
		...
		if (eventClass.equals(RequestEvent.class)) {
			if (requestCollector != null && requestCollector.collectedEvent != null) {
				failureFlag = true;
			}
		} else if (eventClass.equals(ResponseEvent.class)) {
			if (responseCollector != null && responseCollector.collectedEvent != null) {

				failureFlag = true;
			}
		} else if (eventClass.equals(TimeoutEvent.class)) {
			if (timeoutCollector != null && timeoutCollector.collectedEvent != null) {
				failureFlag = true;
			}
			...
			if (this.dialogTerminationCollector != null && this.dialogTerminationCollector.collectedEvent != null)
				failureFlag = true;
		}
