...
//                    .getResourceAsStream("tck.properties"));
			...
			if (lf != null)
				...
				logger.addAppender(new FileAppender(new SimpleLayout(), "tckoutput.txt"));
		} catch (Exception ex) {
	...
	private static void logSuccess(String message) {
		...
		logger.info(className + ":" + frame.getMethodName() + "(" + frame.getFileName() + ":" + frame.getLineNumber() + ")" + " : Status =  passed ! ");
	...
	private static void logFailure(String className, String methodName, String reason) {
		println(" Test in function " + className + ":" + methodName + " failed because of " + reason);
	...
	public static void checkImplementsInterface(Class implementationClass, Class jainInterface) {
		...
		assertTrue(jainInterface.toString() + " is_assignable_from " + implementationClass.toString(), jainInterface.isAssignableFrom(implementationClass));
	}
	...
	/**
	 * Returns a properties object containing all RI settings. The result from this method is passed to the SipFactory when creating the RI Stack
	 */
...
//        properties.setProperty("gov.nist.javax.sip.TRACE_LEVEL", "0");
	...
	/**
	 * Returns a properties object containing all TI settings. The result from this method is passed to the SipFactory when creating the TI Stack
	 *
	 *
	 */
