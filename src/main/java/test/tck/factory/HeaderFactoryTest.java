...
/**
 * Generate header test cases based on RI. This assumes the correctness of the TCK and generates a set of tests for a reference implementation based on the TCK. It does this by introspection.
 */
...
public class HeaderFactoryTest extends FactoryTestHarness {
	...
	/*
	 * JvB: This is using gov.nist specific classes that are not in the RI and is not being used at the moment
	 *
	 *
	 * private static void testParametersHeader( Header referenceHeader, Header headerToTest) { Iterator it = ((ParametersHeader) referenceHeader).getParameterNames(); //logger.info(referenceHeader.getClass()); while (it.hasNext()) { String name = (String) it.next(); String value = ((Parameters) headerToTest).getParameter(name); String referenceValue = ((Parameters) referenceHeader).getParameter(name); NameValue nv = ((ParametersHeader) referenceHeader).getNameValue(name); Object val = nv.getValue(); if (val != null && val.getClass().equals(String.class)) { if (nv.isValueQuoted()) { assertTrue(value.equals(referenceValue)); } else { assertTrue(value.equalsIgnoreCase((String) referenceValue)); } } else if (val != null) { assertTrue(value.equals(referenceValue)); } else { assertTrue(value == null); } } }
	 */
	...
	public void testHeaderFactory() {
		...
		try {
			for (int i = 0; i < headers.length; i++) {
				...
				Header headerToTest = tiHeaderFactory.createHeader(name.toString().trim(), body.toString().trim());
