...
public class AddressFactoryTest extends FactoryTestHarness {
	...
	protected javax.sip.address.SipURI createTiSipURI(String name, String address) {
		SipURI sipUri = null;
		try {
	...
	protected javax.sip.address.SipURI createRiSipURI(String name, String address) {
		SipURI sipUri = null;
		try {
	...
	protected javax.sip.address.TelURL createRiTelURLFromTiTelURL(TelURL tiTelURL) {
		TelURL telUrl = null;
		try {
	...
	protected javax.sip.address.Address createAddress(javax.sip.address.URI uri) {
		javax.sip.address.Address addr = null;
		try {
			...
			assertTrue(addr.equals(tiAddressFactory.createAddress(uri.toString())));
		} catch (Exception ex) {
			throw new TiUnexpectedError(ex.getMessage());
		}
		return addr;
	...
	protected javax.sip.address.Address createRiAddress(javax.sip.address.URI uri) {
		javax.sip.address.Address addr = null;
		try {
	...
	protected javax.sip.address.Address createRiAddressFromTiAddress(Address tiAddress) throws TiUnexpectedError {
		try {
	...
	public void testAddressFactory() {
		try {
			...
			for (int i = 0; i < urls.length; i++) {
				...
				javax.sip.address.Address riAddress = this.createRiAddress(riUri);
				tiAddress = this.createRiAddress(tiAddress.toString());

				assertEquals(riAddress, this.createRiAddressFromTiAddress(tiAddress));
			...
			for (int i = 0; i < hosts.length; i++) {
				...
				assertTrue(((SipURI) tiAddressFactory.createURI("sip:" + hosts[i])).equals(tiSipURI));
				riSipURI = this.createRiSipURI(null, hosts[i]);
				javax.sip.address.Address tiAddress = this.createAddress(tiSipURI);
				assertTrue(tiAddress != null);
				javax.sip.address.Address riAddress = this.createRiAddress(riSipURI);
				assertTrue(riAddress.equals(this.createRiAddressFromTiAddress(tiAddress)));

				tiSipURI = this.createTiSipURI("jaintck", hosts[i]);
				assertTrue(tiSipURI != null);
				assertTrue(tiSipURI.isSipURI());
				assertTrue(!tiSipURI.isSecure());
				assertTrue(((SipURI) tiAddressFactory.createURI("sip:jaintck@" + hosts[i])).equals(tiSipURI));
			...
			for (int i = 0; i < phoneNumbers.length; i++) {
				javax.sip.address.TelURL tiTelUrl = this.createTiTelURL(phoneNumbers[i]);
				assertTrue(tiTelUrl != null);
				javax.sip.address.TelURL riTelUrl = this.createRiTelURL(phoneNumbers[i]);

				System.out.println("TI:" + tiTelUrl);
				System.out.println("RI:" + riTelUrl);

				assertEquals(riTelUrl, createRiTelURLFromTiTelURL(tiTelUrl));
			}

			for (int i = 0; i < telUrls.length; i++) {
				javax.sip.address.TelURL telUrl = (TelURL) this.createTiURI(telUrls[i]);
