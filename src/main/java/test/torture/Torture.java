...
public class Torture extends DefaultHandler implements ParseExceptionListener, TagNames, TokenValues, ConfigurationSwitches {
	...
	public void handleException(ParseException ex, SIPMessage sipMessage, Class headerClass, String headerText, String messageText) throws ParseException {
		...
		if (exceptionTable == null) {
			formatFailureDiagnostic(ex, "unexpected exception (no exceptions expected)");
			setFailure();
			if (debugFlag)
				throw ex;
		} else {
			ExpectedException e = (ExpectedException) exceptionTable.get(headerText.trim());
			if (e == null) {
			...
			if (e == null) {
				formatFailureDiagnostic(ex, "unexpected exception " + ex.getMessage());
				setFailure();
				if (debugFlag)
					throw ex;
			} else if (!ex.getClass().equals(e.exceptionClass)) {
				setFailure();
				formatFailureDiagnostic(ex, " got " + ex.getClass() + " expecting " + e.exceptionClass.getName());
				if (debugFlag)
					throw ex;
			} else {
	...
	public void startElement(String namespaceURI, String local, String name, Attributes attrs) throws SAXException {
		if (name.compareTo(TORTURE) == 0) {
			...
			if (outputParsedString != null && outputParsedString.toLowerCase().compareTo(TRUE) == 0) {
				outputParsedStructures = true;
			}
			String debugString = attrs.getValue(DEBUG);
			if (debugString != null && debugString.toLowerCase().compareTo(TRUE) == 0) {
				debugFlag = true;
			}
			...
			if (outputResultsString != null && outputResultsString.compareTo(TRUE) == 0) {
				outputResults = true;
			} else
				outputResults = false;
			...
			if (outputResults) {
				...
				if (testlog != null) {
					try {
						if (outputResults)
							testLogWriter = new TestLogWriter(new FileOutputStream(testlog, true));
...
//            stringParser.setParseExceptionListener(this);
			...
			while (e.hasMoreElements()) {
				...
				if (!ex.fired) {
					formatFailureDiagnostic(ex, "Expected Exception not trapped ");
				} else {
			...
			ExpectedException ex = new ExpectedException(exceptionClassName, exceptionHeader);
			if (exceptionHeader != null) {
	...
	protected String emitStringCData(String tag, String msg) {
		return emitStringTag(tag) + emitStringCData(msg) + emitStringEndTag(tag);
	}
	...
	protected void formatFailureDiagnostic(ParseException ex, String message) {
		...
		failureReason += emitStringAttribute(EXCEPTION_CLASS, ex.getClass().getName());
	...
	protected void formatFailureDiagnostic(ExpectedException ex, String reason) {
		...
		failureReason += emitStringAttribute(STATUS, "BAD");
		failureReason += emitStringAttribute(REASON, reason);
		failureReason += emitStringAttribute(EXCEPTION_CLASS, ex.exceptionClass.getName());
		failureReason += emitStringEndTag();
	...
	protected void formatStatusMessage(ExpectedException ex, String reason) {
		...
		statusMessage += emitStringAttribute(STATUS, "OK");
		statusMessage += emitStringAttribute(REASON, reason);
		statusMessage += emitStringAttribute(EXCEPTION_CLASS, ex.exceptionClass.getName());
		statusMessage += emitStringEndTag();
