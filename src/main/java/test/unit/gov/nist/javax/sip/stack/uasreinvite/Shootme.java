...
public class Shootme implements SipListener {
	...
	public void processAck(RequestEvent requestEvent, ServerTransaction serverTransaction) {
		...
		try {
			...
			logger.error(s, ex);
			ReInviteAllowInterleavingTest.fail(s);
		}
	}
	...
	public void processInvite(RequestEvent requestEvent, ServerTransaction serverTransaction) {
		...
		try {
			...
			if (st == null) {
				...
				ReInviteAllowInterleavingTest.assertSame("Dialog mismatch ", st.getDialog(), this.dialog);
			}
	...
	public void processResponse(ResponseEvent responseReceivedEvent) {
		...
		try {
			if (response.getStatusCode() == Response.OK && ((CSeqHeader) response.getHeader(CSeqHeader.NAME)).getMethod().equals(Request.INVITE)) {
				...
				ReInviteAllowInterleavingTest.assertNotNull("INVITE 200 response should match a transaction", tid);
	...
	public void checkState() {
		...
		ReInviteAllowInterleavingTest.assertTrue(data.ackCount == 1);
		ReInviteAllowInterleavingTest.assertTrue(okRecieved);
	}
