...
public class Shootist implements SipListenerExt {
	...
	public void checkStateForDialogTimeoutEvent() {
		...
		TestCase.assertEquals("Should see " + this.counter + " distinct dialogs", 2, this.forkedDialogs.size());
		if (!createDialogAfterRequest) {
	...
	public void processDialogTimeout(DialogTimeoutEvent timeoutEvent) {
		...
		if (timedOutDialog.size() > 0) {
			TestCase.assertTrue("Should find timeout dialog in cancelSet", this.timedOutDialog.contains(timeoutEvent.getDialog()));
		} else {
			TestCase.assertTrue("Should find timeout dialog in cancelSet", this.forkedEarlyDialogs.contains(timeoutEvent.getDialog()));
