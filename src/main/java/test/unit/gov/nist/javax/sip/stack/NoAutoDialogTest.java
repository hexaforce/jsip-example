...
public class NoAutoDialogTest extends TestCase {
	...
	protected ViaHeader getLocalVia(SipProvider _provider) throws ParseException, InvalidArgumentException {
		return headerFactory.createViaHeader(_provider.getListeningPoint(testProtocol).getIPAddress(), _provider.getListeningPoint(testProtocol).getPort(), _provider.getListeningPoint(testProtocol).getTransport(), null);
		...
		public void processRequest(RequestEvent requestEvent) {
			...
			if (request.getMethod().equals(Request.INVITE)) {
				try {
					...
					this.inviteStx = provider.getNewServerTransaction(request);
					...
					((ToHeader) response.getHeader(ToHeader.NAME)).setTag("asdgaeyvewacyta" + Math.random());
					inviteStx.sendResponse(response);
				...
				try {
					...
					Response inviteResponse = messageFactory.createResponse(Response.REQUEST_TERMINATED, inviteRequest);
	...
	public class Client implements SipListener {
		...
		private boolean o_sentInvite, o_received180, o_sentCancel, o_receiver200Cancel, o_inviteTxTerm, o_dialogTerinated;
		...
		public void checkState() {
			if (!o_sentInvite || !o_received180 || !o_sentCancel || !o_receiver200Cancel || !o_inviteTxTerm || !o_dialogTerinated) {
				fail("FAILED o_sentInvite[" + o_sentInvite + "] o_received180[" + o_received180 + "] o_sentCancel[" + o_sentCancel + "] o_receiver200Cancel[" + o_receiver200Cancel + "] o_inviteTxTerm[" + o_inviteTxTerm + "] o_dialogTerinated[" + o_dialogTerinated + "]  ");
		...
		protected void sendLocalInvite() throws Exception {
			...
			inviteRequest.addHeader(headerFactory.createFromHeader(localAddress, generateFromTag()));
			...
			ClientTransaction inviteTransaction = this.provider.getNewClientTransaction(inviteRequest);
