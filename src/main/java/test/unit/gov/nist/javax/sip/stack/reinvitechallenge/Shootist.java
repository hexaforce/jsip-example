...
public class Shootist implements SipListener {
	...
	public void processInvite(Request request, ServerTransaction st) {
		try {
			if (this.reInviteCount == 0) {
				...
				Response response = protocolObjects.messageFactory.createResponse(Response.PROXY_AUTHENTICATION_REQUIRED, request);
				((ToHeader) response.getHeader(ToHeader.NAME)).setTag(((ToHeader) request.getHeader(ToHeader.NAME)).getTag());
				Address address = protocolObjects.addressFactory.createAddress("Shootme <sip:" + myAddress + ":" + myPort + ">");
				ContactHeader contactHeader = protocolObjects.headerFactory.createContactHeader(address);
				response.addHeader(contactHeader);
	...
	public void processResponse(ResponseEvent responseReceivedEvent) {
		...
		try {
			...
			} else if (response.getStatusCode() == Response.OK && ((CSeqHeader) response.getHeader(CSeqHeader.NAME)).getMethod().equals(Request.BYE)) {
				this.byeOkRecieved = true;
			} else if (response.getStatusCode() == Response.PROXY_AUTHENTICATION_REQUIRED) {

				AuthenticationHelper authenticationHelper = ((SipStackExt) protocolObjects.sipStack).getAuthenticationHelper(new AccountManagerImpl(), protocolObjects.headerFactory);
	...
	public void sendInvite() {
		...
		try {
			...
			CSeqHeader cSeqHeader = protocolObjects.headerFactory.createCSeqHeader(661081884L, Request.INVITE);
