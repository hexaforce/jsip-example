...
public class DeadSocketTlsTest extends TestCase {
	...
	/**
	 * This class is a UAC template. Shootist is the guy that shoots and shootme is the guy that gets shot. The BadShootist class will disconnect sockets every time it has a chance and it will help testing the socket reconnect capability for the stack. Unlike the other class this one doesnt check the certs because they are in undefined state after disconnect.
	 *
	...
	static public class BadShootist implements SipListener, TlsSecurityPolicy {
		...
		protected static final String usageString = "java " + "examples.shootistTLS.Shootist \n" + ">>>> is your class path set to the root?";
			...
			try {
				if (response.getStatusCode() == Response.OK && ((CSeqHeader) response.getHeader(CSeqHeader.NAME)).getMethod().equals(Request.INVITE)) {
					...
					if (reInviteCount == 0) {
						...
						ClientTransaction ct = tlsProvider.getNewClientTransaction(inviteRequest);
		...
		public void init() {
			...
			properties.setProperty("gov.nist.javax.sip.DEBUG_LOG", "logs/shootistdebug.txt");
			properties.setProperty("gov.nist.javax.sip.SERVER_LOG", "logs/shootistlog.txt");
			properties.setProperty("gov.nist.javax.sip.SSL_HANDSHAKE_TIMEOUT", "10000");
			...
			try {
				...
				tlsListeningPoint = sipStack.createListeningPoint("127.0.0.1", port, transport);
	...
	/**
	 * This class is a UAC template. Shootist is the guy that shoots and shootme is the guy that gets shot. The BadShootme class will disconnect sockets every time it has a chance and it will help testing the socket reconnect capability for the stack. Unlike the other class this one doesnt check the certs because they are in undefined state after disconnect.
	 *
	 * @author vladimir ralev
	 */
	...
	static public class BadShootme implements SipListener {
		...
		public void processInvite(RequestEvent requestEvent, ServerTransaction serverTransaction) {
			...
			try {
				...
				Address address = addressFactory.createAddress("Shootme <sip:" + myAddress + ":" + myPort + ";transport=tls>");
		...
		public void init() {
			...
			properties.setProperty("gov.nist.javax.sip.TRACE_LEVEL", "LOG4J");
			properties.setProperty("gov.nist.javax.sip.TCP_POST_PARSING_THREAD_POOL_SIZE", "20");
			properties.setProperty("gov.nist.javax.sip.DEBUG_LOG", "logs/shootmedebug.txt");
			properties.setProperty("gov.nist.javax.sip.SERVER_LOG", "logs/shootmelog.txt");
