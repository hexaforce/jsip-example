...
public class WebsocketSelfTest extends TestCase {
	...
	public class WebsocketServer implements SipListener {
		...
		public void processInvite(RequestEvent requestEvent, ServerTransaction serverTransaction) {
			...
			try {
				...
				Address address = addressFactory.createAddress("UAS <sip:" + myAddress + ":" + myPort + ">");
	...
	public void testWebsocketBrowserServer() {
		...
		try {
			for (int q = 0; q < 10; q++) {
				Thread.sleep(3000);
				if (this.websocketServer.okByeReceived)
					break;
			}
		} catch (Exception ex) {
	...
	public void testLargeFrame() {
		...
		for (int q = 0; q < 70000; q++)
			sdpData.append("\0"); // use null character for large stuff otherwise eclipse may crash
