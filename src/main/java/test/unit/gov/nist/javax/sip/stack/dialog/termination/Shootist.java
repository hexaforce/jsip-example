...
public class Shootist implements SipListener {
	...
	public SipProvider createSipProvider() {
		try {
			...
			logger.error(ex);
			DialogTerminationOn50XTest.fail("Shootist: unable to create provider");
			return null;
		}
	}
	...
	public void processDialogTerminated(DialogTerminatedEvent dialogTerminatedEvent) {
		...
		System.out.println("Shootist: Dialog Terminated Event " + dialogTerminatedEvent.getDialog().getDialogId());
		if ((this.responseCodeToINFO) >= 300) {
			DialogTerminationOn50XTest.fail("Shootist: Got DialogTerinatedEvent, this shouldnt happen");
			stateIsOk = false;
		}
	...
	public void processRequest(RequestEvent requestReceivedEvent) {
		...
		System.out.println("GOT REQUEST (we shouldnt get that): " + request.getMethod());
	...
	public void processResponse(ResponseEvent responseReceivedEvent) {
		...
		try {
			if (response.getStatusCode() == Response.OK && ((CSeqHeader) response.getHeader(CSeqHeader.NAME)).getMethod().equals(Request.INVITE)) {
				...
				System.out.println("Dialog Confirmed: dialogID = " + dialog.getDialogId() + " dialogState = " + dialog.getState());
				if (tid == null)
				...
				ClientTransaction infoCTX = sipProvider.getNewClientTransaction(infoRequest);
				dialog.sendRequest(infoCTX);
			}
		} catch (Exception ex) {
			ex.printStackTrace();

			DialogTerminationOn50XTest.fail("Shootist: Exception on process respons/send info", ex);
		}
	...
	void sendInviteRequest() {
		...
		try {
			...
			fromHeader = headerFactory.createFromHeader(fromNameAddress, Integer.valueOf(fromTag).toString());
			String sdpData = "v=0\r\n" + "o=4855 13760799956958020 13760799956958020" + " IN IP4  129.6.55.78\r\n" + "s=mysession session\r\n" + "p=+46 8 52018010\r\n" + "c=IN IP4  129.6.55.78\r\n" + "t=0 0\r\n" + "m=audio 6022 RTP/AVP 0 4 18\r\n" + "a=rtpmap:0 PCMU/8000\r\n" + "a=rtpmap:4 G723/8000\r\n" + "a=rtpmap:18 G729A/8000\r\n" + "a=ptime:20\r\n";
			// Create ViaHeaders
			List<ViaHeader> viaHeaders = new ArrayList<>();
			ViaHeader viaHeader = headerFactory.createViaHeader(myAddress, listeningPoint.getPort(), transport, null);
			...
			DialogTerminationOn50XTest.fail("Shootist: Failed to send invite: ", ex);
