	...
	public class Shootist implements SipListener {
		...
		public void processResponse(ResponseEvent responseReceivedEvent) {
			try {
				if (responseReceivedEvent.getResponse().getStatusCode() == Response.OK) {
					...
					long cseq = ((CSeqHeader) responseReceivedEvent.getResponse().getHeader(CSeqHeader.NAME)).getSeqNumber();
		...
		public void init() {
			...
			try {
				...
				assertTrue("Should not see a parse exception ", !sawParseException);

				assertTrue("Should see unparsed headder", request.getUnrecognizedHeaders().hasNext() && ((String) request.getUnrecognizedHeaders().next()).equals("Date: 123 GMT 789"));
