...
public class ServerTransactionRetransmissionTimerTest extends TestCase {
	...
	class Shootist implements SipListener {
		...
		public void processResponse(ResponseEvent responseReceivedEvent) {
			logger.info(System.currentTimeMillis() - startTime + "ms: Got a response");
		...
		public void init() {
			...
			properties.setProperty("gov.nist.javax.sip.DEBUG_LOG", "logs/" + this.getClass().getName() + ".shootistdebug.txt");
			properties.setProperty("gov.nist.javax.sip.SERVER_LOG", "logs/" + this.getClass().getName() + ".shootistlog.txt");
		...
		public void processTransactionTerminated(TransactionTerminatedEvent transactionTerminatedEvent) {
			if (transactionTerminatedEvent.getServerTransaction() != null)
				logger.info("Shootist: Transaction terminated event recieved on transaction : " + transactionTerminatedEvent.getServerTransaction());
			else
				logger.info("Shootist : Transaction terminated event recieved on transaction : " + transactionTerminatedEvent.getClientTransaction());
		}
...
//                }
		...
		private void sendInviteOK() {
			try {
				if (inviteTid.getState() != TransactionState.COMPLETED) {
					if (!enableRetransmitAlerts) {
						logger.info("shootme: Dialog state before 200: " + inviteTid.getDialog().getState());
					}
					inviteTid.sendResponse(okResponse);
					if (!enableRetransmitAlerts) {
						logger.info("shootme: Dialog state after 200: " + inviteTid.getDialog().getState());
					}
				}
			} catch (Exception ex) {
				logger.error("Unexpected exception", ex);
		...
		public void init(String name) {
			...
			// Your code will limp at 32 but it is best for debugging.
			properties.setProperty("gov.nist.javax.sip.TRACE_LEVEL", "32");
			properties.setProperty("gov.nist.javax.sip.DEBUG_LOG", "logs/stxretransmission_shootmedebug.txt");
			properties.setProperty("gov.nist.javax.sip.SERVER_LOG", "logs/stxretransmission_shootmelog.txt");
			if (System.getProperty("enableNIO") != null && System.getProperty("enableNIO").equalsIgnoreCase("true")) {
				logger.info("\nNIO Enabled\n");
				properties.setProperty("gov.nist.javax.sip.MESSAGE_PROCESSOR_FACTORY", NioMessageProcessorFactory.class.getName());
			}
			...
			try {
				...
				addressFactory = sipFactory.createAddressFactory();
				messageFactory = sipFactory.createMessageFactory();
				lp = sipStack.createListeningPoint("127.0.0.1", myPort, "udp");
		...
		public void processTransactionTerminated(TransactionTerminatedEvent transactionTerminatedEvent) {
			if (transactionTerminatedEvent.isServerTransaction())
				logger.info("Transaction terminated event recieved" + transactionTerminatedEvent.getServerTransaction());
			else
				logger.info("Transaction terminated " + transactionTerminatedEvent.getClientTransaction());
