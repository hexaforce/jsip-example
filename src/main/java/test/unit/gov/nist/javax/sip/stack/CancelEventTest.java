...
public class CancelEventTest extends ScenarioHarness {
	...
	class Shootist implements SipListener {
		...
		Shootist() {
			...
			// implementation.
			properties.setProperty("gov.nist.javax.sip.DEBUG_LOG", logFileDirectory + this.getClass().getName() + ".debug.txt");
		...
		public void processResponse(ResponseEvent responseReceivedEvent) {
			...
			logger.info("Got a response" + ((CSeqHeader) response.getHeader(CSeqHeader.NAME)).getMethod());
		...
		public void sendInvite() {
			...
			String localAddress = sipProvider.getListeningPoint("udp").getIPAddress();
			int localPort = sipProvider.getListeningPoint("udp").getPort();
			String localTransport = sipProvider.getListeningPoint("udp").getTransport();
			...
			try {
				...
				Address toAddress = addressFactory.createAddress("sip:" + this.peerHost + ":" + peerPort);

				contactHeader = headerFactory.createContactHeader(fromAddres);
				toHeader = headerFactory.createToHeader(toAddress, null);
				fromHeader = headerFactory.createFromHeader(fromAddres, Integer.toString(new Random().nextInt()));
				...
				Address routeAddress = addressFactory.createAddress("sip:" + this.peerHost + ":" + peerPort);
				...
				inviteRequest = request = messageFactory.createRequest(requestURI, Request.INVITE, callIdHeader, cseqHeader, fromHeader, toHeader, list, maxForwardsHeader, contentTypeHeader, "CANCEL".getBytes());
		...
		public String[] conditionsState() {
			...
			result[3] = "[" + dialogTerminated + "] - Dialog terminated state[" + state + "]";
	...
	class Shootme implements SipListener {
		...
		public void processInvite(RequestEvent requestEvent, ServerTransaction serverTransaction) {
			...
			try {
				...
				if (st == null) {
					...
					logger.info("Created a new server transaction for " + request.getMethod() + " serverTransaction = " + st);
				}
				inviteTid = st;
		...
		public void processTransactionTerminated(TransactionTerminatedEvent transactionTerminatedEvent) {
			if (transactionTerminatedEvent.isServerTransaction()) {
				ServerTransaction serverTx = transactionTerminatedEvent.getServerTransaction();
		...
		public String[] conditionsState() {
			...
			if (dialog == null)
				...
				state = dialog.getState().toString();
			result[3] = "[" + dialogTerminated + "] - Dialog terminated state[" + state + "] count [" + dteCount + "]";
			return result;
		}
