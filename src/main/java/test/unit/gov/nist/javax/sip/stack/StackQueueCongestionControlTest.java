...
/**
 * Testing for deadlock under massive load on the same call. This testcase reproduces deadlock that occurs most frequently with TCP thread pool size set to 1 from this issue https://jain-sip.dev.java.net/issues/show_bug.cgi?id=301
 * 
	...
	public class Shootme implements SipListener {
		...
		public void processInvite(RequestEvent requestEvent, ServerTransaction serverTransaction) {
			...
			for (int q = 0; q < volume; q++) {
				sentResponses++;
				try {
					Response okResponse = messageFactory.createResponse(180, request);
		...
		public void processResponse(ResponseEvent responseReceivedEvent) {
			try {
				...
				if (h != null) {
					...
					if (i <= lastNumber)
						throw new RuntimeException("Messages out of order");
					lastNumber = i;
				}
				inUse = true;
				if (receivedResponses % 100 == 0)
					System.out.println("Receive " + receivedResponses);
