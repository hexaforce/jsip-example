...
public class CtxExpiredTest extends TestCase {
	...
	class Shootist implements SipListener {
		...
		public void sendInvite() {
			...
			try {
				...
				FromHeader fromHeader = headerFactory.createFromHeader(fromNameAddress, Integer.valueOf((int) (Math.random() * Integer.MAX_VALUE)).toString());
				...
				ViaHeader viaHeader = headerFactory.createViaHeader(myAddress, port, "udp", null);
				...
				Address address = addressFactory.createAddress("<sip:" + PEER_ADDRESS + ":" + PEER_PORT + ">");
		...
		public void processTimeout(TimeoutEvent timeoutEvent) {
			...
			TestCase.assertSame("Should see a timeout for INVITE", ctx, this.inviteTid);
			TestCase.assertEquals("Should be in proceeding state", TransactionState.PROCEEDING, ctx.getState());
			try {
				Request cancelRequest = timeoutEvent.getClientTransaction().createCancel();
				ClientTransaction cancelTx = this.provider.getNewClientTransaction(cancelRequest);
			...
			try {
				...
				if (request.getMethod().equals(Request.INVITE)) {
					if (requestEvent.getServerTransaction() == null) {
						...
						ServerTransactionExt serverTransaction = (ServerTransactionExt) this.provider.getNewServerTransaction(request);
	...
	public void setUp() throws Exception {
		...
		try {
			...
			this.shootistStack = (SipStackExt) sipFactory.createSipStack(properties);
