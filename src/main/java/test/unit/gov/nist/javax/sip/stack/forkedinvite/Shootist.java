...
public class Shootist implements SipListener {
	...
	class SendBye extends TimerTask {
		...
		public void run() {
			try {
				TestCase.assertEquals("Dialog state must be confirmed", DialogState.CONFIRMED, dialog.getState());
				...
				if (cseq.getMethod().equals(Request.INVITE)) {
					...
					if (!isFromFork) {
						...
						if (ackedDialog == null) {
							...
							TestCase.assertTrue("Dialog state should be CONFIRMED", dialog.getState() == DialogState.CONFIRMED);

							TestCase.assertTrue(this.ackedDialog == null || this.ackedDialog == dialog);
						...
						TestCase.assertSame("original ctx must match ", this.originalTransaction, responseReceivedEvent.getOriginalTransaction());
						if (cseq.getMethod().equals(Request.INVITE)) {
							TestCase.assertSame("Must preserve original dialog", this.originalDialog, responseReceivedEvent.getOriginalTransaction().getDefaultDialog());
						}
	...
	public void checkState() {
		TestCase.assertEquals("Should see " + this.counter + " distinct early dialogs", counter, this.forkedEarlyDialogs.size());
		TestCase.assertEquals("Should see " + this.counter + " distinct dialogs", counter, this.forkedDialogs.size());
		if (!createDialogAfterRequest) {
			TestCase.assertTrue("Should see the original (default) dialog in the forked set", this.forkedDialogs.contains(this.originalDialog));
	...
	public void sendInvite(int forkCount) {
		try {
			...
			Header pPreferredServiceHeader = headerFactory.createHeader(PPreferredServiceHeader.NAME, InviteTest.PREFERRED_SERVICE_VALUE);
			request.addHeader(pPreferredServiceHeader);
			Header pAssertedServiceHeader = headerFactory.createHeader(PAssertedServiceHeader.NAME, InviteTest.PREFERRED_SERVICE_VALUE);
			request.addHeader(pPreferredServiceHeader);
			...
			if (isAutomaticDialogSupportEnabled) {
				...
				TestCase.assertTrue("Initial dialog state should be null", dialog.getState() == null);
			} else {
				if (!createDialogAfterRequest) {
