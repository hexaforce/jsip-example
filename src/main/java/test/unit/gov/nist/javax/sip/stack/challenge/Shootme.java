	...
	public void processInvite(RequestEvent requestEvent, ServerTransaction serverTransaction) {
		...
		try {
			...
			if (!challenged) {
				...
				Response challenge = protocolObjects.messageFactory.createResponse(401, request);
				ToHeader toHeader = (ToHeader) challenge.getHeader(ToHeader.NAME);
			...
			if (st == null) {
				...
				ChallengeTest.assertSame("Dialog mismatch ", st.getDialog(), this.dialog);
			}
	...
	public void processResponse(ResponseEvent responseReceivedEvent) {
		...
		try {
			if (response.getStatusCode() == Response.OK && ((CSeqHeader) response.getHeader(CSeqHeader.NAME)).getMethod().equals(Request.INVITE)) {
				ChallengeTest.assertNotNull("INVITE 200 response should match a transaction", tid);
