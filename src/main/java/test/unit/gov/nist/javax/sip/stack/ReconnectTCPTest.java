...
/**
 * Non regression test for http://code.google.com/p/mobicents/issues/detail?id=3188 and http://java.net/jira/browse/JSIP-362
 * 
 * Shootist sends Invite to Shootme, Shootme before responding close all TCP Sockets simulating a disconnect. Shootme should be able to send the responses back to the shootist and get the ACK
 */
public class ReconnectTCPTest extends ScenarioHarness implements SipListener {
	...
	class Shootme implements SipListener {
		...
		public class DelayedInviteResponsesTask extends TimerTask {
			RequestEvent requestEvent;
			SipProvider sipProvider;

			public DelayedInviteResponsesTask(RequestEvent requestEvent, SipProvider sipProvider) {
	...
	class Shootist implements SipListener {
		...
		public void processResponse(ResponseEvent responseReceivedEvent) {
			...
			if (tid != null) {
				...
				logger.info("Dialog State is " + responseReceivedEvent.getDialog().getState());
			}
		...
		public void sendInvite() {
			...
			try {
				...
				ReconnectTCPTest.assertTrue(this.dialog.getState() == null);
	...
	public ReconnectTCPTest() {
		super(ReconnectTCPTest.class.getName(), true);
	}
