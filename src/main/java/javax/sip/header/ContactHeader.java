 ...
 * <ul>
 * <li>Requests: A contact header is mandatory for INVITE's and optional for ACK, OPTIONS and REGISTER requests. This allows the callee to send future Requests, such as BYE Requests, directly to the caller instead of through a series of proxies.
 * <li>Informational Responses - A contact header is optional in a Informational Response to an INVITE request. It has the same semantics in an Informational Response as a Success Response.
 * <li>Success Responses - A contact header is mandatory in response to INVITE's and optional in response to OPTIONS and REGISTER requests. An user agent server sending a Success Response to an INIVTE must insert a ContactHeader in the Response indicating the SIP address under which it is reachable most directly for future SIP Requests.
 * <li>Redirection Responses - A contact header is optional in response to INVITE's, OPTIONS, REGISTER and BYE requests. A proxy may also delete the contact header.
 * <li>Ambiguous Header: - A contact header is optional in response to INVITE, OPTIONS, REGISTER and BYE requests.
 * </ul>
 *
 * The ContactHeader defines the Contact parameters "q" and "expires". The <code>q-value</code> value is used to prioritize addresses in a list of contact addresses. The <code>expires</code> value suggests an expiration interval that indicates how long the client would like a registration to be valid for a specific address. These parameters are only used when the Contact is present in a:
 * <ul>
 * <li>REGISTER request
...
public interface ContactHeader extends HeaderAddress, Parameters, Header {
	 ...
	 *
	 * @param expires new relative value of the expires parameter. 0 implies removal of Registration specified in Contact Header.
	...
	/**
	 * Returns the value of the <code>expires</code> parameter or -1 if no expires parameter was specified or if the parameter value cannot be parsed as an int.
	 *
	 * @return value of the <code>expires</code> parameter measured in delta-seconds, O implies removal of Registration specified in Contact Header.
	 */
	 ...
	 * @throws InvalidArgumentException if the q-value parameter value is not <code>-1</code> or between <code>0 and 1</code>.
	 */
