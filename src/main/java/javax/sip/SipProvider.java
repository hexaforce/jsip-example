 ...
 * <ul>
 * <li>Register a {@link javax.sip.SipListener} to the SipProvider. Once the SipListener is registered with the SipProvider it will get notified of Events representing either Request, Response, Timeout messages, Network errors and Transaction termination.
 * <li>De-register a {@link javax.sip.SipListener} from the SipProvider. Once a SipListener is de-registered, it will no longer receive any Events from that SipProvider.
...
public interface SipProvider {
	 ...
	 * @throws TooManyListenersException when a new SipListener attempts to register with the SipProvider when another SipListener is already registered with this SipProvider.
	 */
	 ...
	 *
	 * @deprecated Since v1.2. Note that in v1.1 a SipProvider could only be associated to a single listening point, this restriction has been lifted to allow a SipProvider to have a specific ListeningPoints for each transport. For backwards compatibility, this method will add the ListeningPoint to the list of ListeningPoints associated with the SipProvider. This method has been replaced with {@link SipProvider#addListeningPoint(ListeningPoint)}, the same semantics apply to this method.
	 */
	 ...
	 * @throws ObjectInUseException               if the supplied ListeningPoint is being used by another SipProvider or if there is already a ListeningPoint for the given transport.
	 * @throws TransportAlreadySupportedException if there is already a ListeningPoint associated to this SipProvider with the same transport of the ListeningPoint.
	 *
	 * @since 1.2
	 */
	 ...
	 * @throws ObjectInUseException if the ListeningPoint is already in use or is the last ListeningPoint associated with this SipProvider.
	 * @since 1.2
	 */
	...
	/**
	 * Before an application can send a new request it must first request a new client transaction to handle that Request. This method is called by the application to create the new client transaction befores it sends the Request on that transaction. This methods returns a new unique client transaction that can be passed to send Requests statefully.<b>Do not call this method for ACK requests.</b>
	 *
	 * @param request the new Request message that is to handled statefully by the ClientTransaction.
	 * @return a new unique client transaction.
	 * @throws TransactionUnavailableException if a new transaction can not be created, for example the next hop of the request can not be determined or the method is "ACK"
	...
	/**
	 * An application has the responsibility of deciding to respond to a Request that does not match an existing server transaction. This method is called by an application that decides to respond to an unmatched Request statefully. This methods return a new unique server transaction that can be used to respond to the request statefully. <b> Do ~not~ call this method for ACK requests. </b>
	 *
	 * @param request the Request message that the doesn't match an existing transaction that the application decides to handle statefully.
	 * @return a new unique server transaction.
	 * @throws TransactionAlreadyExistsException if a transaction already exists that is already handling this Request. This may happen if the application gets retransmits of the same request before the initial transaction is allocated.
	 * @throws TransactionUnavailableException   if a new transaction can not be created, for example the next hop of the request can not be determined or the method is "ACK"
	 ...
	 *
	 *
	 * @param transaction - transaction that is used to extract the relevant information to create the dialog.
	 *
	 * @throws SipException if one or more of the following is true: <br/>
	 *                      <ol>
	 *                      <li>The Method of the Request is not a Dialog creating</li>
	 *                      <li>There is missing required information such as From header Tag in the Request</li>
	 *                      <li>This method is called after the response recieved on the client side</li>
	 *                      <li>This method is called after the response is sent out on the server side of the dialog.</li>
	 *                      </ol>
	 *
	 * @since 1.2
	 */
