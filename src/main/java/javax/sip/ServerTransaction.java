...
/**
 * A server transaction is used by a SipProvider to handle incoming Request messages to fire Request events to the SipListener on a specific server transaction and by a User Agent Server application to send Response messages to a User Agent Client application. This interfaces enables an application to send a {@link javax.sip.message.Response} to a recently received Request in a transaction stateful way.
 * <p>
 * A new server transaction is generated in the following ways:
 * <ul>
 * <li>By the application by invoking the {@link SipProvider#getNewServerTransaction(Request)} for Requests that the application wishes to handle.
 * <li>By the SipProvider by automatically populating the server transaction of a RequestEvent for Incoming Requests that match an existing Dialog. Note that a dialog-stateful application is automatically transaction stateful too
 * </ul>
 ...
 * <p>
 * For the detailed server transaction state machines refer to Chapter 17 of <a href="http://www.ietf.org/rfc/rfc3261.txt">RFC 3261</a>, the allowable transitions are summarized below:
 * <p>
...
public interface ServerTransaction extends Transaction {
	...
	/**
	 * Sends the Response to a Request which is associated with this ServerTransaction. When an application wishes to send a Response, it creates a Response using the {@link javax.sip.message.MessageFactory} and then passes that Response to this method. The Response message gets sent out on the network via the ListeningPoint information that is associated with the SipProvider of this ServerTransaction.
	 * <p>
	 ...
	 *
	 * <p>
	 * <b>ACK Processing and final response retransmission:</b> <br/>
	 ...
	 * @throws SipException             if the SipProvider cannot send the Response for any other reason.
	 * @throws InvalidArgumentException if the Response is created by {@link Dialog#createReliableProvisionalResponse(int)} and the application attempts to use this method to send the response.
	 * @see Response
	 */
