 ...
 * <li>Local sequence number - used to order requests from the User Agent to its peer.
 * <li>Remote sequence number - used to order requests from its peer to the User Agent.
 * <li>Local URI - the address of the local party.
 * <li>Remote URI - the address of the remote party.
 * <li>Remote target - the address from the Contact header field of the request or response or refresh request or response.
 * <li>"secure" boolean - determines if the dialog is secure i.e. use the <var>sips:</var> scheme.
 * <li>Route set - an ordered list of URIs. The route set is the list of servers that need to be traversed to send a request to the peer.
 * </ul>
 * A dialog also has its own state machine, the current {@link DialogState} is determined by the sequence of messages that occur on the initial dialog.
 * <p>
 * <b>Invite Dialog States:</b><br>
 * Null --> Early --> Confirmed --> Terminated
 * <p>
 * <b>Other Dialog-creating Requests Dialog States (ie. SUBSCRIBE):</b><br>
 ...
 * <li>Additional 2xx responses will be presented to the listener if no dialog is associated with the response. In such a case the listener is expected to ACK the 2xx response.
 * </ul>
 *
 ...
 * <li>If a Dialog is not associated with a ServerTransaction, and if the application explicitly requests to be notified of such timeouts (see {@link ServerTransaction#enableRetransmissionAlerts()} the SipListener is periodically alerted to retransmit 2xx responses for the ServerTransaction.</li>
 * <li>If a Dialog is associated with a ServerTransaction then the implementation takes care of retransmitting final responses until the ACK is received i.e. the SipListener will receive no notifications to retransmit responses.
 * </ul>
 *
 * For an INVITE server Transaction 300-699 response:
 * <ul>
 * <li>Either the application or the dialog layer sends the first response
 * <li>Retransmissions of 300-699 responses from the INVITE Server Transaction are handled by the transaction layer independent of existence of an associated Dialog.
 * </ul>
 *
 ...
 * <ul>
 * <li>The application sends a reliable provisional Response to an Invite Dialog using {@link javax.sip.Dialog#sendReliableProvisionalResponse(Response) }
 * <li>The Stack takes care of retransmitting the provisional response with exponentially increasing intervals until a PRACK is received or the Transaction times out.
 * </ul>
 *
...
public interface Dialog extends Serializable {
	 ...
	 * <ul>
	 * <li>User Agent Client - the Call-Id value of the dialog Id is set to the Call-Id of the message, the remote tag is set to the tag in the To field of the message, and the local tag is set to the tag in the From field of the message.
	 * <li>User Agent Server - the Call-Id value of the dialog Id is set to the Call-Id of the message, the remote tag is set to the tag in the From field of the message, and the local tag is set to the tag in the To field of the message.
	 * </ul>
	 *
	 ...
	 *
	 * @deprecated This method is replaced with {@link #getLocalSeqNumber()} with type long.
	 ...
	 *
	 * @deprecated This method is replaced with {@link #getRemoteSeqNumber()} with type long.
	 ...
	 *
	 * @return <code>true</code> if this dialog was secure, and <code>false</code> otherwise.
	 */
	...
	/**
	 * Creates a new reliable provisional response based on an Invite request that created this Dialog or that is now refreshing this Dialog. This new Response does not contain a body. Only the required headers are copied from the Request. The response will include also the following headers:
	 * <ul>
	 * <li>RequireHeader
	 * <li>RSeqHeader
	 * </ul>
	 * The RequireHeader will include the 100rel option tag. The Dialog will handle the RSeq count that will be updated and icreased if needed when the reliable provisional response is sent. Any Response created by this method must be sent via the {@link Dialog#sendReliableProvisionalResponse( Response)} method.
	 * <p>
	 *
	 * @since 1.2
	 *
	 ...
	 * @throws InvalidArgumentException when an invalid status code or request method is supplied.
	 * @throws SipException             when the Dialog is not a server dialog or if this method is called after a final response is sent to the ServerTransactin that created the Dialog.
	 *
	 */
	public Response createReliableProvisionalResponse(int statusCode) throws InvalidArgumentException, SipException;

	/**
	 * Sends a Request to the remote party of this dialog. When an application wishes to send a Request message on this dialog, it creates a Request and creates a new ClientTransaction to handle this request from {@link SipProvider#getNewClientTransaction(Request)}. This ClientTransaction is passed to this method to send the request. The Request message gets sent via the ListeningPoint information of the SipProvider that is associated to this ClientTransaction.
	 * <p>
	 ...
	 * <p>
	 * A Dialog may be created by an INVITE request and subsequently SUBSCRIBE/NOTIFY are sent withing that Dialog. In this case the application may call {@link Dialog#terminateOnBye } to prevent the Dialog from Terminating upon reciept of a BYE.
	 *
	 * @param clientTransaction - the new ClientTransaction object identifying this transaction, this clientTransaction should be requested from {@link SipProvider#getNewClientTransaction(Request)}
	 * @throws TransactionDoesNotExistException if the clientTransaction does not correspond to any existing client transaction.
	 * @throws SipException                     if implementation cannot send the Request for any reason.
	 */
	public void sendRequest(ClientTransaction clientTransaction) throws TransactionDoesNotExistException, SipException;

	/**
	 * Sends a reliable provisional response to the remote party of this dialog. When an application wishes to send a reliable provisional response on the Invite transaction that created this dialog or that is now refreshing this dialog, it creates a reliable provisional response message with {@link Dialog#createReliableProvisionalResponse(int)}. This Response is passed to this method that sends it. This method will update the RSeq header of the response if needed and will increase the RSeq count of the Transaction. The application will not be able to send further reliable provisional responses for this Dialog until PRACK is received for a previously sent provisional response. The only response that can be sent in parallel with an active reliable Provisional Response is a final response. The Final response will stop retransmission of the reliable responses.
	 * <p>
	 *
	 * @since 1.2
	 *
	 * @param relResponse - the reliable provisional response
	 * @throws SipException if implementation cannot send the Request for any reason (e.g. because another reliable response is still pending).
	 */
	public void sendReliableProvisionalResponse(Response relResponse) throws SipException;

	/**
	 * Creates a new PRACK Request message based on a reliable provisional response received by this Dialog and the Dialog internal information. The returned Request will be correctly formatted that is the Dialog implementation is responsible for assigning the following:
	 * <ul>
	 * <li>RequestURI
	 * <li>FromHeader
	 * <li>ToHeader
	 ...
	 * @return the newly created Request message on this Dialog.
	 * @throws SipException                if the Method of the transaction that created the Dialog or Refeshing the Dialog is not an INVITE ( for example SUBSCRIBE).
	 * @throws DialogDoesNotExistException if the Dialog is not yet established (i.e. dialog state equals null) or is terminated.
	 * @since 1.2
	 */
	 ...
	 * @throws InvalidArgumentException if there is a problem with the supplied cseq ( for example <= 0 ).
	 * @throws SipException             if the cseq does not relate to a previously sent INVITE or if the Method that created the Dialog is not an INVITE ( for example SUBSCRIBE)
	 * @since 1.2
	 */
	 ...
	 * @throws SipException if implementation cannot send the ACK Request for any reason
	 */
	public void sendAck(Request ackRequest) throws SipException;

	/**
	 * Returns the current DialogState of the dialog or null. A dialog that is created but not yet mapped to any state must return null, multiple requests can be generated on the Dialog in a null state. The dialog states for INVITE transaction are:
	 * <ul>
	 * <li>Early - A dialog is in the "early" state, which occurs when it is created when a provisional response is recieved to the INVITE Request.
	 * <li>Confirmed - A dialog transitions to the "confirmed" state when a 2xx final response is received to the INVITE Request.
	 * <li>Terminated - A dialog transitions to the "terminated" state for all other reasons or if no response arrives at all on the dialog.
	 * </ul>
	 *
	 * A Subscibe/Refer dialog has the following states:
	 * <ul>
	 * <li>Null - refers to a state that is not terminated.
	 * <li>Terminated - when the Application deletes the Dialog or if no response arrives at all.
	 * </ul>
	 *
	 * Independent of the method, if a request outside of a dialog generates a non-2xx final response, any early dialogs created through provisional responses to that request are "terminated". If no response arrives at all on the early dialog it is also "terminated".
	 *
	...
	/**
	 * This method retrieves the transaction which resulted in the creation of this Dialog. The transaction type either server or client can be determined based on whether this is a server or client Dialog, see {@link Dialog#isServer()}.
	 *
	...
	/**
	 * Terminate Dialog on BYE. If this flag is set to true then the stack will Terminate the dialog automatically when BYE is received. This is the default behavior. This method is useful for SUBSCRIBE/NOTIFY processing within a Dialog. In the case that several subscriptions are associated with a single Dialog, the Dialog does not terminate until all the subscriptions in it are destroyed. Hence, if the application is aware of active subscriptions in a dialog it should set this flag to false. In this case when BYE is received the dialog will not be terminated and it will be the application�s responsibility to call the {@link Dialog#delete()} function when all active subscriptions are terminated.
	 *
	 * @param terminateFlag -- if true then the dialog is terminated when a BYE is received.
