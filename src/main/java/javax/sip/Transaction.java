...
public interface Transaction extends Serializable {
	 ...
	 * 
	 * <ul>
	 * <li>If the stack is configured with the AUTOMATIC_DIALOG_SUPPORT property set to </it>ON</it> ( default behavior ) then the following behavior is defined:
	 * <ul>
	 ...
	 * <li>If the Transaction is neither dialog creating nor can be associated with an existing dialog, then the implementation must return null when the application issues getDialog on the transaction.
	 * </ul>
	 * <li>If the stack is configured with AUTOMATIC_DIALOG property set to </it>OFF</it> then the stack does not automatically create a Dialog for a transaction nor does it maintain an association between dialog and transaction on behalf of the application. Hence this method will return null. It is the responsibility of the application to create a Dialog and associate it with the transaction when the response is sent.
	 * </ul>
	 *
	 ...
	 * @throws UnsupportedOperationException if this method is not supported by the underlying implementation.
	 */
	 ...
	 * 
	 * @throws ObjectInUseException if the transaction cannot be terminated as it is associated to a dialog.
	 * @since v1.2
	 */
