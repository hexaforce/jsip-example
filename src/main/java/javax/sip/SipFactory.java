 ...
 * <ul>
 * <li>The <b>upper-level package structure</b> referred to by the SipFactory with the attribute <var>pathname</var> can be used to differentiate between proprietary implementations from different SIP stack vendors. The <var>pathname</var> used by each SIP vendor <B>must be</B> the domain name assigned to that vendor in reverse order. For example, the pathname used by Sun Microsystem's would be <code>com.sun</code>.
 * <li>The <b>lower-level package structure and classname</b> of a peer object is also mandated by this specification. The lowel-level package must be identical to the package structure defined by this specification and the classname is mandated to the interface name appended with the <code>Impl</code> post-fix. For example, the lower-level package structure and classname of a proprietary implementation of the <code>javax.sip.SipStack</code> interface <B>must</B> be <code>javax.sip.SipStackImpl</code>.
 * </ul>
 *
...
public class SipFactory {
	 ...
	 * <p>
	 * See {@link SipStack} for the expected format of the <code>properties</code> argument.
	 *
	 ...
	 *
	 * @param pathName - the reverse domain name of the vendor, e.g. Sun Microsystem's would be 'com.sun'
	 */
	...
	/**
	 * Constructor for SipFactory class. This is private because applications are not permitted to create an instance of the SipFactory using "new".
	 */
	private SipFactory() {
