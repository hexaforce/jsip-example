...
/**
 * This interface represents a user's display name and URI address. The display name of an address is optional but if included can be displayed to an end-user. The address URI (most likely a SipURI) is the user's address. For example a 'To' address of <code>To: Bob sip:duke@jcp.org</code> would have a display name attribute of <code>Bob</code> and an address of <code>sip:duke@jcp.org</code>.
 * 
 * @see SipURI
 * @see TelURL
 *
...
public interface Address extends Cloneable, Serializable {
	...
	/**
	 * This determines if this address is a wildcard address. That is <code>((SipURI)Address.getURI()).getUser() == *;</code>. This method is specific to SIP and SIPS schemes.
	 *
