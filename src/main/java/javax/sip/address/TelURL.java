...
/**
 * This class represents Tel URLs, which are used for addressing. The Tel URL starts with the scheme <code>tel:</code>. This tells the local entity that what follows is a URL that should be parsed as described in <a href = "http://www.ietf.org/rfc/rfc2806.txt">RFC2806</a>. After that, the URL contains the phone number of the remote entity.
 * <p>
 ...
 * <ul>
 * <li>ISDN Subaddress - Phone numbers can also contain subaddresses, which are used to identify different remote entities under the same phone number.
 * <li>Post Dial - Phone numbers can also contain a post-dial sequence. This is what is often used with voice mailboxes and other services that are controlled by dialing numbers from your phone keypad while the call is in progress.
 * <li>Global - Phone numbers can be either "global" or "local". Global numbers are unambiguous everywhere. Local numbers are usable only within a certain area.
 * <li>URL parameters - Parameters affecting a request constructed from this URL. URL parameters are added to the end of the URL component and are separated by semi-colons. URL parameters take the form:<br>
 * parameter-name "=" parameter-value
 * </ul>
 * See <a href = "http://www.ietf.org/rfc/rfc2806.txt">RFC2806</a> for more information on the use of TelURL's.
 *
...
public interface TelURL extends URI, Parameters {
	...
	/**
	 * Returns <code>true</code> if this TelURL is global i.e. if the TelURI has a global phone user.
	 *
	 * @return <code>true</code> if this TelURL represents a global phone user, and <code>false</code> otherwise.
	 */
	 ...
	 * @throws ParseException which signals that an error has been reached unexpectedly while parsing the postDial value.
	 */
	 ...
	 * @throws ParseException which signals that an error has been reached unexpectedly while parsing the phoneNumber value.
	 */
	public void setPhoneNumber(String phoneNumber) throws ParseException;

	/**
	 * Returns the value of the <code>phoneNumber</code> parameter. This method will not return the "+" associated with telephone numbers.
	 *
	 ...
	 * @throws ParseException which signals that an error has been reached unexpectedly while parsing the isdnSubAddress value.
	 */
	 ...
	 *
	 * @param phoneContext - new value of the <code>phoneContext</code> parameter
	 * @throws ParseException which signals that an error has been reached unexpectedly while parsing the phoneContext value.
	 * @since v1.2
	 */
	public void setPhoneContext(String phoneContext) throws ParseException;

	/**
	 * Returns the value of the <code>phoneContext</code> parameter, or null if it is not set.
	 *
