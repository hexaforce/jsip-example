...
/**
 * This class represents SIP URIs, that may have either a <code>sip:</code> or <code>sips:</code> scheme. All SIP implementations MUST support the sip: URI scheme.
 * <p>
 * SIP and SIPS URIs are used for addressing. They are similar to email addresses in that they are of the form <code>user@host</code> where user is either a user name or telephone number, and host is a host or domain name, or a numeric IP address. Additionally, SIP and SIPS URIs may contain parameters and headers (although headers are not legal in all contexts). A SipURI can be embedded in web pages, business cards or other hyperlinks to indicate that a particular user or service can be called via SIP.
 * <p>
 ...
 * <ul>
 * <li>User - The set of valid telephone-subscriber strings is a subset of valid user strings. The user URI parameter exists to distinguish telephone numbers from user names that happen to look like telephone numbers. If the user string contains a telephone number formatted as a telephone-subscriber, the user parameter value "phone" SHOULD be present. Even without this parameter, recipients of SIP and SIPS URIs MAY interpret the pre-@ part as a telephone number if local restrictions on the name space for user name allow it.
 * <li>UserPassword - A password associated with the user. While the SIP and SIPS URI syntax allows this field to be present, its use is NOT RECOMMENDED, because the passing of authentication information in clear text (such as URIs) has proven to be a security risk in almost every case where it has been used. For instance, transporting a PIN number in this field exposes the PIN.
 * <li>URI parameters - Parameters affecting a request constructed from this URI. URI parameters are added after the hostport component and are separated by semi-colons. URI parameters take the form:<br>
 ...
 * <ul>
 * <li>Lr Parameter - the element responsible for this resource implements the routing mechanisms specified in RFC 3261.
 ...
 * <li>TTL Parameter - the time-to-live value when packets are sent using UDP multicast.
 * <li>User Parameter - the set of valid telephone-subscriber strings.
 * <li>Transport Parameter - specifies which transport protocol to use for sending requests and responses to this entity
 * </ul>
 * <li>URI Headers - Header fields to be included in a request constructed from the URI. Headers fields in the SIP request can be specified with the "?" mechanism within a URI. The header names and values are encoded in ampersand separated 'hname = hvalue' pairs. The special hname "body" indicates that the associated hvalue is the message-body of the SIP request.
 * <li>Secure - This determines if the scheme of this URI is either <code>sip:</code> or <code>sips:</code>, where <code>sips:</code> is secure.
 * </ul>
 * See section 19.1.2 of <a href = "http://www.ietf.org/rfc/rfc3261.txt">RFC3261</a> for the use of SIP and SIPS URI components based on the context in which the URI appears.
 *
...
public interface SipURI extends URI, Parameters {
	 ...
	 * @throws ParseException which signals that an error has been reached unexpectedly while parsing the user value.
	 */
	 ...
	 * @throws ParseException which signals that an error has been reached unexpectedly while parsing the userPassword value.
	 */
	 ...
	 *
	 * @return <code>true</code> if this SipURI represents a sips URI, and <code>false</code> if it represents a sip URI.
	 */
	...
	/**
	 * Set the host part of this SipURI to the newly supplied <code>host</code> parameter.
	 *
	 * @param host - the new interger value of the host of this SipURI
	 * @throws ParseException which signals that an error has been reached unexpectedly while parsing the host value.
	 */
...
// header manipulation methods
	 ...
	 * @throws ParseException which signals that an error has been reached unexpectedly while parsing the name or value parameters.
	 */
...
//Param Covenience methods
	 ...
	 * @throws ParseException which signals that an error has been reached unexpectedly while parsing the transport value.
	 */
	 ...
	 * @throws InvalidArgumentException if supplied value is less than zero, excluding -1 the default not set value.
	 */
	 ...
	 * @throws ParseException which signals that an error has been reached unexpectedly while parsing the userParam value.
	 */
	 ...
	 * @throws ParseException which signals that an error has been reached unexpectedly while parsing the mAddr value.
	 */
