 ...
 *
 * <p>
 * For backwards compatibility reasons, the default behavior of the stack is to consult the application provided Router implementation for all requests outside of a dialog. This is controlled through the stack property <code>javax.sip.USE_ROUTER_FOR_ALL_URIS</code> which defaults to <code>true</code> when not set.
 *
 * <p>
 ...
 *
 * <p>
 * When <code>javax.sip.USE_ROUTER_FOR_ALL_URIS</code> is set to <code>false</code>, the next hop is determined according to the following algorithm:
 * <ul>
 * <li>If the request contains one or more Route headers, use the URI of the topmost Route header as next hop, possibly modifying the request in the process if the topmost Route header contains no lr parameter(See Note below))
 * <li>Else, if the property <code>javax.sip.OUTBOUND_PROXY</code> is set, use its value as the next hop
 * <li>Otherwise, use the request URI as next hop. If the request URI is not a SIP URI, call {@link javax.sip.address.Router#getNextHop(Request)} provided by the application.
 * </ul>
 *
 * <p>
 * <b>Note:</b> In case the topmost Route header contains no 'lr' parameter (which means the next hop is a strict router), the implementation will perform 'Route Information Postprocessing' as described in RFC3261 section 16.6 step 6 (also known as "Route header popping"). That is, the following modifications will be made to the request:
 * <ol>
 * <li>The implementation places the Request-URI into the Route header field as the last value.
 * <li>The implementation then places the first Route header field value into the Request-URI and removes that value from the Route header field.
 * </ol>
 ...
 * <p>
 * The routing policy can not be changed dynamically, i.e. the SipStack needs to be deleted and re-created. Outbound proxy should be passed to the {@link javax.sip.SipFactory#createSipStack(Properties)} method upon creation of the SIP Stack object.
 *
 * <p>
...
public interface Router {
	 ...
	 *
	 * @deprecated Since v1.2. This method is replaced with {@link Router#getNextHop(Request)} method which returns the next Hop for this request.
	 *
	 */
	public ListIterator getNextHops(Request request);

	/**
	 * Gets the next Hop from this Router for the specified request, this method may return <code>null</code> if a default route is not defined.
	 *
