...
/**
 * This interface represents the application view to a SIP stack therefore defines the application's communication channel to the SIP stack. This interface defines the methods required by an applications to receive and process Events that are emitted by an object implementing the {@link javax.sip.SipProvider}interface.
 * <p>
 * The Events accepted by a SipListener may be one of four types:
 * <ul>
 * <li>{@link RequestEvent}- these are request messages emitted as events by the SipProvider. Request events represent request messages i.e. INVITE, that are received from the network to the application via the underlying stack implementation.
 * <li>{@link ResponseEvent}- these are response messages emitted as events by the SipProvider. Response events represent Response messages i.e. 2xx's, that are received from the network to the application via the underlying stack implementation.
 * <li>{@link TimeoutEvent}- these are timeout notifications emitted as events by the SipProvider. Timeout events represent timers expiring in the underlying SipProvider transaction state machine. These timeout's events notify the application that a retranmission is required or a transaction has timed out.
 * <li>{@link IOExceptionEvent}- these are IO Exception notifications emitted as events by the SipProvider. IOException events represent failure in the underlying SipProvider IO Layer. These IO Exception events notify the application that a failure has occured while accessing a socket.
 * <li>{@link TransactionTerminatedEvent}- these are Transaction Terminated notifications emitted as events by the SipProvider. TransactionTerminated events represent a transaction termination and notify the application of the termination.
 * <li>{@link DialogTerminatedEvent}- these are Dialog Terminated notifications emitted as events by the SipProvider. DialogTerminated events represent a Dialog termination and notify the application of the termination.
 * </ul>
 * <p>
 * An application will only receive Request, Response, Timeout, TransactionTerminated, DialogTerminated and IOException events once it has registered as an EventListener of a SipProvider. The application registers with the SipProvider by invoking the {@link SipProvider#addSipListener(SipListener)}passing itself as an argument.
 * <p>
 * <b>Architecture: </b> <br>
 ...
 * An application can send messages by passing {@link javax.sip.message.Request} and {@link javax.sip.message.Response}messages to that the following object:
 * <ul>
 * <li>Request and response messages can be sent statelessly via the SipProvider using the send methods on the {@link javax.sip.SipProvider}.
 * <li>Request messages can be sent transaction stateful via the ClientTransaction using the {@link ClientTransaction#sendRequest()}method.
 * <li>Response messages can be sent transaction stateful via the ServerTransaction using the {@link ServerTransaction#sendResponse(Response)} method.
 * <li>Request messages can be sent dialog stateful via the Dialog using the {@link Dialog#sendRequest(ClientTransaction)}method.
 * </ul>
...
public interface SipListener extends EventListener {
	 ...
	 * <li>Forward the new request using the {@link ClientTransaction#sendRequest()}method.
	 * <li>Process all responses recieved on the {@link SipListener#processResponse(ResponseEvent)}method.
	 ...
	 * As a stateless proxy does not have any notion of a transaction, or of the response context used to describe stateful proxy behavior, <code>requestEvent.getServerTransaction() == null;</code> always return <var>true </var>. The transaction layer of the SipProvider implementation is by-passed. For all requests including any with unknown methods, an application intending to stateless proxy the request MUST:
	 * <ul>
	 * <li>Validate the request.
	 * <li>Preprocess routing information.
	 * <li>Determine a single target(s) for the request.
	 * <li>Forward the request to the target using the {@link SipProvider#sendRequest(Request)}method.
	 * <li>NOT perform special processing for CANCEL requests.
	 * </ul>
	 *
	 * @param requestEvent - requestEvent fired from the SipProvider to the SipListener representing a Request received from the network.
	 */
	 ...
	 * <li>When necessary, choose the best final response from the response context. If no final response has been forwarded after every client transaction associated with the response context has been terminated, the proxy must choose and forward the "best" response from those it has seen so far.
	 * </ul>
	 * <p>
	 ...
	 * <li>Forward the response using the {@link ServerTransaction#sendResponse(Response)}method.
	 ...
	 * <li>If that address matches the proxy, the proxy MUST remove that header from the response.
	 * <li>Forward the resulting response to the location indicated in the next Via header using the {@link SipProvider#sendResponse(Response)}method.
	 * </ul>
	 *
	 * @param responseEvent - the responseEvent fired from the SipProvider to the SipListener representing a Response received from the network.
	 */
	 ...
	 *
	 * @param timeoutEvent - the timeoutEvent received indicating either the message retransmit or transaction timed out.
	 */
	 ...
	 *
	 * @param transactionTerminatedEvent -- an event that indicates that the transaction has transitioned into the terminated state.
	 * @since v1.2
	 */
	 ...
	 *
	 * @param dialogTerminatedEvent -- an event that indicates that the dialog has transitioned into the terminated state.
	 * @since v1.2
	 */
