...
/**
 * A client transaction is used by a User Agent Client application to send Request messages to a User Agent Server application. The client transaction is also used to match Responses from the User Agent Server to fire Response events to the SipListener for a specific client transaction. This interfaces enables an application to send a {@link javax.sip.message.Request}'s statefully. A new client transaction is generated by the application calling the {@link SipProvider#getNewClientTransaction(Request)} method.
 * <p>
 ...
 * <p>
 * For the detailed client transaction state machines refer to Chapter 17 of <a href="http://www.ietf.org/rfc/rfc3261.txt">RFC 3261</a>, the allowable transitions are summarized below:
 * <p>
...
public interface ClientTransaction extends Transaction {
	 ...
	 * <p>
	 * Note that both the transaction corresponding to the original request and the CANCEL transaction will complete independently. However, a UAC canceling a request cannot rely on receiving a 487 (Request Terminated) response for the original request, as an RFC 2543 compliant UAS will not generate such a response. Therefore if there is no final response for the original request the application will receieve a TimeoutEvent with {@link javax.sip.Timeout#TRANSACTION} and the client should then consider the original transaction cancelled.
	 * <ul>
	 * <li>UAC - A UAC should not send a CANCEL request to any request explicitly supported by this specification other than INVITE request. The reason being requests other than INVITE are responded to immediately and sending a CANCEL for a non-INVITE request would always create a race condition. CANCELs are useful as a UAC can not send a BYE request on a dialog until receipt of 2xx final response to the INVITE request. The CANCEL attempts to force a non-2xx response to the INVITE, therefore if a UAC wishes to give up on its call attempt entirely it can send a CANCEL.
	 * <li>Stateful proxies - A stateful proxy may generate CANCEL requests for:
	 * <ul>
	 * <li>INVITE Requests - A CANCEL can be sent on pending INVITE client transactions based on the period specified in the INVITE's Expires header field elapsing. However, this is generally unnecessary since the endpoints involved will take care of signaling the end of the transaction.
	 * <li>Other Requests - An implementation of this specification does not need to support CANCELing non-INVITE transactions.
	 * </ul>
	 * </ul>
	 *
	 * @return the new cancel Request specific to the Request of this client transaction.
	 * @throws SipException if this method is called to cancel a request that can't be cancelled i.e. ACK.
	 */
	 ...
	 * @throws SipException if this method is called before a final response is received for the transaction.
	 * @deprecated Since v1.2. As a transaction that received a 2xx response terminates immediately, it cannot be used for creating the corresponding ACK. If this transaction created a dialog, the {@link Dialog#createAck(long)} method should be used. Otherwise the stack will automatically create and send the ACK for non-2xx responses that need to be acknowledged. That is the application should never need to use this method.
	 */
